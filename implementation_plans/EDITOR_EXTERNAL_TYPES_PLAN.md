...
- **前提**: **#1**（直した状態を基準にする）。
- **手法** ✅ **実装済（2026-09-20）**:
  1. `stress.js` に `bind misses` を追加。期待値は**ソースの行スキャン**で作る
     （パーサから取ると一致してしまい何も検出しない）。例題 368 本で 182 束縛を検査。
  2. `tag misses` を追加。`import[lang]` のタグごとに**解析だけの最小フィクスチャ**を持つ。
     ⚠ 例題では足りない: 実測で **`import[cpp-dll]` は `examples/archived/` 以外に 1 本も無い**
     — 索引フックが抜けていたのがまさにこのタグで、例題が無いから全ゲート緑だった。
     例題を足すには DLL の同梱が要るので、エディタビルドが import 先を読まない性質を使って
     解析だけのフィクスチャで代替した。
- **留意点**:
...
- **検証** ✅: #1（`parse_cpp_import_syntax` の索引フック）を実際に外して wasm を再ビルドし、
  **`bind misses 12 / 182` と `tag misses 2 / 14`（cpp-dll / cpp-lib）で落ちること**を確認。
  このとき `hover misses` / `def misses` は **0 のまま**で、これが §1-g で述べた盲点そのもの。
  フックを戻すと両方 0 に復帰。`threw` 0。
...
| #6 | 索引欠落を検出する網 | #1 | ✅ **実装済（2026-09-20）** |
| #7 | 同梱 py スタブ（`time` / `math`）をエディタへ届かせる | — | ✅ **実装済（2026-09-19）** |

**#1〜#7 すべて完了（2026-09-20）。**
