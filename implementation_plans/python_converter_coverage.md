...
**⚠ CPython との意図した違い（B4 で仕様として決着）**: Arrow の `for` のループ変数は
**ブロック内の束縛**で、外側の同名を覆えず（規則 1）、ループを抜けると解放される（規則 2）。
CPython は関数スコープに残すので、下の 2 つは Arrow ではエラーになる。
...
⚠ **これは値の食い違いではなく明示エラー**で、そうなるのが**想定どおり**。
`f` は同じ名前が 2 つの別のものを指し、`g` はループを抜けた後の変数の中身に依存する——
どちらも**本来避けるべき書き方**なので、黙って通さずに止める。

**推奨形**（CPython とも一致する。巻き上げもこの形を想定している）:

```python
def g(xs):
    latest = None
    for x in xs:
        latest = x      # ループ変数をその場で別の変数へ退避させる
    return latest
```
...
- 懸念（限定的）: RHS が **mutable 値を指す変数/式**なら、Arrow の代入は参照を共有するためエイリアスは崩れない（ユーザー確認済み。実機でも `let c = a; a === c`→True）。残る差異は **`a = b = <新規リテラル>`**（例 `a = b = []`）のみ — Python は単一オブジェクトを共有するが分割 `a=[]; b=[]` は別オブジェクトになる。~~この稀なケースが問題なら一時変数化 `__t=[]; a=__t; b=__t` で回避可能。~~
  ⚠⚠ **この見立ては失効した（2026-09-19・実装時に判明）**。Arrow は **代入で複製する**
  （bug_fix.md B8 / B9 / L4「格納も複製する」—— この coverage を書いたあとに決まった規則）。
  一時変数を挟んでも `a` と `b` は**別オブジェクト**になる。純 Arrow で再現する
  **モデル差**で、変換器では埋められない。
...
**✅ 中間オペランドの二重評価は解消した（2026-09-19・D6）— CPython と完全一致**:

脱糖を **2 通り**に分けた。振り分けは**中間オペランドに副作用があるか**だけで決まる。

| 中間オペランド | 脱糖 | 理由 |
|---|---|---|
| 素の名前・定数（`0 < x < 10` — 圧倒的多数） | 従来どおり `and` 連結 | 2 回読んでも観測できない。命令を増やす理由が無い |
| 副作用がある（呼び出し・添字・属性 …） | **早期脱出のブロック式** | 1 回だけ評価する必要がある |

```text
a < f() < c
  →  block->bool:
         mut __py_cmp_0 = f()          # 中間は 1 回だけ
         if not (a < __py_cmp_0):
...
`while 0 < f() < 10:` が 1 回しか評価されない・`p and (0 < f() < 10)` で `p` が偽でも
`f()` が走る、という形で**黙って評価回数が変わる**。`Expr::Block` は VM コンパイラの
**一般の式ディスパッチ**が扱うので、条件式の中でも内包表記の中でもそのまま置ける。

⚠ 一時変数を作るのは「**2 回読まれる（＝中間）**」かつ「**読むと副作用がある**」ものだけ。
末尾は使う直前に評価する位置なのでその場で使う。
**先頭は副作用があれば退避する** —— ⭐ ここを「1 回しか読まないから」で素通しすると
**評価順序が壊れる**（先に積んだ中間の一時変数が先に走り、`v(a) < v(b) < v(c)` が
`b a c` の順になる。実測で踏んだ）。

⭐ **素の名前を退避しなくてよい根拠**は、変換器が `global` / `nonlocal` を明示エラーに
していること（群5）。呼び出しが呼び出し元のローカル名を書き換える経路が無いので、
`a < f() < b` の `a` を後ろで読んでも同じ値になる。

⚠ 一時変数を作っても**同一性（`is`）は動かない** — 呼び出し結果の束縛は複製しない
（実測。例題 ⑬）。

⚠ 検査は例題 [`py_chained_compare.ar`](../examples/interop/py_chained_compare.ar) の
⑦〜⑬（短絡 / 評価回数 / 評価順序 / `while` / `and`・`or` の右辺 / 三項 / 内包表記 / 同一性）。
**21 項目すべて CPython と一致**を実測済み。
...
| 分岐 | 判定 |
|---|---|
| ① メタ関数（`!`） | **既知イディオムのホワイトリスト**。`@dataclass` / `@staticmethod` / `@classmethod` / `@abstractmethod` / `@property`（言語側の計算フィールド実装後）/ 単純なシグネチャ保存ラッパ |
| ② クロージャ（`@`） | 同一ファイル定義で、**禁止機構を使っていない**もの（本体を走査して `setattr` / `type()` / `__dict__` 操作 / ディスクリプタが無いこと） |
| ③ 読み込み時エラー | 禁止機構を検出、または本体が見えない（方針次第） |

- 実現可能性: **骨格は可能。ただし判定の完全性は原理的に得られない**（下記 3 制約）。
- 難易度: 高（前提 2 件を含む）。
...
**制約 3: ②の判定にも本体が要る**
`setattr` / `type()` / ディスクリプタの使用は本体を見ないと分からない。見えなければ検出できず、**読み込み時エラーにできない**（実行時に落ちる）。
...
| コレクション型のアンパック（`*` / `**` 展開を統合） | 明示エラー。⚠ **項目 28 の前提 1**（呼び出し側 `f(*a, **k)` が開かないとデコレータのクロージャ分岐が空振りする） | 代入 `a,b=` / `a,*rest=`、`for a,b in`、呼び出し `f(*xs)` / `f(**d)`、リテラル `[*a]` / `{*a}` / `{**d}` を**すべてこの項目に統合**。Arrow には `LetTuple`・多ターゲット `For`・呼び出しの `f(...=a,b,c)`（`CallArg::Variadic`）が実在するため将来対応可能 |
