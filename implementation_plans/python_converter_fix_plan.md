...
### 5.4 フェーズ G（転送パターン）の現在地

`def wrapper(*args, **kwargs): return inner(*args, **kwargs)` は **U4/U5 の実装で通るように
なった**（`py_call_spread.ar` の ④）。
...
def deco(f):
    def wrapper(*a, **k):
        return f(*a, **k)      # NameError: 'f' is not defined
    return wrapper
```

残っているのは §5.1 #2 の **A7（関数値の `mut` パラメータが入れ子 `fn` から見えない）**だけで、
引数展開とは無関係。⇒ **G の残作業は A7 の 1 件**。
