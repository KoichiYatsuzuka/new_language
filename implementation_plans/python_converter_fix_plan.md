...
| 5 | **`list()` 組込みが無い**（`set()` はある）。そのため `[*a]` の脱糖が**展開元をリストに限る**（他はループしく `TypeError`。黙っては壊れない） | ⏳ 仕様として据え置き | C6 |
| 6 | **`dict()` コンストラクタが無い**。`{**d1, **d2}`（U3 の dict 部分）と辞書内包（FUTURE_FEATURE §4 (4-a)）の**共通の前提**。`items()` が入ったので `dict(d1.items() + d2.items())` で両方片付く | ⏳ 未着手・**起票候補** | C6 |
