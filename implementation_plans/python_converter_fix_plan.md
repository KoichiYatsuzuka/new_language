...
| 5 | ~~`list()` 組込みが無い~~ → **あった**。`eval_type_call` に完全な実装があるのに**グローバルに名前が登録されておらず**到達できなかった（配線漏れ） | ✅ 修正（名前を登録＋ Tuple / Dict も受けるよう `set()` と揃えた） | C6 → 組込み追加 |
| 6 | **`dict()` コンストラクタが無い**（`dict` は `Value::Type` として**登録済みなのに** `eval_type_call` にケースが無く `'dict' object is not callable`。`list` とちょうど**逆向きの配線漏れ**）| ✅ 追加（空 / ペア列 / 複製。対でない要素は明示エラー）| C6 → 組込み追加 |
...
| 9 | **`open()` が Python と非互換**。Arrow は `open(path, FileOpenMode.rewrite)` の**2 引数必須**で、Python の文字列モード（`open(p, "w")`）も 1 引数版（`open(p)`）も `TypeError`。⇒ **項目25（`with`）を実装しても最頻出の `with open(...) as f:` は通らない**。`block` 退出でファイルが閉じること自体は実測で確認済み（脱糖の前提は成立） | ⏳ 未着手。**要判断**（変換器が `"w"` → `FileOpenMode.rewrite` を写すか）| H1 調査 |
...
| 3 | **H1**（項目25 `with`）を**今は**実装しない | 脱糖の前提（`block` 退出で閉じる）は成立するが、**`open` が Python と非互換**（5.2 #9）で最頻出の `with open(...) as f:` が通らない。`open` のモード写し替えは**方針判断が要る**ので、決まるまで着手しない。`with` 単体を入れても価値がほぼ出ない |
