...
| | D6 ✅ | 項目16 の中間オペランド二重評価を 1 回評価へ（2026-09-19 完了）⚠ **持ち上げず、その場にブロック式を埋める**のが鍵だった（§5.7）|
...
| 2 | 項目20 †1「`mut` パラメータが入れ子 `fn` にキャプチャされない（**None になる**）」 | int などの値は **B3 で解決済み**。残るのは**関数値**のみで、症状も `NameError`。⇒ さらに切り分けると **2 件の別々の穴**（A7-a: `mut` 実引数の検査／A7-b: callee の `cells` 解決）だった。**両方 2026-09-19 に修正**（§5.4）| A7 |
...
| 8 | ~~`for k in d:` が反復不可~~ → **解決（2026-09-19）**。Python と同じく**キー**を回す。型検査（`check_iterable` / `for_element_type`）と実行時（`make_for_iterator` / `collect_iterable`）の**両方**に入れた | ✅ 解決 | C5 |
...
| # | 項目 | 理由 |
|---|---|---|
| 1 | ~~**D6**（項目16 の中間オペランド二重評価の解消）~~ → **撤回して実装した（2026-09-19・§5.7）**。「短絡があるので完全には直せない」は**持ち上げ（INF-B）しか手が無いと思い込んでいた**もの。`Expr::Block` を**その場に埋める**と短絡も評価位置も保たれる ⇒ 4 オペランド以上でも直り、不整合は残らなかった |
| 2 | ~~フェーズ F をやらない~~ → **撤回して実装した**（2026-09-19）。別スレッドが D34 で扱わないことになったため。⇒ `{**d}`・`f(*xs)`・`f(**d)` を **Arrow の構文として**追加済み |
| 3 | **H1（項目25 `with`）は実装したが、`__exit__` を持つ他モジュール由来のクラスは検出できない** | `hasattr` 相当の組込みが無く**実行時ガードを組めない**。判定は同じモジュールで定義されたクラスに限る。組込みを足すのは Arrow 側への露出なので、方針が決まるまでやらない |

### 5.4 フェーズ G（転送パターン）✅ **完了（2026-09-19）**

デコレータの転送パターンが CPython と一致するようになった（`py_decorator_forward.ar`）。

⚠⚠ **A7 は 1 件ではなく 2 件の別々の穴だった**（切り分けて初めて分かった）:

| # | 症状 | 原因 | 直し方 |
|---|---|---|---|
| A7-a | 関数値を渡すと `parameter 'f' of 'deco' expects a mutable argument` | 変換器が全パラメータを `mutable: true` にする一方、関数名・モジュールメンバは不変 | **関数値を `mut` 実引数の検査から除外**。実測した `mut` の意味論は「スカラは値渡し・コンテナは参照」で、**関数値は書き戻しようがない**。⚠ `is_python` 限定にしない（純 Arrow の `fn deco(mut f: function)` も同じく通らなかった） |
| A7-b | 捕捉した関数を呼ぶと `NameError: 'f' is not defined` | **値として読む経路は `cells` を見ているのに、呼び出し先の名前解決だけが見ていなかった** | VM コンパイラの callee 解決に `cells` 分岐を足す（`slots` より先・`Expr::Ident` と同じ順序） |

⭐ **切り分けの決め手**は次の非対称だった:

```text
捕捉して返すだけ              → 動く（関数値が返る）
捕捉して直接呼ぶ `f(x)`       → NameError
...
| 起票先 | 内容 | 出所 |
|---|---|---|
| §5 (z) | **`for` のループ変数を捕捉する入れ子 `fn` が VM に載らない**（`VmForceError`）。`while` 本体・ループ変数以外の捕捉・ループ外は通る | §5.2 #3 |
| §5 (f) | **関数の中の `block` 退出でリソースが解放されない**。最上位の `block` と関数退出では解放される。`FileObject` に `close()` が無いので明示クローズもできない | §5.2 #10 |
| §5 (g) | **他モジュール由来のコンテキストマネージャを `with` が検出できない**。`hasattr` 相当の組込みが無く実行時ガードを組めない | §5.3 #3 |
| §5 (h) | **循環 import が CPython と違う**（CPython は通る・Arrow は明示エラー）。AST を確定してから型検査する形なので部分初期化のモジュールを持てない | §5.5 |
| §5 (i) | **Python モジュールの関数内 import が充填されない**。遅延読み込みの意図を保つかは設計判断 | §5.5 |
| §5 (j) | **`compare_wasm_frontend.ps1` が wasm を再ビルドしない**（存在確認のみ）＝ 古い成果物で緑になる。この作業中に **3 回**踏んだ | §5.2 #4 |
| §5 (k) | **`cargo build --release` が緑でも `cargo test` のコンパイルが落ちる**（テストは別ビルド）。AST を触ったら `cargo test` まで回す運用で回避中 | §5.2 #12 |
| **#90** | **coverage 項目28（デコレータの三分岐）** — ①の行き先である**メタ関数が未実装**なので分類しても受け皿が無い。⏸ **メタ関数の実装後に対処**（ユーザー判断・2026-09-19）。前提は #89 | coverage 28 |
| 未起票 | **辞書リテラルが値を複製する** — `L = [9]` / `D = {"k": L}` として `D["k"] is L` が `False`（CPython は `True`）。⚠ **変更を通すと書き戻る**（`D["k"].append(..)` は効く）ので、ずれるのは `is` の同一性だけ | §5.7 |

### 5.7 D6（連鎖比較の中間オペランド二重評価）✅ **完了（2026-09-19）**

`import[py]` の連鎖比較が **CPython と完全一致**した（短絡・評価回数・評価順序・
評価位置・同一性の 21 項目を実測）。

⚠⚠ **§5.3 #1 の「完全には直せない」は思い込みだった。** 当時は脱糖手段を
**INF-B（文への持ち上げ）だけ**で考えていて、「`a<b<c<d` の `c` を持ち上げると
条件付き評価が無条件になる」で詰まっていた。⭐ 実際には
**`Expr::Block` をその場に埋める**という手がある:

- `Expr::Block` は VM コンパイラの**一般の式ディスパッチ**（`compile_expr`）が扱うので、
  **式の位置ならどこにでも置ける**（パーサは `f(block->bool: ...)` を受け付けないが、
  変換器は AST を直接組むので構文の制約を受けない）。
- 埋めるので**評価位置が動かない** ⇒ `while` の条件・`and`/`or` の右辺・三項の腕・
  内包表記の中でも、持ち上げたときのような「黙って評価回数が変わる」事故が起きない。

| 中間オペランド | 脱糖 |
|---|---|
| 素の名前・定数（`0 < x < 10`） | 従来どおり `and` 連結（2 回読んでも観測できない） |
| 副作用あり（呼び出し・添字・属性） | 早期脱出のブロック式（`block_return False` で抜ける） |

⭐ **踏んだ落とし穴（実測で見つけた）**: 先頭オペランドを「1 回しか読まないから」で
一時変数にしないと、**評価順序が壊れる**。`v("a") < v("b") < v("c")` で先に積まれる
`mut __py_cmp = v("b")` が先に走り、`b a c` の順になっていた
（CPython は `a b c`）。⇒ **先頭も副作用があれば退避する**。

⭐ **素の名前を退避しなくてよい根拠**: 変換器が `global` / `nonlocal` を明示エラーに
している（群5）ので、呼び出しが呼び出し元のローカル名を書き換える経路が無い。
⇒ `a < f() < b` の `a` を後ろで読んでも同じ値。**C1 の明示エラー化がここで効いた**。

⚠ 一時変数を作っても**同一性（`is`）は動かない**（呼び出し結果の束縛は複製しない・実測）。
⚠ ついでに判ったこと: **辞書リテラルは値を複製する** — `D = {"k": L}` として
`D["k"] is L` が `False`（CPython は `True`）。⇒ D6 とは無関係の既存差。§5.6 へ起票。

**ゲート**: `compare_bytecode -A` **316/317**・`compare_outputs -A` **297/298** で、
差は**書き換えた `py_chained_compare.ar` 1 本だけ**＝ 他の例題のバイトコードは 1 命令も
変わっていない。`force_gate` 0 件（ブロック式が `while` の条件や内包表記に入っても
VM に載る）/ `scan_examples` / `compare_python_impl` 101 一致 /
`compare_import_paths -A` 13/13 / `type_obligations` STATIC 112/114 退行なし /
`cargo test` 784 / `clippy` 該当ファイル警告 0 / `stale_doc_refs` 緑。
