...
### D-15 異型の等値比較（`==` / `!=` / `in`）は静的エラー（変種 C）

⚠⚠ **タスク 4.4 の撤回を覆した決定**（2026-09-16 に利用者が決定・7.6 で実装）。

| 論点 | 決定 |
|---|---|
| 異型の `==` / `!=` / `in` | **静的エラー**（同型・数値族・`__eq__` の宣言で受ける場合を除く） |
| `x == None`（非 `Option`） | **弾く**。`None` になりうる型だけが書ける |
| 変種 | **C（overload）** — 数値族は相互に許し、クラスは `__eq__` の宣言で判定 |

⚠ 実行時の意味論は変えない。Python 翻訳時は異型比較を許す前提。
⚠ `Option` の `None` 判定は `is None`（`== None` は `OperationOnUnion` で元から不可）。
...
| ~~**7.5**~~ | ~~メンバーの存在検査~~ → **✅ 完了 2026-09-17** | `M1` `M2` | 7.8 | 下記「7.5 の記録」。⇒ **`RUNTIME` が 0 件に** |
| ~~**7.6**~~ | ~~異型の等値比較を静的エラーにする~~ → **✅ 完了 2026-09-17** | `O4` `O5` `O8` | — | 下記「7.6 の記録」。決定 **D-15** |
| ~~**7.7**~~ | ~~リテラル要素の個別照合~~ → **✅ 完了 2026-09-17** | `L17` | — | 下記「7.7 の記録」。⇒ **フェーズ 7 完了** |
...
##### 7.5 の記録 【✅ 完了 2026-09-17・⇒ **`RUNTIME` 0 件**】

###### 偽陽性 24 件 → **0 件**（真陽性 5 件は残る）

タスク 7.8 の後に測り直して 28 → 24 件（外部言語クラスの 5 件が消えた）。
そこから**レジストリの穴を 5 つ埋めて 0 件**にした:

| 原因 | 件数 | 直し方 |
|---|---|---|
| `new_type` の `value` が未登録 | 6 | 基底型で登録。実行時の `make_primitive_wrapper_class` は**宣言済みフィールド**として合成しているのに、静的側は `class_method_sigs` しか引き継いでいなかった |
| protocol を普通のクラスとして引いていた | 6 | `registry.protocol()` の要求メンバーを見る |
| enum 型自身に `value` が無い | 1 | `let m: Color = Color.Green`（2.3 のアップキャスト）の後 `m.value` を引けるように、変種側だけでなく**enum 型の表にも**登録 |
| `Error` にフィールド未登録 | 2 | 組み込み例外と同じ 5 つを `Error` 自身にも。利用者の `class MyErr(Error)` が継承できるようになる |
| 全インスタンス共通の `copy` | 1 | 実装側（`method_call.rs`）がクラスの表を引く**前に**特別扱いしているものを表に追加 |
| 情報を 1 つも持たないクラス | 7 | 下記の「開いている」規則で素通し |
| 意図的な実行時 `AttributeError` の実演 | 1 | 例題を移行（4.4 の `try_except.ar` と同型） |

###### ⚠⚠ 「情報が無い」と「メンバーが無い」を取り違えない

レジストリにフィールド表もメソッド表も基底も無いクラスは、
**「メンバーが無い」のではなく「こちらが知らない」**。素通しする。該当したもの:
...
⇒ これが**タスク 7.8 で設計した `member_set_is_closed` の実体**。
⚠⚠ **真偽値に潰さずクラス単位の性質として持つこと。** スタブが入ったとき、言語によって
「スタブ＝完全な宣言（閉じる）」「部分的な宣言（開いたまま）」が分かれる。

###### ⚠ `M2` は別経路だった

`M1`（フィールドの読み）を `infer_attr` で閉じても `M2`（メソッドの呼び出し）は動かなかった。
メソッド呼び出しは**オブジェクトを二度推論しないため `infer_attr` を通らず**、
`infer_call_inner` が `self.infer(object)` を直接呼んでいる。⇒ **両方に書く**必要があった。

###### ⚠ 全インスタンス共通メソッドは**実測で確かめた**

`copy` だけが該当。`keys` / `values` / `clear` / `items` / `get` / `add` / `append` / `pop` は
どれも `AttributeError: 'C' has no method ...`（推測で足していたら偽陽性を撒いていた）。
⚠ 実装側に共通メソッドを足したら `UNIVERSAL_INSTANCE_METHODS` にも足すこと。

###### 結果

**検体**: `M1` `M2` が `RUNTIME` → **`STATIC`**。静的検査の割合 93% → **95%**（108/114）。
⇒ **`RUNTIME` が 0 件になった**（残る 6 件はすべて `NONE`）。

⚠ 副産物: `new_type` の `.value` と enum の `.value` に**型が付いた**ので、
`let s: str = Celsius(1.0).value` のような誤りも捕まるようになった。
...
| `cargo test --release` | 775 passed / 0 failed |
| `scan_examples.ps1` | 既知の `bench_ab_native.ar` TIMEOUT のみ |
| `force_gate.ps1` | 0 fall back（310 例題） |
| `compare_python_impl.ps1` | 93/93 identical・stale 0（py は protocol のフィールド要求を method と誤判定するので `$knownDiff` に登録） |
| `compare_outputs.ps1 -A 3b03672` | 240/241。差分は**新規例題 1 件のみ** |
| `stale_doc_refs.ps1` | OK |
| `compare_wasm_frontend.ps1` | **311/311 agreed・INVENTED 0** |

**追加した例題**: `examples/classes/member_existence{,_error}.ar`
**移行した例題**: `classes/attr_access_paths.ar`（`o.nope` を不透明な経路経由に）
...
##### 7.6 の記録 【✅ 完了 2026-09-17・決定 D-15】

###### タスク 4.4 の撤回を覆した

4.4 では「異型の等値比較は `False` を返す**仕様**」として厳密化を撤回した。7.6 で測り直すと:

| | 4.4 の想定 | 7.6 の実測 |
|---|---|---|
| 仕様を主張している場所 | 「既存例題が固定している」 | **例題 1 ファイルだけ** |
| 移行コスト | （測っていない） | 全比較地点 **237** のうち **10 箇所（4.2%）**・**2 ファイル**に集中 |

⇒ **コスト見積もりが過大だった。** ⚠ 実行時の意味論は**一切変えていない**
（`1 == True` は今も `False`）。

###### ⚠⚠ 実装して分かったこと 2 つ

**① `Any` と `Union` はこの検査に届かない。**
`check_binop` の冒頭が「`Any` / `Union` を被演算子にする二項演算」を
`OperationOnAny` / `OperationOnUnion` で**先に弾いている**（7.6 より前からの規則）。
⇒ 素通しの表に `Any` / `Union` を書いても効かない。実際に届くのは `Unresolved` など。
⚠ 表には多重防御として残してある（前段が変わったときに穴にならないため）。

**② `Option[T] == None` は 7.6 より前から静的エラーだった。**
①の `OperationOnUnion` が `==` も対象にしているため。
⇒ **Arrow に `== None` という作法はもともと無い**。正しい書き方は **`is None`**
（型ガードなので絞り込みも効く: `if x is not None: print(x + 1)`）。

⇒ 利用者の決定「`Option` 以外の `None` を弾く」の効果は
**「`int == None` のような非 Option との比較を弾く」**ことで、`Option` 側の書き方は変わらない。
...
4. どちらかのクラスが **`__eq__` の宣言で相手を受ける**なら通す（継承チェーン込み）

⚠ 3 は必須。実行時が `uint → int → float` の昇格ラティスで比べるので、厳密にすると
`if n == 0`（`n: float`）が落ちる（変種 A で実測したとき +6 箇所だった）。
⚠⚠ **`bool` はラティスの対象外**なので `x == True`（`x: int`）は弾かれる。
⚠ `in` は右辺の**要素型**と比べる（`dict` はキー型）。

###### 移行した例題（実測どおり 2 ファイル）

| 例題 | 箇所 | 移行の形 |
|---|---|---|
| `basics/equality_numeric_promotion.ar` | 8 | **この仕様を実演している例題**。`hide(..)` で型を隠して**実行時の答えを固定し続ける**（出力は 1 文字も変えていない） |
| `typing/arith_operand_check.ar` | 2 | 同じ形 |

⚠ ユニットテスト 2 件（`eq_different_types_ok` / `neq_different_types_ok`）も
`err` へ反転して改名した。**仕様変更なので当然の更新**だが、
「テストが仕様を主張している」形は 7.2 と同じなので記録しておく。

###### 結果

**検体**: `O4` `O5` `O8` が `NONE` → **`STATIC`**。静的検査の割合 95% → **97%**（111/114）。

**ゲート結果**

| ゲート | 結果 |
|---|---|
| `cargo test --release` | **778 passed** / 0 failed（2 件を反転・3 件追加） |
| `scan_examples.ps1` | 既知の `bench_ab_native.ar` TIMEOUT のみ |
| `force_gate.ps1` | 0 fall back（312 例題） |
| `compare_python_impl.ps1` | 93/93 identical・stale 0 |
| `compare_outputs.ps1 -A c639b56` | 242/243。差分は**新規例題 1 件のみ** |
| `stale_doc_refs.ps1` | OK |
| `compare_wasm_frontend.ps1` | **313/313 agreed・INVENTED 0** |

**追加した例題**: `examples/typing/cross_type_equality{,_error}.ar`
...
##### 7.7 の記録 【✅ 完了 2026-09-17・⇒ **フェーズ 7 完了**】

###### ⚠⚠ join の規則は**変えなかった**

素直な直し方は「`join_elem_types` が `Unresolved` を無視して既知だけ合成する」だが、
それは**下流で偽陽性**を生む:
...
⇒ タスク 5.5 で 3 箇所に揃えた「判らない側が混ざったら全体が判らない」を崩さない。
**推論する型は変えず**、期待型が判っている束縛地点でだけ要素を個別に見る。

###### ⚠ 見るのは「リテラルの要素」だけ — 二重報告を避けるため

要素式を再推論すると**診断が二重に出る**（束縛の推論で既に通っている）。
⇒ 型が式だけで決まり**診断を出しようがない**リテラル（`1` / `"s"` / `True` / `None` /
浮動小数 / 虚数）に限って照合する。`zs[0]` のような式には触らない。

⚠ `join` が汚染されていないときは通常の `types_compatible` が既に見ているので、
この検査は**汚染されたときだけ**働く（二重報告しない）。

###### 未使用の引数が既にあった

`resolve_declared_type` は `_stmt: &Stmt` を受け取って**捨てて**いた。
リテラルの要素式を取るのに必要だったので、そのまま使えた（新しい引数は足していない）。

###### 結果

**検体**: `L17` が `NONE` → **`STATIC`**。静的検査の割合 97% → **98%**（112/114）。
⚠ 実測どおり**既存例題への影響は 0 件**（「既知と判定不能が混ざったリテラル」は
例題 270 件に 1 つも無かった）。
...
| ゲート | 結果 |
|---|---|
| `cargo test --release` | 778 passed / 0 failed |
| `scan_examples.ps1` | 既知の `bench_ab_native.ar` TIMEOUT のみ |
| `force_gate.ps1` | 0 fall back（314 例題） |
| `compare_python_impl.ps1` | 94/94 identical・stale 0 |
| `compare_outputs.ps1 -A 57f81c0` | 244/245。差分は**新規例題 1 件のみ** |
| `stale_doc_refs.ps1` | OK |
| `compare_wasm_frontend.ps1` | **315/315 agreed・INVENTED 0** |

**追加した例題**: `examples/typing/literal_element_check{,_error}.ar`
...
| | 着手前 | 完了時 |
|---|---|---|
| 静的に検査されている型義務 | 84%（96/114） | **98%（112/114）** |
| `NONE` | 7 | **2** |
| `RUNTIME` | 11 | **0** |

**残る 2 件はどちらも仕様**（閉じてはいけない）:

| 検体 | 内容 | 根拠 |
|---|---|---|
| `L11` | 注釈なしの空リテラル `let xs = []` | D-11 / U-6。`list[Any]` として legal |
| `O7` | `and` の被演算子型が違う | D-10。join を返すので正当な式。誤りは条件位置で D-12 が弾く |

⚠ ⇒ **型義務の網としては上限に到達した。** 以降は
フェーズ 8（要素型を捨てる経路）・フェーズ 9（campaign 中に見つけた本体側のバグ）が残る。
...
| # | 内容 | 移行コスト | 状態 |
|---|---|---|---|
| **8.0** | **`generator` 注釈が静的側の表に無い** | **0**（実測） | ✅ **完了 2026-09-17**（下記） |
| **8.1** | 素の `list` / `dict` / `set` / `fixed_list` / `list_like` / `tuple` 注釈を禁止 | **55 箇所 / 28 ファイル** | ✅ **完了 2026-09-18**（利用者の決定: **案 A**・下記） |
| **8.2** | 解決できない要素型が黙って捨てられる | **0**（実測） | ✅ **完了 2026-09-17**（下記） |
| **8.3** | `tuple` 注釈を解決できるようにする | **0**（実測） | ✅ **完了 2026-09-17**（8.2 と同時・下記） |
| **8.4** | 内部で素の容器型が生まれる経路の棚卸し | **0**（実測） | ✅ **完了 2026-09-17**（下記） |
| **8.5** | **未知の型名の妥当性検査が無い**（8.2 で判明） | **1 箇所**（実測） | ✅ **完了 2026-09-18**（下記） |

⚠⚠ **「8.2 / 8.3 は 8.1 に吸収」という起票時の判断は誤りだった。** 実測し直すと、
8.2 の真因は「素の容器型を許すか」ではなく **`from_ann` の大文字/小文字の非対称**で、
8.1 の決定とは**独立に直せた**。8.3 も表に 1 行足して変種を 1 つ作るだけで、
`tuple[...]` 必須（＝ 8.1 の方針）を先取りする必要は無かった。
⇒ **8.1 は「素の容器型注釈を禁止するか」だけの判断として残っている。**

#### 8.0 `generator` 注釈が静的側の表に無かった 【✅ 完了 2026-09-17】

実行時には型名として実在する（`runtime_type_name` の `Value::Generator(_) => "generator"`）のに、
`InferredType::from_ann` に枝が無く **`Unresolved`（＝万能受容体）に化けていた**。

⚠ **例題が実際に注釈として使っている**（`fn counter_from(let n: int) -> generator:` など
**9 箇所**・`basics/generator_closure.ar` ほか）。⇒ 注釈が黙って捨てられていた。

⇒ `NamedInstance("generator")` として解決するようにした。専用の変種は作らない
（メンバー情報を持たないので存在検査は素通しになる＝タスク 7.5 の `member_set_is_closed`）。

**ゲート**: `compare_outputs -A ee000f6` が **245/245 完全一致**、
`compare_wasm_frontend` 315/315、`type_obligations` 98% 退行なし、`cargo test` 778 passed。

#### 8.2 / 8.3 の記録（2026-09-17・完了）— **1 行の非対称が 8 箇所の原因だった**

##### 先に実測した（7 形）

| 書いたもの | 修正前 | 修正後 |
|---|---|---|
| `mut xs: list[foo] = [1]` → `xs.append("s")` | ⛔ **通る**（`[1, 's']`） | 静的エラー |
| `fn f(xs: list[foo])` / `fn g() -> list[foo]` / フィールド | ⛔ **通る** | 静的エラー |
| `let d: dict[str, foo] = {"a": 1}` | ⛔ **通る** | 静的エラー |
| `let s: set[foo] = {1}` | ⛔ **通る** | 静的エラー |
| `let t: tuple[int, foo] = (1, 2)` | ⛔ **`tuple[int]` という嘘の診断**で落ちる | 正直な診断 |
| `let x: foo = 1` | ⛔ **通る** | 静的エラー |
| `let t: tuple = 1` / `let b: tuple = [1,2]` | ⛔ **通る** | 静的エラー |

##### 真因は「脱落地点が 8 つある」ことではなかった

`InferredType::from_ann` の末尾は **未知の大文字名だけ**を `NamedInstance` として受け、
小文字始まりの未知名は `None` を返していた。`None` は呼び出し側で
`Unresolved`（＝**万能受容体**）に倒れる。この 1 つの非対称が

```
list[foo] / fixed_list[foo] / list_like[foo] / set[foo]   … None => Self::List 等へ落ちる
dict[str, foo]                                            … Some(Self::Dict) へ落ちる
tuple[int, foo] / Union[int, foo] / Intersection[A, foo]  … filter_map が黙って 1 要素消す
```

の **8 箇所すべての引き金**だった。`tuple` / `Union` / `Intersection` は `filter_map` なので
**要素数まで変わる**（`tuple[int, foo]` が `tuple[int]` になり、診断が嘘になる）。

⇒ 脱落地点を 8 か所塗るのではなく、**非対称そのものを消した**（小文字も `NamedInstance`）。
存在しない型は**整合性検査が正直な診断で弾く**ので、新しいエラー種別も要らない。

⚠ `np.ndarray` のような `.` を含む名前は従来どおり `None`（英数と `_` のみを受ける）。

##### 8.2 と 8.3 は**同時でなければならなかった**

8.2 を入れた時点で `tuple` が `NamedInstance("tuple")` に化け、
**正しい `let t: tuple = (1, 2)` まで弾く偽陽性**が出た（実測）。
`tuple` は `from_ann` のプリミティブ表に無かった（8.3 の本体）ので、
非対称を消した副作用でいきなり表面化した形。⇒ 2 つは 1 つの変更として入れた。

`InferredType::TupleAny` を新設。既存の `Tuple(Vec<_>)` は**要素数が固定**で素の `tuple` を
表せないため、`ListOf` に対する `List` と同じ位置づけの変種を足した。
適合は **`tuple[...]` → `tuple` の一方向だけ**（D-3）。

⚠ `TupleAny` を足したとき、`ffi_boundary.rs` の**網羅 match がビルドを止めた**
（＝仕掛けが効いた）。これが効くのは **9.8 で debug ビルドを直した直後**だから、という点も記録しておく。

##### 移行コスト: **0**（例題側の修正なし）

`scan_examples` は既知の `bench_ab_native.ar` TIMEOUT のみ、`force_gate` 0 件、
`compare_bytecode` は 9.1 基準と **265/265 完全一致**（純粋に静的側の変更）、
`compare_outputs` は**新設した 3 例題だけ**が差分（既存 247 件は完全一致）、
`compare_import_paths` 13/13、`compare_wasm_frontend` **320/320**、
`type_obligations` 98% 退行なし、`cargo test` 778 passed。

⚠ **新設した例題が基準バイナリで落ちること自体が証拠**になっている:
`elem_type_kept.ar` を 9.1 のバイナリで走らせると
`'pair' is declared 'tuple[int]' but initialized with 'tuple[int, point]'` という
**アリティが脱落した嘘の診断**が出る。

##### 起票時の記述で誤っていたもの

- ⛔ 「⚠ 大文字の未知型は既に弾かれている」 → **右辺が食い違うときだけ**。
  `let xs: list[Foo] = []` は空リテラルが上位互換なので**素通りする**（実測）。
  ⇒ これは 8.2 の守備範囲ではなく、**未知の型名の妥当性検査が無い**という別の穴。⇒ **8.5** へ起票。
- ⛔ 「移行コストは `list[tuple]` ×2 だけ」 → 箇所の数は合っていたが、
  影響範囲は `list[foo]` に限らず **`let x: foo` のような素の未知名**も含んでいた。

#### 8.5 未知の型名の妥当性検査が無い（8.2 で判明・新規起票）

```arrow
let xs: list[Foo] = []      # ⛔ Foo というクラスはどこにも無いのに通る
let ys: list[foo] = []      # ⛔ 同上（小文字も同じ）
```

注釈が**存在しない型名**を書いていても、それ自体は検査されない。
`NamedInstance("Foo")` は作られるが、**右辺と食い違ったときにしか露見しない**。
空リテラル（`list[Never]`）は任意の `list[T]` へ上位互換なので、この形では露見しない。

⚠ 起票時に「妥当性検査（タスク 3.4）が `Foo` を未知クラスとして弾く」と書いたが、
**そのような検査は存在しない**（実測で確認）。3.4 が作ったのは `UnknownGuardType`
＝ `is` / `except` / `match` の**ガード位置**の検査で、注釈位置は対象外。

⇒ 「注釈に現れる型名は実在しなければならない」という妥当性検査を足す。

##### 記録（2026-09-18・完了）

##### 先に数えた（起票時の指示どおり）

`AR_ANN_UNKNOWN=1` で「レジストリに無い注釈名」を出す計測版を作り、例題 393 件に掛けた。
**3 回測り直している**（測るたびに計測側のバグが出た）:

| 回 | サイト | ファイル | 何が判ったか |
|---|---|---|---|
| 1 回目 | 60 | 20 | `T` が **17 件**。テンプレート型変数を誤判定していた |
| 2 回目 | 46 | 7 | `T` が 3 件残った。**trait の型変数**だけ落ちていた |
| 3 回目 | **43** | **4** | `T` は 0。残りは py スタブ 42 ＋ 本物 1 |

**最終的な内訳**:

| 区分 | サイト | 中身 |
|---|---|---|
| py スタブ由来の外部型 | **42** | `data_analysis.ar` 40（matplotlib の `Figure` `Axes` `Line2D` …）＋ `NoReturn` 2 |
| 本物の未知名 | **1** | `elem_type_kept_error.ar` の `foo`（意図的に書いたもの） |
| テンプレート型変数 | **0** | 下記 2 件のバグを直した結果 |

##### 計測そのものが見つけたバグ 2 件

1. **`check_fn_def` / `check_gen_def` が注釈を型変数の投入より前に見ていた。**
   `is_type_param` は**スコープ状態**なので、`push_type_params` の前に見ると
   `fn f[T](x: T)` の `T` が「存在しない型名」になる。⇒ 検査を投入後へ移した。
2. **`TraitDef` が型変数を積んでいなかった。** `ClassDef` は積んでいたのに
   `trait Holder[T]: let item: T` の `T` が**スコープから見えない**まま本体を検査していた。
   ⇒ `ClassDef` と同じく積むようにした（タスク **9.9** と同じ根の一部）。

##### 決め手: 外部スタブは**もともと診断が捨てられている**

起票時の心配（「py / C# のスタブが持ち込む型名で相互運用が壊れる」）は**杞憂だった**。
[`annotate_module_body`] が import 先を歩く前後で
...
としており、**モジュール本体で出た診断は捨てられる**。42 箇所はすべてそこに入るので、
普通に `report_error` で書くだけで利用者には出ない。
⇒ 「外部由来なら飛ばす」という分岐を**検査側に書かない**こと。モジュール境界の判断は
`annotate_module_body` が 1 箇所で持っている。

##### ⚠⚠ 例題は無傷だったが、**エディタが 6 件の偽陽性**を出した

`scan_examples` も `force_gate` も `compare_outputs` も緑だったのに、
`compare_wasm_frontend` が **INVENTED 6** を出した:

```
flat_bench.ar / partial_call_overhead.ar / cpp_out_param_writeback.ar /
cpp_struct_ptr.ar / cross_module_annotation.ar / swd_nested_runner.ar
```

VS Code 拡張の wasm フロントエンドは **fs に触れないので import 先を読まない**。
import 文の body が空になり、その先で定義された型（`Particle` `Item` C の構造体…）が
レジストリに載らないため、**import した型を全部「存在しない」と言っていた**。

⇒ `TypeChecker::registry_incomplete` を足し、
「本体を読み込めていない import があるなら検査しない」ようにした。
⚠ **`cfg!(feature = "editor")` で直接分岐していない。** 「編集中だから」ではなく
「レジストリが不完全だから」止める、と書いておかないと、将来 import を解決する
編集環境が出たときに誤って止め続ける。

⚠ これで **INVENTED は 0 に戻り 323/323 一致**。ネイティブ側では
`import[ar]` を解決した状態でも検査が効くことを実測で確認した。

##### 何を「実在する型名」とみなすか

クラス ・ `enum` ・ `new_type`（`is_known_class` が 3 つとも含む）・ `trait` ・
`protocol` ・ **いま見えているテンプレート型変数** ・ `generator`（タスク 8.0 で
実行時の型名として入れた、クラスとしては登録されない名前）。

##### ゲート

`cargo test` 782 passed ／ `scan_examples` 既知の TIMEOUT のみ ／ `force_gate` 0 件（322）／
`compare_bytecode` 負の対照 272/272・8.1 基準比は**新設した error 例題 1 件のみ**／
`compare_outputs` 251/253（新設 1 ＋ 診断が 1 件増えた `elem_type_kept_error.ar`）／
`compare_import_paths` 13/13 ／ `compare_python_impl` 95/95 clean ／
`compare_wasm_frontend` **323/323 INVENTED 0** ／ `type_obligations` 98% 退行なし ／
`stale_doc_refs` OK。

例題: `examples/typing/unknown_type_name{,_error}.ar`

⚠ `examples/practical_examples/pd_numpy_pyplot/data_analysis.ar` は 8.1 の時点から
`parameter 'backend' of 'use' expects a mutable argument` で落ちる**既存の破損**
（ungated・タスク 9.3 の対象）。8.5 とは無関係であることを基準バイナリで確認した。

#### 8.1 の決定と記録（2026-09-18・完了）— **案 A ＋ 型は Python 翻訳用に残す**

##### 利用者の決定

> arrow 内部では A 案で通し、不明瞭な list は作れないようにします。
> ただし、list 型自体は Python 翻訳のために残してください。
> list 生成は純 arrow コードの時の経路から隔離させ、Python 翻訳時にあとで繋げます。

⇒ **注釈位置では禁止、型そのものは残す**という 2 段構えで入れた。

##### 何をどこで弾いているか

| 位置 | 例 | 8.1 以降 |
|---|---|---|
| **注釈** | `let xs: list` / `fn f(v: list)` / `-> list` / `mut f: list` / `->list`（ブロック式） | ⛔ **静的エラー** |
| **型テスト** | `x is list` / `x mustbe list` / `case list:` | ✅ 通る（「list かどうか」を問う正しい用法） |
| **キャスト先** | `__cast__[list]` / `cp => list` | ✅ 通る（注釈位置ではない） |

⚠⚠ **型テスト位置を巻き込まないことが実装上の要点。** `parse_type_expr` は
注釈・型テスト・キャストの**すべてから呼ばれる**ので、そこで弾くと
`runtime_type_predicates.ar` の `xs is list` まで落ちる。
⇒ **型検査側の注釈入口 5 箇所**に `check_ann_not_bare` を置いた
（`let`/`mut`/`const` ・ クラスフィールド ・ `fn` の引数と戻り値 ・ `gen` の引数と yield 型 ・
ブロック式の `->T`）。ブロック式は 5 種類あるが `ann_or_none` 1 本を通るのでそこに置いた。

##### 型は残した（Python 翻訳用の隔離）

`InferredType::List` / `Dict` / `Set` / `FixedList` / `ListLike` / `TupleAny` は**残っている**。
生成地点は `from_ann` のプリミティブ表ただ 1 つで、そこに `⚠⚠⚠` 付きで
**「Python 翻訳のために残してある・消すな」**と明記した。

⚠ `from_ann` の入力は Arrow のソースだけではない。`.pyi` スタブ・`--compile-cs` の `.ars`・
`py_type_to_arrow` は**パーサを通らない文字列**を渡してくる（8.4 の棚卸し ② と同じ経路）。
Python の `list` / `dict` / `set` には要素型が無いので、翻訳時には「要素型を知らない容器」を
素直に表す型が要る。⇒ **純 Arrow の経路からは隔離済み**。翻訳機能を実装するときにそこへ繋ぐ。

##### 移行: 55 箇所 / 28 ファイル

`list[Any]` が**多くの場合そのまま代わりになった**。起票時に「代替にならない」と書いたが、
測り直すと**限定的**だった:

| 操作 | 素の `list` | `list[Any]` |
|---|---|---|
| `append` / `for` / 添字 / 格納 | 通る | **通る** |
| 取り出して**演算**（`==` / `+` / メンバーアクセス） | 通る（黙って無検査） | ⛔ 静的エラー |

⇒ `Unresolved` は**どこでも黙って寛容**、`Any` は**保管と反復には寛容、演算には厳格**。
差が出るのは「取り出して演算する」瞬間だけだった。

**明示ダウンキャスト（`mustbe` / `is` 絞り込み）が受け皿になった**のも大きい。
`store_copy_semantics.ar` の `p.f[0] === p` は `(p.f[0] mustbe N) === p` で残せたし、
`except_bind_and_match_expr.ar` は `is Dog` の絞り込みがそのまま働いた。

##### 実演例題: **3 件は残せた・3 件は失った**

| 例題 | 何を実演していたか | 結果 |
|---|---|---|
| `classes/attr_access_paths.ar` | 実行時 `AttributeError` の捕捉 | ✅ **受け手を `complex` に変えて残した**（メンバー集合を静的に持たない組み込み型） |
| `exceptions/try_except.ar` | 実行時 `TypeError` の捕捉 | ✅ **`dict` の `NaN` キーへ移して残した**（値によってしか決まらない失敗） |
| `collections/store_copy_semantics.ar` | 格納時の複製 | ✅ **`list[Any]` ＋ `mustbe` で残した** |
| `typing/arith_operand_check.ar` ・ `cross_type_equality.ar` ・ `basics/equality_numeric_promotion.ar` | 異型比較の**実行時の答え** | ⛔ **失った** ⇒ 単体テストへ移した（下記） |
| `typing/literal_element_check{,_error}.ar` | 判定不能な要素の個別照合（7.7） | ⛔ **失った**（`_error` は削除・本体は記録として残した） |
| `classes/field_type_runtime_error.ar` ・ `field_class_type_error.ar` | フィールド型の**実行時**検査 | ⛔ **失った**（静的側が先に捕まえる形へ書き換え） |
| `typing/upcast_only{,_error}.ar` | 容器の一方向アップキャスト | ⚠ **綴りを変えて残した**（`list[int]` ↔ `list[Any]`） |

##### 失った分の担保（案 A の「コードとテストで担保」）

`src/interpreter/ops/equality.rs` に **`mod eq_expr_rules`（4 件）** を新設し、
`values_eq_expr` を**実際の経路のまま**呼んで実行時規則を固定した:
異型は `False` ／ 数値族は昇格 ／ `bool` は対象外 ／ コンテナの中へは再帰しない。

⚠ フィールド型の実行時検査は述語（`TypeTag::matches`）が
`src/vm/op.rs` の `mod type_tag_tests` で既に固定済み（タスク 6.1）。
⇒ 新たに失ったのは「**配線が効いていること**」の実演だけで、述語の網は残っている。

##### D-3 の容器部分は**到達不能**になった

`(ListOf(_), List) => true` 等 7 本は実装に残るが、右辺（期待型）の素の容器型は
**注釈からしか生まれない**ので、Arrow のソースからは発火しない。
⇒ `upcast_only.ar` は同じ方向を `list[int]` → `list[Any]` で示す形へ書き換えた。
⚠ D-3 のもう一方（具体クラス → trait は可・trait → 具体クラスは不可）は**無関係に残る**。

##### ゲート

`cargo test` **782 passed**（新規 4 件）／`scan_examples` 既知の TIMEOUT のみ／
`force_gate` 0 件（320）／`compare_bytecode` 負の対照 270/270・9.7 基準比は
**書き換えた `bare_tuple_annotation_error.ar` 1 件のみ**／`compare_outputs` 250/251（同上）／
`compare_import_paths` 13/13／`compare_python_impl` 94/94 clean（knownDiff 3 件追加・1 件削除）／
`compare_wasm_frontend` **321/321**／`type_obligations` 98% 退行なし／`stale_doc_refs` OK。

⚠ `compare_bytecode` で `module_async_body.ar` が 1 度だけ差分に出たが、**再実行で消えた**
（async の非決定的な順序）。⇒ 実差分ではない。

⚠ `examples/archived/` の `compile_types.ar` / `heavy_ops.ar` / `point_ops.ar` は
**UTF-8 BOM でパースできない既存の破損**で 8.1 とは無関係（タスク 9.3 の対象）。
`archived/generator.ar` だけ 8.1 起因だったので直した。

---

#### ⚠⚠ 8.1 の起票時の検討（判断が出る前の記録）

##### 実測し直した移行コスト: **55 箇所 / 28 ファイル**（GATED 39 / ungated 16）

起票時の 46 箇所から増えたのは、**フェーズ 7 で私が追加した例題が素の `list` を使っている**ため。

##### ⚠⚠ `list[Any]` は代替にならない（実測）

| 書き方 | 要素の型 | `hide([1])[0] == "s"` |
|---|---|---|
| 素の `list` | **`Unresolved`** | ✅ 通る（実行時に `False`） |
| `list[Any]` | **`Any`** | ⛔ **静的エラー**（`Any` は明示ダウンキャストを要求する） |

⇒ 機械的な `list` → `list[Any]` 置換は**できない**。

##### 何が困るのか

素の `list` は「**静的に型が決まらないが、エラーでもない**」唯一の手軽な経路で、
**実行時エラーを実演する例題**がこれに依存している:

| 例題 | 何を実演しているか |
|---|---|
| `exceptions/try_except.ar` | 実行時 `TypeError` の捕捉（タスク 4.4 でこの形へ移行した） |
| `classes/attr_access_paths.ar` | 実行時 `AttributeError` の捕捉（タスク 7.5 で移行） |
| `basics/equality_numeric_promotion.ar` | 異型比較の**実行時の答え**（タスク 7.6 で移行） |
| `typing/arith_operand_check.ar` ・ `typing/cross_type_equality.ar` ・ `typing/literal_element_check.ar` | 同上 |

⚠⚠ **静的検査が完全に近づくほど、「実行時エラーを実演する例題」は書けなくなる。**
これは想定どおりの帰結でもあるが、**実行時検査（FFI・import 経由の `Unresolved` に対する
最後の砦）が例題で確認できなくなる**ことを意味する。

##### 選択肢

| 案 | 内容 | 代償 |
|---|---|---|
| **A** | 8.1 を実施し、実演例題は**諦める**（実行時検査はコードとテストで担保） | 実行時検査の回帰を例題で検出できなくなる |
| **B** | 8.1 を実施し、**明示的な「不透明」機構**を入れる（例: `opaque[T]` 注釈やテスト専用の組み込み） | 言語に「型検査を避ける」手段を足すことになる |
| **C** | 8.1 を**見送る**（素の容器型を許し続ける） | 要素型を捨てる経路が残る（`append` / `for` / 添字の検査が消える・実測 11 地点） |

⚠ 利用者の判断が要る。判断が出るまで 8.1 には着手しない。

### 順序

~~`8.2` → `8.3`~~ → **`8.0`** ✅ → **`8.2` + `8.3`** ✅ → **`8.4`** ✅ → **`8.1`** ✅ → **`8.5`** ✅ ⇒ **フェーズ 8 完了**

⚠⚠ **起票時の分割（8.1 / 8.2 / 8.3）は実測で成り立たなかった。**
`tuple` を許すなら `list` も許すことになり、`list[tuple]`（8.2 の唯一の実例）の扱いも割れる。
⇒ **1 つの規則**「注釈を書くなら要素型まで書け」に統合した。
代わりに、移行コスト 0 で独立していた `generator`（8.0）だけを先に片付けた。
...
##### 棚卸しの結果（2026-09-17・完了）

`InferredType` の**素の容器型を作っている箇所**を全数で洗った（`List` / `Dict` / `Set` /
`FixedList` / `ListLike` / `TupleAny` の全出現 39 箇所を「作る」と「読む」に仕分け）。
**生成元は 3 つだけ**だった:

| # | 生成元 | Arrow ソースからの到達性 | 決定 |
|---|---|---|---|
| ① | `from_ann` のプリミティブ表（`"list"` / `"dict"` / `"set"` / `"fixed_list"` / `"list_like"` / `"tuple"`） | **到達する**（利用者が素の容器型を書く） | **8.1 の判断対象**。ここだけが残件 |
| ② | `from_ann` の「内側が解決できないとき」のフォールバック（5 箇所） | **到達しない**（実測） | **形を残す**（スタブの入口・下記） |
| ③ | `call_check.rs` の空の可変長引数 | **到達しない**（実測） | `ListOf(Never)` へ寄せた |

⚠ `scope.rs` / `type_utils.rs` / `ffi_boundary.rs` / `infer.rs` の出現は**読むだけ**（照合）。

##### ② が到達しないことの実測

`parse_type_expr`（`src/parser/types.rs`）が**先に弾いている**。10 形で確認:

```
list[a.b] / set[a.b]            → ParseError: expected `]`, got `.`
dict[str] / list[Result[int]]   → ParseError: expected type name, got `]`
dict[str, dict[int]]            → ParseError: expected type name, got `]`
Union[int] / Intersection[A]    → ParseError: requires at least 2 type arguments
list[foo] / list[Box[int]] / list[type[foo]]  → 8.2 以降は NamedInstance になるので落ちてこない
```

⚠⚠ **それでも「死枝だから消す」としなかった。** `from_ann` の入力は Arrow のソースだけでは
ない。`.pyi` スタブ・`--compile-cs` の `.ars`・`py_type_to_arrow` は**パーサを通らない
文字列**を渡してくる。スタブによる型検査（タスク 7 の ⑥A で利用者が要望した仕組み）が
入る余地を残すため、**形を保ったまま `⚠⚠⚠` 注記つきで残した**
（「リファクタリングで潰されないようにメモを残す」という指示に対応する実体）。

⚠ そのとき「素の容器へ落とす＝万能受容体にする」ことの是非は改めて決めること。
8.2 で潰したのと**同じ性質の穴**になる。⇒ 8.5 と一緒に判断する。

##### ③ の決定: `ListOf(Never)`

空の可変長引数は `parser/exprs.rs` が
`variadic argument `... = ...` requires at least one expression` で弾くので到達しない。
それでも素の `List` を置き換えたのは、**素の `List` は下流の可変長検査に当たらない**
（`Some((_, IT::ListOf(elem_ty)))` で受けている）ため、パーサ側が緩んだときに
**「検査が静かに消える」形で露見する**から。空リテラルの要素型を `Never`（⊥）にした
タスク 4.1 と同じ判断に揃えた。

##### 起票時の記述で誤っていたもの

- ⛔ 「`types.rs` の `from_ann` → 8.2 で潰す」 → 8.2 が潰したのは**未知の名前という原因**で、
  フォールバック自体は残っている（上記 ②）。到達しなくなっただけ。

##### ゲート

`compare_bytecode` **269/269 完全一致**・`compare_outputs` **250/250 完全一致**
（＝到達しない枝であることの裏取りでもある）、`force_gate` 0 件、
`compare_python_impl` 96/96 clean、`compare_import_paths` 13/13、
`compare_wasm_frontend` 320/320、`type_obligations` 98% 退行なし、`cargo test` 778 passed。
...
## §12 フェーズ 10 — テンプレートの特例扱いをやめる（起票 2026-09-19）

⚠ 採番は `.claude/rules/regulations.md` の規則に従い **`10-1` 形式**（フェーズ-タスク）。

### 発端

利用者の報告「テンプレートクラスのメソッド引数が型を捕まえていない」。実測すると
**報告より範囲が広かった**。
...
class Box[T]:
    ...
    fn __init__(mut self, v: T) -> None:
        self.v = v
    fn tag(mut self, n: int) -> None:     # ← 型変数ですらない具体型
        print(n)

mut b = Box[int](1)
b.tag("wrong")        # ⛔ 通って "wrong" を出力する
```

### 根本原因: **型検査だけがテンプレートを具体化していない**

| 層 | テンプレートクラスの扱い |
|---|---|
| **実行時**（`interpreter/templates.rs`） | `subst_stmts(&tmpl.body, &type_map)` で **`T` を `int` に置換した具体的な `ClassDef` を作り**、`build_template_class` で普通のクラスにする。`(テンプレート, 型引数)` でメモ化 |
| **型検査** | 具体化せず `GenericInstance { name, args }` を持ち回り、**使用地点ごとに** `class_and_subst` で置換表を作り直す |

⇒ 型検査側は「テンプレートを特例として扱い続ける」設計で、`NamedInstance` を扱う
**すべての地点が `GenericInstance` も覚えていなければならない**。
実測: `src/type_check/` の `GenericInstance` 出現は **29 箇所 / 7 ファイル**。

**今回のバグは、そのうち 1 箇所が忘れたもの**:

```rust
// call_check.rs — メソッド呼び出しのレシーバからクラス名を取り出す
InferredType::NamedInstance(cls) => Some(cls.clone()),
InferredType::TypeValOf(inner) => { … }
_ => None,          // ← GenericInstance{Box,[int]} がここへ落ちる
```

`None` になると `method_call_info` が `None` になり、**メソッド呼び出しの検査ブロックが
丸ごと skip される**。
...
| 検査 | 非テンプレート | テンプレート |
|---|---|---|
| コンストラクタ引数 | ✅ | ✅ **別経路**（`check_template_call_args`）を通るため |
| メソッド引数（型変数 `T`） | — | ⛔ |
| メソッド引数（**具体型 `int`**） | ✅ | ⛔ |
| メソッド戻り値型 | ✅ | ⛔（`let s: str = b.get()` が通る） |
| メンバー存在検査（7.5） | ✅ | ⛔ |
| アクセス制御・`static` メソッド誤用 | ✅ | ⛔ |

⇒ コンストラクタだけ通るのは**そこが唯一の別経路**だから。

### テンプレート**関数**は無事（実測）

```
fn ident[T](let x: T) -> T   ident[int]("wrong")   → ⛔ 静的エラー ✅
fn tagged[T](x: T, n: int)   tagged[int](1,"x")    → ⛔ 静的エラー ✅
let s: str = ident[int](1)                          → ⛔ 静的エラー ✅
```

関数は `check_template_call_args` が**型引数で置換してから**照合しているため。
⇒ **クラスのメソッドだけが取り残されている。**

### ⚠⚠ ただし `gen` に**別の退行**を見つけた（タスク 8.5 で私が入れたもの）
...
```
```
StaticTypeError  parameter `xs` of gen `take` is annotated 'list[T]' but 'T' is not a known type
```

`Stmt::GenDef` は `template_params` を**持っている**のに、`check_stmt` の分岐が `..` で
捨てており、`check_gen_def` は受け取っていない ⇒ 型変数がスコープに入らない。
タスク 8.5 で「`gen` には型変数の投入が無いので、ここで検査してよい」と書いたのは
**誤った推論**だった（「投入コードが無い」＝「投入が不要」ではなく、**投入が漏れていた**）。

⚠⚠ **例題に `gen f[T]` が 1 本も無いため、全ゲートが緑のまま通った。**
`syntax_cov.ps1` が拾うべき「例題が一度も書いていない構文」そのもの。

### 方針: **具体化して普通のクラスとして検査する**

Arrow には**暗黙の実体化が無い**（型引数なしの呼び出しは `TemplateMissingTypeArgs`）。
⇒ **必要な実体化の集合はソース上に静的に列挙できる**。チェック時モノモーフィゼーションが
成立する条件がそろっている。
...
| # | 内容 | 依存 | 規模 |
|---|---|---|---|
| **10-1** | テンプレート `gen` の型変数がスコープに入らない（**8.5 の退行**） | なし | 小 |
| **10-2** | 置換ロジック（`subst_stmts` 群）を型検査から使えるようにする | なし | 小 |
...
#### 10-1 テンプレート `gen` の型変数がスコープに入らない

`check_stmt` の `Stmt::GenDef` 分岐が `template_params` を `..` で捨てている。
`check_fn_def` は受け取って `push_type_params` しているので**非対称**。

⇒ `check_gen_def` に `template_params` を渡し、`check_fn_def` と**同じ位置**
（注釈検査の前）で積む。8.5 のコメント（「`gen` には型変数の投入が無い」）も直す。

⚠ 例題が 1 本も無いので、**`gen f[T]` の例題を足すこと**（10-5 の一部を先取りする）。
足さないと同じ壊し方が何度でもできる。
...
⇒ `pub(crate)` にして型検査からも呼べるようにする。
⚠⚠ **実行時と型検査で同じ置換を使うことが要点。** 別実装を書くと
「実行時は通るのに型検査が落ちる（逆も）」が構造的に発生する
（この campaign で何度も踏んだ「同じ問いに 2 つの実装」）。

⚠ 置き場所は要検討。`interpreter/` に型検査が依存するのが嫌なら、
`src/template_subst.rs` のような中立な場所へ移して両方から使う。
...
2. `Name` が既知テンプレートなら `subst_stmts` で具体化した `ClassDef` を作り、
   **`"Box[int]"` という名前で普通のクラスとして**レジストリへ登録する
3. `Box[Box[int]]` があるので**不動点まで回す**（visited と深さ上限を持つこと）

そのうえで `from_ann("Box[int]")` が `NamedInstance("Box[int]")` を返すようにする。

⚠ 実行時のメモ化キーは `(テンプレート, 型引数)`。**型検査側も同じ粒度**にすること
（同じ型引数なら同じ型、という実行時の性質と揃える）。
⚠ 制約（`class Box[T: Printable]`）の検証は実行時が `check_template_constraints` で
行っている。実体化パスでも同じ判断をするか、静的には見送るかを決めること。

#### 10-4 `GenericInstance` の特例を撤去する

10-3 が入れば `GenericInstance` は使用地点に現れなくなるので、
**29 箇所（7 ファイル）の特例を消す**:

```
type_utils.rs 6 / infer.rs 6 / stmt/check.rs 5 / types.rs 4 /
call_check.rs 4 / binop.rs 3 / registry/builder.rs 1
```

⚠ `class_and_subst` / `subst_type_params` / `template_call_result_type` /
`check_template_call_args` が役目を終えるか、役割が変わる。
⚠⚠ **一度に全部消さないこと。** 10-3 の後に `GenericInstance` が本当に到達しなくなったかを
`debug_assert!` か計測で確かめてから外す（8.4 の「到達しない枝」の棚卸しと同じ手順）。
...
| 足すもの | 理由 |
|---|---|
| `gen f[T]` の例題（正常系＋`_error`） | **1 本も無い**（10-1 の再発防止） |
| テンプレートクラスの**メソッド呼び出し**の例題（正常系＋`_error`） | 引数・戻り値・メンバー存在の 3 経路 |
| `type_obligations` の検体 | テンプレートのメソッド引数・戻り値を義務として登録する |
