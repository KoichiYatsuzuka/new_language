...
| **8.5** | **未知の型名の妥当性検査が無い**（8.2 で判明） | 未計測 | 新規起票（下記） |
...
⚠⚠ **影響範囲の見積もりが要る。** テンプレート型変数（`T`）・外部言語由来の型・
`.pyi` スタブ（**9.4**）・`import[cs-dll]` の `.ars` が**すべて注釈に名前を出す**ので、
素朴に「レジストリに無ければエラー」にすると相互運用が壊れる可能性が高い。
⇒ **先に例題全体で「レジストリに無い注釈名」を数えること。**
...
~~`8.2` → `8.3`~~ → **`8.0`** ✅ → **`8.2` + `8.3`** ✅ → **`8.4`** ✅ → **`8.1`** ✅ → `8.5`
