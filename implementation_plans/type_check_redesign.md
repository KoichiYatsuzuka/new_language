...
| 検体 | 内容 | 根拠 |
|---|---|---|
| `O4` `O5` `O8` | `==` / `!=` / `in` の異型比較 | `equality_numeric_promotion.ar`（B2-b）が「異型は `False`」を仕様として固定。**順序比較と違い意味のある答えがある**（タスク 4.4 で厳密化を撤回した） |
| `O7` | `and` の被演算子型が違う | D-10。`and` / `or` は join を返すので正当な式。誤りは**条件位置**で、D-12（5.5）が弾く |
| `L11` | 注釈なしの空コレクション | U-6。`[]` は `list[Any]` として legal |

#### 実行時が受け持つのが正しい（10 件）

| 検体 | 内容 | 理由 |
|---|---|---|
| `T1` `T2` | `cast` / `mustbe` の失敗 | **動的変換・動的表明**そのもの。静的に決まるなら書く必要が無い |
| `M5` `Z7` | 不変インスタンス・`freeze` 後の代入 | 値の**状態**であって型ではない |
| `C13` `E2` `K3` `M1` `M2` `O6` `Z5` | 非関数の呼び出し・非例外の `except`・非反復可能の `for`・存在しないメンバー・`-"s"`・`range` に非 int | `Unresolved` 経路が残る限り実行時が最後の砦。⚠ `M1` / `M2` は**静的化の risk を §7 に記録済み**（動的に生えるメンバーで偽陽性が出る） |

#### ⚠ 本当に残っている穴（3 件）

| 検体 | 内容 | 状況 |
|---|---|---|
| `Z10` | 組み込み `len()` の戻り値型 | **未着手**。`len` は利用者が再定義できる（名前の占有）ため、静的に型を付けると shadow 時に偽陽性が出る。タスク 2.4（U-1）で変換関数だけ型付けし、`len` は見送った |
| `L17` | `[1, zs[0]]` の `zs[0]` が `Unresolved` で**リテラル全体**の要素型が崩れる | `join_elem_types` の「判らない側が混ざったら全体が判らない」規則の帰結。**判っている要素（`1`）だけでも照合する**余地がある |
| `M1` / `M2` | （上と重複）メンバー存在検査 | risk を承知で入れるかの判断待ち |
