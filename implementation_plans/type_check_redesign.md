...
##### ⚠⚠ A（外部言語）— スタブ機構のための座席を潰さないこと

**スタブ機構は既に在る。** `import[js-proc]` / `import[cs-dll]` / `import[cs-proc]` は
`.ars` ファイルを**通常の Arrow AST としてパース**してモジュール本体にする
（`imports/cs_js_modules.rs::load_js_module` ほか）。⇒ スタブがあるモジュールの型は
**特別扱い不要**で、通常の宣言として型検査に載る。

問題は**スタブの無い言語**（現状 `import[py]`）。`mo.SharedState(2000)` の戻り値が
`NamedInstance("SharedState")` と推論されているが、`SharedState` は `matrix_ops.py` の
**Python クラス**で Arrow のクラスではない。名前だけ知っていてメンバーを知らない状態を
「Arrow のクラス」として表しているのが**嘘**で、偽陽性 5 件の原因。
