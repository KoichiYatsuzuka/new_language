...
### 棚卸し — 型義務 94 件の網の目検査 【🔍 調査のみ 2026-09-12】

> 型検査に関するバグがあまりにも多いので、静的型検査の方法を根本的に見直す必要があります。
> まず、型検査が必要となるパターン（値束縛、比較演算子、など）を列挙し、
> それらですべて型検査が走っているかを調べてください

#### 方法 — 列挙を勘でやらない

義務の一覧を記憶で作ると漏れる（それが今までの穴の作り方そのもの）。**AST のノード集合から
機械的に導出**した: `src/ast.rs` の `Stmt` 40 バリアント・`Expr` 30 バリアントを全部出し、
そのうち「2 つの型が適合しなければならない地点」を抜いて **94 件の型義務**にした。

各義務につき **意図的な型エラーを 1 つだけ**含む検体を書き、実行して分類した:

| 判定 | 意味 |
|---|---|
| `STATIC` | `StaticTypeError` が出た（= 静的検査がある。あるべき姿） |
| `RUNTIME` | 実行が始まってから `TypeError` 等で落ちた（= **その経路を踏まなければ見逃す**） |
| `NONE` | 完走した（= **無検査**。黙って不整合な型が通った） |

⚠ 検体は `scripts/type_obligations/*.ar` に置いた（`examples/` に置くと意図的に落ちるので
scan_examples / force_gate / compare_* が「壊れた例題」として拾ってしまう）。
`scripts/type_obligations.ps1` が実行して `# @baseline:` と比べ、**弱くなったら退行**で exit 1。
...
**静的に検査されているのは 41% しかない。** 残り 59% は「実行経路を踏まないと判らない」か
「そもそも誰も検査していない」。

#### 根本原因 — 3 つある（どれも個別修正では閉じない）

##### ① `Unresolved` が万能受容体になっている

`type_matches_exact`（`type_check/type_utils.rs`）の冒頭:
...
**推論が型を出せなかった地点は、以後あらゆる検査を黙って通過する。** `NONE` の大半が
これに帰着する（`enum .value` / `except as e` / 混在コレクション / `xs[0]` / オーバーロード）。
⚠ 「判らないものは通す」は**保守的に見えて逆**。検査の穴は "判らない" が伝播した先すべてに
開くので、1 箇所の推論漏れが**下流の全義務を無効化**する。

##### ② 推論が「揃わなければ要素型を捨てる」— そして捨てた形は何にでも適合する

`Expr::List`（`type_check/infer.rs:20-32`）:

```rust
if *first != InferredType::Unresolved && types.iter().all(|t| t == first) {
    InferredType::ListOf(Box::new(first.clone()))
} else {
    InferredType::List          // ← 要素型を捨てる
}
```

素の `List` は `is_list_like` 規則で `ListOf(任意)` と適合するので、**捨てることが
「何でも通す」になる**。実測:

| 式 | 推論 | `list[int]` | `list[str]` | `list[float]` |
|---|---|---|---|---|
| `[1, 2]` | `list[int]` | ✅ | ❌ 弾く | ✅（昇格） |
| `[1, "s"]` | **`list`** | ⛔ **通る** | ⛔ **通る** | ⛔ **通る** |
| `[1, zs[0]]`（1 つが `Unresolved`） | **`list`** | ⛔ 通る | ⛔ 通る | ⛔ 通る |

`Set` / `Dict` も同じ崩れ方をする（`Tuple` は位置ごとに持つので崩れない）。
⚠ ユーザーの方針（「注釈が無い束縛にも最大限の厳格さ」）に照らすと、`[1, "s"]` は
`list[Union[int,str]]` と推論されるべきで、素の `list` に落としてはいけない。
`[1, 2.0]` は昇格して `list[float]` になるべき。

##### ③ 検査が「AST の地点ごとの手書き」で、数え上げる仕組みが無い

検査の実体は各所に散った `if !type_matches(..) { push_error(..) }`。問題は `infer()` が
**2 つの役割を兼ねている**こと:

1. 「この式の型を返す」
2. 「部分木を歩いて診断を出す」

そのため **`self.infer(e);` と結果を捨てる書き方が正当な場合もあり**（2 の用途）、
**義務を忘れた場合と区別が付かない**。実測で **19 箇所**が結果を捨てている。
`InferredType` は `#[must_use]` でもないので、**コンパイラは何も言わない**。
0-1〜0-12 / A-2〜A-4 で塞いだ 16 件の穴はすべてこの形だった
（「`infer()` を呼んで結果を捨てている」か「`infer()` を呼んでいない」）。

⇒ **必要なのは個別の検査追加ではなく、「型義務」を第一級の概念にすること。**
「この式はこの型でなければならない」を 1 箇所に登録し、登録漏れが構造的に判る形にする
（`Directive::CheckBefore` の枠組みが既にあるので、そこへ寄せるのが筋）。

#### `NONE`（無検査）30 件の内訳

| 分類 | ID | 義務 |
|---|---|---|
| **コレクションの要素型**（原因 ②） | L1, L3, L9, L15, L16, L17, L18, L19 | 混在リテラル・入れ子・`Unresolved` 混入・`dict`/`set` |
| **コレクションへの書き込み** | L5, L14 | `a[i] = v` / `xs.append(v)` |
| **比較・論理演算子** | O4, O5, O7, O8 | `==` / `!=` / `and` / `in`。⚠ `<` は `STATIC`（**非対称**） |
| **条件式** | K1, K2 | `if` / `while` の条件が非 `bool`。Python 流の真偽性なら仕様 → **要判断** |
| **`enum`** | N2, N3, N4 | 未呼出関数内の不正値・`.value` と メンバーの静的型 |
| **`match`** | X5, X6, X7 | `case` パターン型 vs subject・`is` の型名未検証・`match` 式で絞り込まない |
| **呼び出し** | C8, C12, C14, C15 | `yield` 型・オーバーロード解決・`function[]` シグネチャ・可変長要素型 |
| **`static mut`** | F4 | クラス名経由の代入（§7 #14） |
| **その他** | E3, L11, T3 | `except as` の束縛型・空コレクションの注釈必須（**文書にあるのに未実装**）・`new_type` の基底型 |

#### `RUNTIME`（実行時のみ）25 件で特に重いもの

| ID | 義務 | なぜ重いか |
|---|---|---|
| O1, O2, O6, Z1 | **算術演算子**（`int + str` 等） | 最も頻出の型エラーが静的に出ない |
| B6, F2 | **複合代入** `x += v` / `o.f += v` | §7 #2 |
| X1〜X4 | **block / if / for / match 式の結果型** | `->T` 注釈があるのに実行時まで判らない |
| M1, M2 | **存在しないメンバーの読み・呼び出し** | 「フィールドは全て宣言必須」なのに静的に検出しない |
| N1 | `enum` バリアント値 | 定義が実行されなければ見逃す（N2 が `NONE` な理由） |
| E2 | `except` が非例外クラス | 捕まらない `except` が黙って残る |

#### 所見 — どこから直すべきか

個別の 30 件を順に塞ぐのは**これまでと同じやり方**で、同じ密度で穴が再生する。
効く順に:

1. **原因 ② を直す**（コレクション推論を「捨てる」から「合成する」へ）。`NONE` の 8 件が
   一度に閉じ、かつ `Unresolved` の発生源が減るので ① の影響範囲も縮む。**破壊的でない**
   （今より厳しくなるだけ）が、既存例題が引っかかる可能性があるので実測が必要。
2. **原因 ① に向き合う**。`Unresolved` を「通す」から「**その地点を記録して後で報告する**」へ
   変える。いきなり「`Unresolved` を弾く」にすると正しいコードが大量に落ちるので、
   まず `annot_unresolved.ps1`（発生源の内訳を出す既存スクリプト）で**母数を測る**。
3. **原因 ③ の枠組み**（型義務の一元化）。①② を直した後でないと、登録すべき義務の
   正しい型が判らない。
4. 演算子（O1/O2/O4/O5）は ①② と独立に閉じられるので、並行して進められる。

⚠ **K1/K2（`if` の条件が非 `bool`）と O7（`and` が被演算子を返す）は仕様判断**。
