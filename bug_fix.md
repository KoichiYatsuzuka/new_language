...
| **B12** | ⬜ 未調査。**再帰ジェネレータが黙って空の列を返す**（🔴 サイレント）。⚠ `46b8d15` 以前から同じで、B4 / B10 の作業とは無関係 |
...
| **B12** | 再帰ジェネレータが**空の列を返す** | 🔴 サイレント | ⬜ 未調査 |
...
- 発見: 2026-09-04（B10 段階 1 の例題を書いている途中）
- ⚠ **`46b8d15` 以前から同じ**（3 つのバイナリで実測）。B4 / B10 の作業とは無関係。
...
⚠ **非再帰のジェネレータは正常**（`gen up(n)` で `[0, 1, 2, 3]` を確認済み）。
壊れているのは「ジェネレータの中で自分自身を `for` で回す」形だけ。

### 影響

🔴 **サイレント**。例外も警告も出ず、空の列が返る。木構造の走査など
再帰ジェネレータが自然な書き方になる場面で、黙って何も出ない。

### 原因

⬜ 未調査。⚠ 起票時の見当は当てにしないこと（B3 / B8 で外している）。
最小形での切り分けから始める。
