...
| **B10** | 🟠 **題を読み替えた**（循環の複製 → スタック枯渇でプロセスを落とさない）。✅ 段階 1（再帰の深さ上限 → catchable な `RecursionError`）完了。⬜ 段階 2（スタック拡張）/ 段階 3（`deep_copy_value` の上限） |
...
| **B10** | スタック枯渇で**プロセスごと abort** | 🟠 クラッシュ | ✅ 特定済み（段階 1 完了）|
...
## B10. スタック枯渇でプロセスが落ちる 🟠（段階 1 完了・段階 2/3 未着手）
...
| 経路 | 落ちる深さ | 状態 |
|---|---|---|
| 関数・メソッド・相互再帰・**ジェネレータ**の再帰 | **~134** | ✅ 段階 1 で `RecursionError` |
| `deep_copy_value`（`let` / `mut` / 関数の `let` 引数 / `append` / `dict[k] =`）| ~1300 | ⬜ 未対応 |
...
### ⬜ 段階 2: インタプリタのスタックを広げる

現状は **main スレッドの既定スタック**で走っている（`main.rs` に `stack_size` 指定なし）。
`std::thread::Builder::new().stack_size(N)` で走らせれば閾値が一桁上がり、
`MAX_CALL_DEPTH` を CPython 並み（1000）に上げられる。

⚠⚠ **段階 1 の定数だけを上げてはいけない。** 上限に当たる前にスタックが満ちて、
防ごうとした abort に戻る。**スタックを広げるのが先、定数を上げるのは後。**

### ⬜ 段階 3: `deep_copy_value` の深さ上限

⚠ コストは **17 ファイル 30 箇所**。`deep_copy_value(val: Value) -> Value` が `Result` を
返さないので、上限を入れるならシグネチャを変えて全呼び出しに波及する。
**段階 2 のあとにもう一度測ってから**判断する（スタックが広がれば実質到達不能かもしれない）。

⚠ やる根拠は既存の上限との不整合: dict キー 64 / `values_eq` 200 / hash 64 に対し
**複製だけ無制限**。深さ 1300 の値は既に比較もキーにもできないのに、
**変数へ束縛しただけでプロセスが落ちる**。

⚠ 打ち切りは不可（値が壊れる）ので、エラーか memo のどちらか。
⚠ B9 と同じ関数。
...
残りは **B10 の段階 2 / 3** と **B12**。
⚠ B10 は段階 2（スタック拡張）が先。段階 3（`deep_copy_value` の上限・17 ファイル 30 箇所）は
**段階 2 のあとに再測してから**判断する。
