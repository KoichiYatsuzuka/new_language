...
| **B4** | ✅ 修正済み。結果として**言語仕様の決定**になった（ループ変数はブロック内の束縛）ので、症状の記述は削除し、節に**決まった規則**を残してある。⚠⚠ **素の CPython イディオムを壊す**ので変換器利用時は節を読むこと |
...
### ⚠⚠ Python 互換への影響（変換器の落とし穴）

規則 2 は**素の CPython イディオムを壊す**:

```python
def loop_var_only(xs):
    for i in xs:
        pass
    return i        # CPython は最後の要素。Arrow は NameError
```

`for` の後でループ変数を読む Python は普通にあるので、**Python→Arrow 変換後にここで落ちる**。
ループ内で別変数に退避させること。実演は
[py_reassign.ar](examples/interop/py_reassign.ar) ⑨⑩。
