...
| **B13** | ⬜ 未着手。**ジェネレータが先行評価される**（遅延しない）ので**無限ジェネレータがハング**する。⚠ 設計の問題で規模が大きい——**やるかどうかから決める** |
...
| **B13** | ジェネレータが先行評価され、**無限ジェネレータがハング** | 🟠 ハング | ✅ 特定済み（設計）|
...
## B13. ジェネレータが先行評価される（遅延しない）🟠

- 発見: 2026-09-04（B12 の原因を読んでいる途中）
- ⚠ **`46b8d15` 以前から同じ**（3 つのバイナリで実測）。B12 の修正とは無関係の**設計**。

### 実測

`run_vm_generator` は**本体を最後まで走らせて全 yield を `Vec` に集めてから**
`Value::Generator` を作る（`GeneratorState { values, index }`）。よって:
...
```
  produce 0 / 1 / 2 / 3 / 4      ← 消費が始まる前に全部走る
consume 0
consume 1                        ← break しても produce 2..4 は既に起きている
```

### 影響

- 🟠 **無限ジェネレータが書けない。** `while True: yield i` は**永久にハング**する
  （実測。出力は 1 つも出ない）。消費側の `break` は届かない。
- ⚠ 副作用の順序が Python と違う。生成と消費が交互にならない。
- ⚠ メモリは yield の総数に比例する。
- ⚠ Python のジェネレータは遅延なので、**Python→Arrow 変換で意味が変わる**類型。

### 修正方針（要判断）

⚠ 素直に直すなら「本体を中断・再開できる実行」が要る（コルーチン化）。
`vm::run` を再入可能にするか、生成側を別スタック／別スレッドで走らせるかの設計判断が先。
**規模が大きいので、やるかどうかから決めること。**

⚠ 当面の緩和として「無限ジェネレータは書けない」ことを**エラーで知らせる**手は無い
（停止性の判定になる）。⇒ 少なくとも**文書に明記**するのが先。
...
残りは **B13**（ジェネレータの先行評価）のみ。⚠ これはバグというより**設計の選択**で、直すならコルーチン化が要る。**やるかどうかから決めること。**
