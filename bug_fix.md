...
| **B13** | ✅ 修正済み。コルーチン化完了（中断・再開・`close()`・クロージャ化）。結果として**言語仕様の決定**になったので節に規則を残してある。⚠ **速度は未検証**（計測環境が壊れている）|
...
段階 B 以降、**信用できる A/B が取れていない**:
- Windows Defender が**新規ビルドの exe の複製をブロック**する（scratchpad でも `target/` でも）
- 負の対照（同一 exe 同士）でさえ **±5.4%** 振れた

⇒ **静かな環境で取り直すこと**（`vm-pitfalls` §1）。⚠ それ以前の測定
（B10 段階 2 の「起動 +0.5ms」など）も、毎回 B 側だけが新規コピーだったため
**同じ系統誤差の疑いがある**。
...
**全件完了**。⚠ ただし **B13 の速度は未検証**で、静かな環境での A/B が残っている。
