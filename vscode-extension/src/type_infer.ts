...
// ===== Built-in type method table =====

type BuiltinMethodInfo = { ret: LangType; sig: string };

const BUILTIN_TYPE_METHODS: Readonly<Record<string, Readonly<Record<string, BuiltinMethodInfo>>>> = {
    str: {
...
interface PyModuleInfo {
    ...
    sigs: Map<string, string>;    // name → full signature string
...
const _moduleInfoCache = new Map<string, { version: number; data: DocModuleInfo }>();
const _hoverSymbolsCache = new Map<string, { version: number; data: HoverSymbol[] }>();
...
function collectAllPyModuleInfo(document: vscode.TextDocument): DocModuleInfo {
    ...
    const data: DocModuleInfo = { funcTypes, funcSigs, classMethods, cppClasses };
