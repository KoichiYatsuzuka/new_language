...
import { clearStubs, setStub, stubCount } from './frontend';
...
let loaded: { manifestPath: string; mtimeMs: number } | null = null;
 ...
 * 拾えるようにするため。
 */
function findManifest(documentPath: string): string | null {
 ...
 */
export function loadStubsFor(documentPath: string): number | null {
    ...
    if (loaded && loaded.manifestPath === manifestPath && loaded.mtimeMs === mtimeMs) {
        return stubCount(); // 同じものが既に載っている
    }

    let entries: ManifestEntry[];
    try {
        const raw = JSON.parse(fs.readFileSync(manifestPath, 'utf8')) as { stubs?: ManifestEntry[] };
        entries = raw.stubs ?? [];
    } catch {
    ...
    for (const e of entries) {
        try {
            setStub(e.key, fs.readFileSync(path.join(dir, e.file), 'utf8'));
        } catch {
            // 1 件読めなくても残りは積む。
        }
    }
...
export function loadedManifestPath(): string | null {
    return loaded?.manifestPath ?? null;
}
 ...
 */
export function refreshStubs(documentPath: string): Promise<{ ok: boolean; message: string }> {
    const exe = vscode.workspace.getConfiguration('arrow').get<string>('executablePath')?.trim();
    if (!exe) {
    ...
    return new Promise(resolve => {
        const child = spawn(exe, ['--emit-stubs', documentPath], {
