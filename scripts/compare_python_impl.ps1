...
$knownDiff = @{
    ...
    'list_dict_ctor'                 = 'py: list 内の str の表示形式が違う（py は引用符を付けない・E 後の組込み追加で新設）'
    'list_dict_ctor_error'           = 'py: impl_python は dict([1,2]) を黙って空 dict にする（Rust は TypeError。この silent drop を潰すための例題）'
