...
pub enum Op {
    ...
    BuildDict(u16),
    /// 辞書の合成: 末尾の辞書（展開元）を pop し、**その下の辞書へ全エントリを挿入**する。
    ///
    /// `{a: 1, **b, c: 2}` の `**b` 用。**後から来たキーが勝つ**（Python と同じ）。
    /// ⚠ 展開元が辞書でなければ実行時 `TypeError`（黙って捨てない）。
    DictMerge,
    ...
    ///
    /// `0` = list（そのまま）/ `1` = tuple / `2` = set（`set_insert` で重複排除）。
    /// ⚠ 展開が無い列リテラルは従来どおり `BuildList` / `BuildTuple` / `BuildSet` の
    /// 1 命令なので、**既存のバイトコードは変わらない**。
    SeqFinish(u8),
    // ── ジェネレータ（タスク #8） ──
