...
impl Compiler {
    ...
    /// `for t in iter: body`（#27。#62 で `compile_stmt` から切り出し）。
    ///
    /// ⚠⚠ **外側の同名束縛を覆うループ変数には専用 slot を割り当てる**。Arrow の `for` 変数は
    /// ブロックスコープで、ツリーウォークは反復ごとにスコープを push して宣言する
    /// （ループを抜けると外側の値が戻る）。名前ごとに 1 slot の flat モデルでこれを表現するため、
    /// **本体の間だけ** `slots` の対応を temp slot へ差し替え、ループ後に元へ戻す。
    ///
    /// ⚠ temp は LIFO。`alloc_temp` と `free_temp` の順序を崩さないこと。
    fn compile_for(&mut self, targets: &[String], iter: &Expr, body: &[Stmt]) -> Option<()> {
        if targets.is_empty() {
    ...
    /// `let x = e`（関数本体・入れ子ブロック側。#62 で `compile_stmt` から切り出し）。
    ///
    ...
    /// 入れ子 `fn` 定義（#27。#62 で `compile_stmt` から切り出し）。
    ///
    /// ⚠ **本体はここではコンパイルしない**。`Op::MakeFn` が参照する `ChunkFnDef` に積むだけで、
    /// 実際のコンパイルは初回呼び出し時に**定義サイト単位**で行う（#30）。
    fn compile_nested_fn_def(
        &mut self,
        name: &str,
    ...
    /// `let a, b = t`（#27-c。#62 で `compile_stmt` から切り出し）。
    ///
    /// 束縛先は slot（制御フロー内の宣言）とグローバル宣言（最上位の宣言文）の 2 通り。
    /// `collect_nested_decls` が入れ子のターゲットにだけ slot を割り当てるので、その有無で判別できる。
    /// ⚠ 混ぜると `for` の 2 周目で「already declared」になる（`built_in.ar` の `zx`）。
    fn compile_let_tuple(
        &mut self,
        stmt: &Stmt,
        targets: &[crate::ast::TupleTarget],
        value: &Expr,
    ) -> Option<()> {
