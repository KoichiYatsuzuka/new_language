...
pub enum Expr {
    ...
    /// **関数 base スコープ（実行時は `scopes[frame_floor]`）** に確実に解決できる
    /// `Expr::Ident` を書き換えたもの。`slot` は base スコープ内の宣言順インデックス。
    /// これによりローカル読み取りがスコープ遡り＋文字列ハッシュから配列1回に置き換わる。
    /// `name` はフォールバック（境界外時）とデバッグ時の一致検証、および
    ...
    /// （かつその関数内で一度も宣言されない＝シャドウされない）と確定した `Expr::Ident`
    /// を書き換えたもの。`LocalRef` のグローバル版。
    ///
    /// `cache` は `scopes[0]` 内の slot index を `(slot_epoch, index)` で焼く実行時キャッシュ。
    /// **解決結果を AST ノードに置く**ことが要点で、これまで
    /// ツリーウォーク（キャッシュ無し・毎回スコープ名前走査）と VM（`Chunk` 側の別キャッシュ）が
    /// 同じ問いを別々に解いていたのを、同一ノードの共有へ寄せる。
    ///
