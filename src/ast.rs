...
///   ⚠⚠ **降格であって「再束縛の禁止」ではない**（タスク 9.2 で仕様確定）。
///   以降 `x = ..` / `x.append(..)` / `x[0] = ..` / `x.f = ..` がすべて静的エラーになる。
///   静的側は `CheckState::freeze_var` が**宣言スコープの可変フラグを倒す**ことで
///   `let` 束縛用の既存の検査をそのまま効かせている（別の集合を持たない）。
///   ⚠ 降格はスコープを抜けても戻らない。実行時の `make_var_immutable` が変数そのものを
///   書き換えるのに合わせてある。
...
pub enum Stmt {
    ...
    ClassDef {
        ...
        /// 継承する基底クラス・トレイト名のリスト。
        bases: Vec<String>,
        /// 基底トレイトごとの**具体型引数**（`class C(Holder[int])` なら `[["int"]]`）。
        ///
        /// ⚠⚠ **`bases` と同じ並び・同じ長さ**で入る。`bases` は名前だけの射影で、
        /// 継承関係の判定・実行時のフィールド索引はそちらを見る。
        ///
        /// ⚠ ここを捨てると `trait Holder[T]` の `T` を具体型へ置換できず、
        /// クラスが継承したフィールドの型が **`T` のまま**になる（タスク 9.9）。
        /// パーサは以前からこの情報を持っていた（自動 `__init__` の引数型に使っていた）
        /// が、AST へ載せずに捨てていたので型検査へ届いていなかった。
        base_args: Vec<Vec<String>>,
        /// `@decorator` 構文で付与されたデコレータ式のリスト（上から順、適用は逆順）。
        decorators: Vec<Expr>,
...
/// `**kwargs` を表す**番兵パラメータ名**（`import[py]` の変換器だけが生成する）。
///
/// Arrow に `**kwargs` 相当の構文は無い。変換器は「任意のキーワード引数を受ける」印として
/// この名前のパラメータを 1 つ置き、`bind_args_relaxed` が余ったキーワード引数を dict にして
/// **同じ名前**で束縛する（本体の参照も変換時に同じ名前へ差し替える）。
///
/// ⚠ Arrow の識別子に `*` は使えないので、**実ユーザのパラメータ名と衝突しない**。
/// Arrow の可変長パラメータが `"..."` という番兵名を使うのと同じ考え方。
/// ⚠ ここ（`ast.rs`）に置くのは、**型検査器からも参照する**ため。
