// llvm_codegen.rs — LLVM IR text generator for Havakyrie native compilation.
//
...
//
// The handle-based ABI (all values as i64 tags) and HvCallbacks struct layout
...
struct FlatListInfo {
    /// 要素クラス名。
    class_name: String,
    /// アルファベット順の (フィールド名, 型) リスト（int/float のみ）。
    fields: Vec<(String, Ty)>,
    /// 要素あたりのバイト数 = `fields.len() * 8`。
    stride: usize,
}
...
// ── HvCallbacks field indices (must match native_api.rs) ─────────────────────
...
fn module_header() -> &'static str {
    r#"; Auto-generated by Havakyrie compiler — do not edit.
...
struct GenCtx<'a> {
    ...
    // Pre-read field locals: "param_name.field_name" → (alloca_ptr, Ty).
    // Populated at function entry for class params that are never written.
    // gen_expr uses this to replace CB_GET_FLOAT_FIELD with a plain load.
    preread_fields: HashMap<String, (String, Ty)>,
    // `let list[ClassName]` params that can use the flat GEP iteration path.
    // Populated in emit_fn when param type annotation is "list[ClassName]".
...
impl<'a> GenCtx<'a> {
    ...
    fn call_cb(&mut self, field: usize, args: &[String]) -> String {
        ...
        self.ec(&format!("{fp} = getelementptr inbounds %HvCallbacks, ptr {cb}, i32 0, i32 {field}"));
    ...
    pub fn gen_expr(&mut self, expr: &Expr) -> (String, Ty) {
        match expr {
            ...
            Expr::Attr { object, attr, .. } => {
                // Fast path: if this field was pre-read at function entry (preread_fields),
                // emit a plain load — zero callbacks.
                let preread_key = match object.as_ref() {
                    ...
                    // Also cover flat list loop variables: any var with a preread entry.
                    Expr::Ident(n) => {
                        let k = format!("{n}.{attr}");
                        if self.preread_fields.contains_key(&k) { Some(k) } else { None }
                    }
                    _ => None,
                };
        ...
        match stmt {
            ...
            Stmt::For { targets, iter, body } => {
                ...
                if let Some(ref fi) = flat_info {
                    ...
                    // Allocate pre-read allocas for each field; register in preread_fields.
                    // The actual stores happen at loop-body entry each iteration.
                    for (fname, fty) in &fi.fields {
                        let al   = format!("%_prf_{}_{fname}", target);
                        let tstr = llvm_ty(*fty);
                        self.alloca_buf.push_str(&format!("  {al} = alloca {tstr}, align 8\n"));
                        self.preread_fields.insert(format!("{target}.{fname}"), (al, *fty));
                    }
                    ...
                    // Load each typed field into its pre-read alloca.
                    for (fi_idx, (fname, fty)) in fi.fields.iter().enumerate() {
                        let byte_off = (fi_idx * 8) as i64;
                        let prf_al  = format!("%_prf_{}_{fname}", target);
                        let tstr    = llvm_ty(*fty);
    ...
    fn emit_fn(&mut self, name: &str, params: &[Param], ret_ann: Option<&str>, body: &[Stmt]) {
        ...
        // The flat path is only eligible when the class has typed fields (int/float only).
        self.flat_list_params.clear();
        for p in params {
            ...
            let class_name = &ann[11..ann.len() - 1];  // strip "fixed_list["
            if let Some(fields) = self.class_fields_ord.get(class_name) {
                if fields.is_empty() { continue; }
                // Sort alphabetically to match the runtime flat layout ordering.
                let mut sorted = fields.clone();
                sorted.sort_by(|a, b| a.0.cmp(&b.0));
                let stride = sorted.len() * 8;
        ...
        if ret_ty == Ty::Float {
            ...
            wrapper.push_str(&format!(
                "  %_mf_p = getelementptr inbounds %HvCallbacks, ptr %_cb_tl, i32 0, i32 {CB_MAKE_FLOAT}\n"
            ));
...
pub fn generate_llvm_module(stmts: &[Stmt]) -> Option<(String, Vec<FnExport>)> {
    ...
    let mut ctx = GenCtx::new(&module_fns, &fn_sigs, &class_fields, &class_fields_ord, &fast_fns);
    ...
    let header = if cfg!(target_os = "windows") {
        module_header().replace("define void @hv_init(", "define dllexport void @hv_init(")
    } else {
