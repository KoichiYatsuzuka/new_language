...
// - `ar_to_py`: tl の Value を PyO3 PyObject に変換する
...
pub fn ar_to_py(py: Python<'_>, val: &Value) -> PyResult<PyObject> {
    match val {
        ...
        Value::Dict(d) => {
            ...
            for (k, v) in borrowed.all_keys().iter().zip(borrowed.all_items().iter()) {
                dict.set_item(ar_to_py(py, k)?, ar_to_py(py, v)?)?;
            }
            Ok(dict.into())
        }
