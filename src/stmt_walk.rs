...
pub enum StmtPart<'a> {
    ...
    ///
    /// ⚠ 呼び出し元のフレームとは無関係なので、**実行に関わる 7 本すべてが降りないと決めている**。
    /// ⇒ 既定ビルドでは本体を読む消費者が 1 つも無いため `dead_code` が出る。
    /// **`allow` の範囲はこのフィールド 1 つだけ**（#51 の「属性が別の警告を食っていないか疑う」
    /// に対して、enum の 1 フィールドは他の警告を隠しようがない）。
    ///
    /// ⚠ **読む消費者は 1 つだけある** — `syntax_cov`（#85）が別モジュールの構文カバレッジを
    /// 数えるために降りる。ただし `#[cfg(feature = "tw_stats")]` なので**既定ビルドには居ない**。
    /// ⇒ `allow` は外せない（外すと既定ビルドが警告 2 件になる。実測で確認済み）。
