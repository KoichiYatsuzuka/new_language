...
**速度レーンは打ち止め**。保守性は**第 2 弾（#58〜#68）を 2026-08-21 に起票**した
（全 src 機械診断・全件が前提なし・**先頭の #68 は実バグ**）。直近の到達点は 1 行ずつ（詳細は
...
| 57 | [stale_doc_refs.ps1](stale_doc_refs.ps1) の**再発 14 件を 0 に**（①改名の取り残しを現名へ ②履歴言及にマーカー語 ③接尾辞は `$whitelist`）。**負の対照で検知力も確認** |
...
⚠⚠ **ゲートを作っただけでは陳腐化は止まらない**（#51 → #57）。
#51 が 61→0 にした [stale_doc_refs.ps1](stale_doc_refs.ps1) は、**直後の 4 タスクで 14 件まで戻った**
（#52 の改名・#53 の分割・#56 の削除が**走らせなかった**）。#57 で 0 に戻した。
内訳は ①改名の取り残し **4** ②履歴だがマーカー語なし **7** ③誤検知（`_inner` は接尾辞）**3**
＝ **実害があったのは①だけ**で、残りは #51 が用意した逃げ道を使い忘れただけ。
⇒ **改名・削除をしたら必ず走らせること**（「検証は 5 点セット」に明記した）。
⚠ **マーカー語は行単位で効く** — 足すと**同じ行の他の識別子も検査から外れる**。
「とりあえず『削除』と書いて黙らせる」は検知力を捨てるので、**行の内容ごと確認する**。
...
⚠⚠ **4 度目が 2026-08-21 の診断で出た（→ #68）** — **`enum` を関数本体で宣言すると `VmForceError`**
（参照実装は通る）。`compile_stmt` に `Stmt::EnumDef` のアームが無く catch-all の bail に落ちる。
**またしても「例題が 1 本も無かった」**（#56 と同じ理由で同じことが起きた）。
⇒ **`force_gate` 0 件・151 例題 緑は「0 である」ことの証明ではない**。**未カバーの構文を
...
### 次にやる候補
> 🎉 **本系列の主目的は完了した**（2026-08-18）。解釈実行は**バイトコード VM 一本**になり、
> ツリーウォークの制御フロー・TLS・センチネルは**すべて削除**した（#33・src 実質 **-762 行**）。
> ツリーウォークが実行するのは**定義文だけ**（設計どおり・#10-d）。

> ⚠ **exec の中に速度目的の残タスクは無い**（#24 / #46 を実測して却下した）。
> **#50 の実測で狙い所が変わった**:
...
1. **保守性レーン第 2 弾（#58〜#68）— 2026-08-21 の全 src 機械診断で起票・全件が前提なし**。
   先頭の **#68 は実バグ**（`enum` in fn が `VmForceError`／参照実装は通る）＝ **これが最優先**。
2. 別レーン（#19 / #17-a / #17-b）— 外部接続系。いつ着手しても他をブロックしない。
3. ブロック中（#14 → #11 R2-c）— 前提の「モジュール間ネイティブ直リンク」が未計画。
   ~~保守性レーン第 1 弾~~ — **全 7 件（51〜57）完了**（2026-08-20）。速度目的ではないので
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| **69** | **`interp_init` の削減**（速度・#50 の①） | `ar_config.json` 祖先ウォークの遅延化／打ち切り ＋ `Interpreter::new()` の first-touch 仮説の検証 | — | **未着手・調査済み**（内訳実測: ウォーク **50〜75%**・`new()` 21〜43%。⚠ **61 と同じ場所**なので同時に） |
...
| **68** | **`enum` in fn が `VmForceError`（実バグ）** | `compile_stmt` に `Stmt::EnumDef` アームを足す＋**例題を新設** | — | **未着手・最優先** |
| 58 | `exec` の `Stmt::Import` アーム 210 行 | `exec_import()` へ切り出し `exec/modules.rs` へ移設 | — | 未着手 |
| 59 | 同じ木を歩く walker 8 本の統合 | `collect_declared_names` を `collect_program_globals` 側へ委譲 | — | 未着手（**既にドリフト済み**） |
...
| 66 | `Compiler` の 17 フィールドが `Chunk` の複製 | `ChunkBuilder` 部分構造体へ（`into_chunk` の逐語 move が消える） | — | 未着手 |
...
> [IMPLEMENTATION_LOG.md](IMPLEMENTATION_LOG.md) #58〜#68）。**全件が前提なし＝今すぐ着手できる**。
> 推奨順は **68 →（58・66）→ 59 → 62・63 →（61＋69）・60・64 → 67 → 70**（65 は最後）。
...
> **保守性レーン第 1 弾は全 7 件完了**（2026-08-19 の全 src 診断で起票 → 2026-08-20 に 51〜57 完了）。
> 速度目的ではないので A/B は**退行が無いことの確認**に使う。⚠ **55 は完了**（54 の前提は解除）。
> 結論は「**AST 引数版は通常実行から到達不能だが REPL/デバッガからは到達しうる**」なので、
> 54 は**削除ではなく委譲へ畳む**こと。
> ⚠ **51 で「コメントだけ」の変更がゲートを 2 件動かした** — `ExecResult::Return`（死んでいた）を
> 消したら `#[allow(dead_code)]` が隠していた unreachable 警告が 2 件出た。**属性が警告を
> 食っていないか疑うこと**。
> ⚠⚠ **「挙動不変」の主張は `AR_VM_DUMP=1` の突き合わせで確かめる**（#52 で確立・exit code では弱い）。
> **ただし async 例題は同一バイナリでも dump が揺れる**（worker スレッドの書き込み順とタスクの
> コンパイル回数がスケジューリング依存）ので**対象外にすること**。差分を見たら
> **まず同一バイナリで再現するかを見る**（#52 では 4 件すべてこれで無罪と分かった）。
> ⚠ **23（評価済み引数の struct 化）は 54 に吸収**した — 保留理由「速度効果なし」は #48 以前の
> 評価で、**この重複が実バグを産みゲートを素通りした**という新しい根拠が出たため。
...
🎉 VM 一本化レーンは全完了（2026-08-18）
    ...
    31（参照実装との差分検査）……完了 ＝ `compare_vm_modes` を失った後の代替網
    43（実行時型検査の高速判定）……完了 ＝ 上記で足した検査の費用を回収
    33（ツリーウォーク制御フロー・TLS・センチネル・`--vm` の削除）……**完了**（src 実質 -762 行）

    38（A/B スクリプトのデッドロック解消）……完了 ＝ 30 の前提が外れ、計測手段が揃った
    30（クロージャ Chunk の実体跨ぎ再利用）……完了 ＝ 性能リスクを解消（1.456x）

    44（デバッガ golden の録り直し）……完了 ＝ 全ゲートが緑に戻った
    45（FnValue.body を Rc<[Stmt]> 化）……完了 ＝ 生成費用が本体サイズに依存しなくなった
    24（peephole パターンの追加）……**却下** ＝ 実測で実行時効果 0%
    46（ループ反転）……**却下** ＝ 命令数 -20% でも 1.003x（命令には値段の差がある）
    ...
    50（非コンパイル実行の実行時間分布の実測）……**完了** ＝ 段別＋op 別サンプリング
        （`--features prof`。**exec は全体の 14%（中央値）＝端点 1.59x** ／ exec 内は呼び出し系 37〜38%）
    51（doc/属性の迷子と陳腐化コメントの一掃）……**完了** ＝ 実バグ 1 件（`#[inline(never)]` が
        orphan doc と一緒に別関数へ移っていた）＋ orphan doc 計 3 件 ＋ 存在しない識別子への
        参照 61→0（[stale_doc_refs.ps1](stale_doc_refs.ps1) 新設で再発防止）。A/B は 0.986〜1.013x（退行なし）
    52（`CompileMode` ＋ `Compiler::base`/`finish`）……**完了** ＝ 3 bool を enum 1 本へ、
        構造体リテラル 5×38／5×24 を既定値＋差分へ（compiler.rs -91 行）。
        **バイトコード 206/210 byte-identical**（残り 4 は async で**同一バイナリでも揺れる**）
    53（`vm/compiler.rs` のサブ分割）……**完了** ＝ §5.1 の当初設計を実施。4,331 行の単一ファイルを
        10 モジュールへ（最大 818 行）。**1 行も書き換えない機械的な移動**で、
        **バイトコード 202 例題すべて byte-identical**
    ...
    58〜68（保守性レーン第 2 弾）……**起票のみ・未着手**（2026-08-21 の全 src 機械診断）
        **68 は実バグ**（`enum` in fn が `VmForceError`）／58・66 が費用対効果最良／
        ⚠ **`exec_op` 836 行は対象外**（平坦な表・最大アーム 42 行・割ると #10-b の再演）

残り: 速度目的は無し。**保守性レーン第 1 弾 51〜57 は全完了／第 2 弾 58〜68 は未着手**／
      別レーン（外部接続系）／ブロック中の 14 / 11 R2-c
      ⇒ **今すぐ着手できるのは 58〜68 の 11 件と別レーン 3 件**
