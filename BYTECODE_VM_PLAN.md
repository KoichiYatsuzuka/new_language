...
⇒ **速度目的の残タスクは無い**。残るのは外部接続系の別レーンとブロック中の 2 件だけ。
...
> ⚠ **速度目的の残タスクは無い**（#24 / #46 を実測して却下した）。
> 次に速度をやるなら、`Jump` のような**安い命令**ではなく
> **`exec_op` 1 回あたりのコスト**（ディスパッチ方式）か
> **高い命令**（`Call` / `GetAttr` / `Store*`）を狙うこと。**必ず先に測る**。
...
- **⚠⚠ 実行命令の割合は「時間の割合」ではない**（#46）。`Jump` は実行命令の **12%** だが
  ...
  安い命令（`Jump`）ではなく **`exec_op` 1 回あたりのコスト**か**高い命令**を狙うこと。
  ⚠ **null 結果こそ「そもそも触っているか」を先に確かめる**（#46 では 2 バイナリの
  逆アセンブルを突き合わせて、反転が効いている経路を測っていることを確認してから捨てた）。
- **⚠⚠ 最適化は「何個あるか」ではなく「何回動くか」で決める**（#24）。逆アセンブルの
  静的頻度で 1 位だった `POP; RETURN_NIL`（6.68%）は**最上位文ごとに 1 回**動くだけで、
  1.5 億命令のベンチには現れない。**到達不能コードの除去は原理的に速度へ効かない**
  （実行されないものを消すため）。⇒ 判断の前に**ディスパッチループで実行 op を数える**
  （一時計測でよい。#24 では入れて測って撤去した）。
