...
### 次にやる候補
1. **#42** — import モジュール本体の VM 化。**#33 の残る唯一の前提**。
...
| **42** | **import モジュール本体の VM 化**（#33 の残る阻害要因） | `exec_module` が `push_scope` してから本体を回すので `scopes.len() != 1` になり、**最上位 VM の対象外＝丸ごとツリーウォーク**（実測: [module_toplevel_flow.ar](examples/interop/module_toplevel_flow.ar) で `module_body` 18 文・`tw_control_flow` 2 件）。⚠ #10-d が「モジュール本体は 20 文」として保留にした判断は**例題依存**だった（最上位に制御フローを持つモジュールの例題が 1 本も無かった） | — | 未着手（**#33 の残る唯一の前提**） |
...
--vm=off 廃止レーン（34/35/36/37/39/40 すべて完了 → 33 が着手可能になった）
    ...
    33（--vm とツリーウォークの一部を削除）……**部分完了**（-284 行）
        41（定義文脈の式の VM 化）……完了
        42（import モジュール本体の VM 化） → 33 の残り（制御フロー本体の削除）
