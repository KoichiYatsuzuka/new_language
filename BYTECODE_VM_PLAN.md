...
git の HEAD は `6f05f1d "#27-b"`。**#10-c2 と #10-d の計測は未コミット**。
...
- **#10-d は保留**（2026-08-14・**計測で両半分とも #3 に寄与しないと判明**）。①「モジュール本体」は
  **20 文しかない** ②「定義文」350 件は**制御フローを持たず TLS も使わない**
  （`exec/definitions.rs` は TLS を使う 12 ファイルに無い）。⇒ **#3 をブロックするのは bail の方**。
  ⚠ **#25 を作るときは定義文の扱いを決めること**（「bail 0」だと永久に引っかかる。
  **定義文は設計上インタプリタが実行する**と明示除外するのが素直）。
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（`Op::DeclareGlobal`・受理判定を
  **許可リスト→定義文の除外リスト**へ反転・最上位 Chunk **1,368**）。
  ⚠ **許可リストは新しい文種別を黙って取りこぼす**（着手時、式文 909 件が「試行すらしていない」状態）。
...
1. **#27-c 最上位の残り bail 511 件** — **#3 を実際にブロックしているのはここ**（`for`/`while`/
   `if`/`try` を含み TLS を使う）。**まず未帰属 46 件の計装を潰す**こと（#10-a と同じ手順。
   潰さないと何が効くか決められない）。以降は小粒の集合を 1 つずつ。
