...
保守性は**第 2 弾（#58〜#68）を 2026-08-21 に起票**し、**#68（実バグ）と #66 が完了**。
直近の到達点は 1 行ずつ（詳細は
...
⚠⚠ **4 度目は #68**（2026-08-21・修正済み）— `enum` を関数本体で書くと `VmForceError` だった。
...
2. **保守性レーン第 2 弾 — 残り 9 件（#58〜#65・#67）は全件が前提なし**（#66/#68 は完了）。
   次に費用対効果が良いのは **58**（`exec_import` 切り出し・210 行 1 アーム）。
...
> [IMPLEMENTATION_LOG.md](IMPLEMENTATION_LOG.md) #58〜#68）。**残る 10 件は全件が前提なし**。
> ⚠ **#68 の起票時の見立ては半分外れていた** — 先に発火するのは `compile_stmt` ではなく
> **decl-prepass の明示 bail リスト**で、**採番を足さずに bail だけ外すと `LoadLocal` が別の変数を読む**。
> **66・68 は完了**（2026-08-21）。残りは **58 → 59 → 62・63 →（61＋69）・60・64 → 67 → 70**（65 は最後）。
...
🎉 VM 一本化レーンは全完了（2026-08-18）
    ...
    58〜67（保守性レーン第 2 弾）……**66 完了・残り 9 件は起票のみ**（2026-08-21 の全 src 機械診断）
        次は 58 が費用対効果最良／⚠ **`exec_op` 836 行は対象外**（平坦な表・割ると #10-b の再演）
...
残り: **第 1 弾 51〜57 全完了／第 2 弾は 66・68 完了**／速度 69・70／別レーン（外部接続系）／ブロック中 14・11 R2-c
      ⇒ **今すぐ着手できるのは 58〜65・67 の 9 件・69・70 と別レーン 3 件**
