...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a・#22-a・#22-b・#22-c・#22-d・#21-b・#12・#2b・#2a・**#1** まで完了（**#22 系列・#2 系列・#1 とも完了**）。
git の HEAD は `bd24187 "#1 (partially)"`（#1-a/#1-b までコミット済）。
**#1 本体（#1-1〜#1-4）は未コミット**（ユーザー許可待ち）。

**→ #3（強制バイトコード）の前提は残り #10 のみ**（#1 の完了でツリーウォークへのデバッグ用
フォールバックが不要になった）。
...
- **#15b は消費者 0 件**（実測）。実装は残したが消費側へは配線していない。速度目的で再訪する価値は無い。
- **#15c は挙動保存が要点**。統合で「未解決だけを見ていた箇所」が解決済みも拾う回帰があり得るため、
  インタプリタ実行経路は `Resolution::Unresolved` に限定してある（広げるのは #15d）。
- **#15 は §7.4-1 のみ実施**（`Rc<str>` 化で文字列 1.14〜1.51x）。**§7.4-3 の文字列インターンは消費者 0 件と実測して保留**。
  ⚠ `Value::Str` を複製する新コードを書くときは **`deep_clone` だけは独立バッファ必須**（async のスレッド間 share-nothing）。
- **#22 系列で「重複への対処は 2 通りしかない」と確定**: 同じ判断をする 2 実装は
  **片方を他方への委譲にして畳む**（22-b/22-c）。前提が違って畳めないもの（コンパイル時 vs 実行時）は
  **不変条件をテストで固定する**（22-d の `VM_BUILTIN_NAMES`）。
  ⚠ **`*_evaled` 版を新設するときは必ず片方をもう片方の委譲にすること** —
  「CallArg 版と evaled 版がずれている」で実バグを 3 回出した（#22-a/-b/-c）。
...
1. **#10 import モジュール Chunk** — **#3（強制バイトコード）の唯一残る前提**。
   高コスト・低効果と実測済だが、#1 完了で前提が 1 件に減ったので D2 への最短路。
2. **#23 評価済み引数の struct 化** — `NativeFunction` を C 軸へ寄せる前提（27 箇所・速度効果なし）。

> **⚠ 速度目的の残タスクはほぼ枯れた**（2026-08-13）。#12（2.61x）・#2b（1.6x）・#2a（+4%）・
> #1-x（1.06〜1.32x）で VM 経路の分かりやすい無駄は取り切った。残りは **#3 に向けた地固め（#10）**か、
> 別レーン（外部接続系）。新しく速度課題を立てるなら**まず計測して支配項を出すこと**。
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← 10**（#1 は完了） | ブロック中（本系列の最終段） |
...
（12b → 2c は循環依存のため両方保留）
10 → 3　（1 は完了）
（#22 系列・#2 系列は完了）　（21-b は独立: #15e で 22-d への依存が解消）
```
