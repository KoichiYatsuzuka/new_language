...
**#10-a/-b/-c/-c2・#26・#27 系列（-a/-b/-c/-d 全段）・#25・#32** まで完了。
git の HEAD は `80551e6 "#27-d"`（それ以降は未コミット）。

**🎉 `force_gate` が 0/123**（2026-08-16）。**制御フローを持つツリーウォークは 1 文も残っていない**
（`vm_bail_fn` 0・`vm_bail_toplevel` 0・`vm_ineligible` 0・`in_fn` 0）。最上位の 348 件と
`module_body` の 20 件は**全て定義文**（設計上インタプリタが実行する・#10-d）。
**⇒ #3 に着手できる状態**。⚠ ただし **#29 の穴が残る**（GUI/対話/長時間ベンチの 5 例題は
タイムアウトで**判定されていない**）。

直近の要点だけ先に:
- **#27-d 段階 2b 完了（2026-08-16）: 可変キャプチャを VM 化 → `force_gate` 2→0**。
  `Value` は変えず、**slot と並行するセル表**（`Vec<Rc<RefCell<Value>>>`）をフレームに持つ。
  op は `LoadCell`/`StoreCell`/`StoreCellDeepCopy` の 3 つ。`run`/`exec_op` に `cells` を通す。
  セル変数になるのは ①このクロージャの可変キャプチャ ②入れ子 `fn` に可変キャプチャされる
  自分のローカル ③`static mut`（こちらは `static_cells` 直読みでセル表は使わない）。
  `MakeFn` が `cell_captures`/`static_captures` から `CapturedVar::Mutable(cell)` を組む
  ＝ `capture_env` の `Var::Mutable`→`Var::Cell` 昇格と同じ効果。
  ⚠ **`n_cells == 0`（大多数）は `Vec::new()` で確保しない**。
  ⚠⚠ **リゾルバが `push_base` する名前は、slot に入れなくても必ず 1 slot 消費する**（**「採番は
  リゾルバと同順・同数」が契約**）。段階 2a で `static` の採番を飛ばしたため**以降の slot が全部
  ずれ**、`LoadLocal` が範囲外を読んで落ちた（2a 単体では症状が出ず 2b で露見）。セル化した名前も
  同じ理由で**穴を残す**（`cell_by_slot` で `Resolution::Local` を振り替える）。
  ⚠ A/B: `--vm=auto` 0.960〜1.046x／**`--vm=off` でも同じ幅**＝VM 経路とは無関係（ノイズ床）。
- **#32 完了（2026-08-16）: async 本体を VM 化・3.77x**。`mng <- async->T:` の本体は worker で
  **100% ツリーウォーク**だった。`compile_async_body`（`compile_block_expr` ＋ `Return`）で Chunk 化し、
  捕捉環境は #27-d 段階 1 と**同じ `captured_slots`** で末尾 slot へ。
  ...
  コンパイル＝型特化は乗らない。それでも 3.77x 出るのは**ディスパッチと名前引きが消える**から。
  ⚠ **捕捉名と本体の宣言が衝突したら bail**。回帰検知は
  [async_vm_body.ar](examples/async/async_vm_body.ar)。
- **#27-d 段階 1・2a（2026-08-16）: `force_gate` 4→2**。
  ...
  **4 例題中 2 つは可変キャプチャを持っていなかった**。architectural なのは可変キャプチャだけ。
  - **段階 1（不変キャプチャ）**: `compile_fn` に `captures` を渡し、**全宣言の採番後・末尾に** slot を
    採る。`Chunk.captured_slots` を呼び出し側が**名前で引いて**書き込む（`captured_env` は
    HashMap なので反復順に依存させない。#32 も同じ表を使う）。
  ...
  - **段階 2a（`static`）**: 記憶域は元から**フレームではなく `Interpreter::static_cells`**（宣言位置が
    ...
    ⚠ `static` 名は `slots` に入れないが **slot は 1 つ消費する**（上の採番ずれ参照）。
- **#27-c 完了（2026-08-16）: `vm_bail_toplevel` 0**。flat リスト組み込み（1 実装化）・
  `let x = <グローバル識別子>`（`StoreLocalFromIdent`）・**`try/except/finally`**・ブロック式内の
  入れ子 `fn`・一般の呼び先式・**組み込み/メソッドのキーワード引数**（`CallBuiltinKw`/`CallMethodKw`）。
  ⚠ **`force_gate` は例題ごとに最初の 1 件で止まる**（5 例題の裏に 7 原因）。潰すたび測り直す。
  ⚠⚠ **`let x = <識別子>` は `Resolution` を見てはいけない**（`exec_let` は ident でありさえすれば
  ソースの可変性で分岐する。`Resolution::Global` を非識別子式の枝へ落とすと**可変グローバルの
  コピー＆フリーズが漏れる** — 潜在バグ 1 件・#15e の原則）。
  ⚠ **「相互作用が複雑」と書かれた bail は既存の組み合わせで足りることがある**: `try/except/finally`
  は **`try/except` を `try/finally` で包む**だけで揃った（根拠は `run` が例外時に `handlers.pop()`
  してから landing pad へ跳ぶこと）。
  ⚠ **採番 walker の漏れが再発**（`collect_nested_decls` の `Stmt::FnDef`）。**`compile_stmt` に
  文種別を足したら必ず採番側も見る**。
  ⚠ **組み込みのキーワードは名前ごとに扱いが違う**（`enumerate` は `start` だけ許容／`zip` はエラー／
  `len` は名前を無視）。確認済みの名前だけ `VM_BUILTIN_KW_NAMES` へ。メソッドは
  **`CallMethodLocal` 融合の可否を引数コンパイル前に決める**。
- **#27 続き（2026-08-15）: `fn_FAILED` 11→4**。**関数本体の未解決 Ident**（`Op::LoadGlobal`）・
  **属性複合代入のレシーバ無制限**・**for ターゲットのシャドウ**・**可変長引数**（`local::args`）。
  ⚠ **`force_gate` の文言だけで bail と `vm_ineligible` を区別するな**（どちらも
  "cannot compile function 'X'"）。帰属は `AR_TW_STATS` の両表を突き合わせる。優先度は
  「その例題が落ちる理由が**全部**消えるか」で決めること（片方だけ潰しても落ち続ける）。
...
- **#27 前半**: `pass`/`break_point`/`undefined`/`obj::Trait.attr`/`Self` ＋ **クロージャの外側関数**
  （`Op::MakeFn`）。⚠ `break_point` は **`vm_debug_pause` 経由**（直呼びだと REPL からローカルが
  見えない）。⚠ **`capture_env` と自由変数の定義がずれると閉包変数が黙って消える**。
- **#25 完了。`--vm=force` が実際のゲート**（[force_gate.ps1](force_gate.ps1)・36→**2**/123）。
  ⚠ **`vm_eligible` が偽も失敗として数える**／**定義文は対象外**（#10-d）／
  **force_gate = 止めて判定・AR_TW_STATS = 数えるだけ**（件数は後者で見る）。
...
- **#10-d は保留**（計測で **#3 に寄与しない**＝設計上インタプリタが実行するものとして除外）。
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（受理判定を**許可リスト→定義文の除外
  リスト**へ反転。⚠ 許可リストは新しい文種別を黙って取りこぼす／宣言文を `collect_nested_decls`
  に渡すと宣言名に slot を振り値がフレームへ消える）。
- **#27-b 完了。メソッド dispatcher を 1 本化**（実質 **-230 行**。見積もり 644 行は過大）。
...
- **#15b / #15-3 は消費者 0 件**。⚠ `Value::Str` を複製する新コードは **`deep_clone` だけ独立バッファ必須**（async の share-nothing）。
...
### 次にやる候補
1. **#29（ゲートの穴を塞ぐ）** — **#3 の前に片付けるべき**。`force_gate` は 0 件になったが、
   GUI/対話/長時間ベンチの 5 例題（`spider_solitaire`・`flat_bench`・`cs_form_app`・
   `langtons_ant`×2）は 20s で kill され**判定されていない**。今の「0 件」はその 5 件を
   含んでいないので、#3 の根拠としては弱い。
2. **#3（強制バイトコード）** — フォールバック撤去＋TLS 4 本／センチネル 2 種の実削除。
   ⚠ ツリーウォークは `parse_ar` / デバッガ REPL / **定義文の実行**のために残る（§2.1/§2.3・#10-d）。
   消すのは**フォールバックと、制御フローのための TLS/センチネル**。
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **`force_gate.ps1` は 0 件になった**（制御フローを持つツリーウォークは 0 文）。⚠ ただし **#29 のタイムアウト 5 件が未判定**なので、先にそこを塞ぐのが安全 | **着手可**（本系列の最終段） |
...
```
29（ゲートの穴を塞ぐ）→ 3   ← 27/27-c/27-d/32 は全て完了。`force_gate` は 0/123 だが
                              **タイムアウト 5 例題は未判定**なので #29 を先に片付ける
30（クロージャ Chunk 再利用の計測）／ 31（参照実装との差分検査）は独立していつでも可
