...
**#14 の一部**（`.arc` 陳腐化検査）、**#15b**（`Ident` の AST 表現再設計）、
**#15c**（識別子 3 変種の統合）まで完了。
git の HEAD は `3bbfca9 "#11/14/18"`。**#15b / #15c と文書再編は未コミット**（ユーザー許可待ち）。
...
続けて **#15c**（`Ident`/`LocalRef`/`GlobalRef` の 3 変種統合）も完了。
`Expr::Ident { name, node_id, res: Resolution }` の 1 変種になり、リゾルバは変種を差し替えず `res` を書く。
**統合で「未解決だけを見ていた箇所」が解決済みも拾う**という非自明な回帰があり得るので、
インタプリタ実行経路は `Resolution::Unresolved` に限定して旧挙動を保存している（下記 #15c の項）。
...
15c. ~~**`Ident`/`LocalRef`/`GlobalRef` の 3 変種統合**~~ 【✅ 完了 2026-08-11】
    #15b から切り出した項目。**1 つの概念（識別子参照）に 3 変種**あり、各パスが同じ 3 アームを
    書かされる一方、名前だけ欲しい大多数のサイトはその区別を使っていなかった。
    `Expr::Ident { name, node_id, res: Resolution }` の 1 変種へ統合し、
    `Resolution::{Unresolved, Local(u32), Global(SlotCache)}` で解決状態を表す。
    **リゾルバは変種を差し替えるのではなく `res` を書く**ようになった。

    **注意（機械的な置換では壊れる）**: 統合前に `Expr::Ident` **だけ**にマッチしていた箇所は
    「未解決の識別子」しか見ていなかった。素直に統合すると解決済みも拾ってしまう。
    実際 `eval_builtin_ident_call` は名前だけで組み込みへ振り分けシャドウ検査が無いため、
    **`len` という名のローカル変数が組み込みに横取りされる**回帰になる。
    インタプリタ実行経路（`eval/`・`exec/`・`functions/`）の 18 サイトは
    `res: Resolution::Unresolved` に限定して旧挙動を保存した。VM の呼び出し経路も
    アーム順（Unresolved → Global → Local）で旧 if-chain と同一に保っている。
    **広げる（解決済みも受ける）のは意味論の変更なので、別途計測して判断すること。**

    効果: 3 変種 → 1 変種。3 way or-pattern を全廃。`Expr` のサイズは **112 バイトで不変**
    （`Resolution` は 16 バイトで既存の最大変種に収まる）。差分 28 ファイル・+177/−176 行。
    検証: `cargo test` 696 緑・警告 0・clippy 増分 0（HEAD と同じ 50 件）・
    off/auto **identical 42 / differing 0**・例題 **FAIL 0**・IR **byte-identical**。
