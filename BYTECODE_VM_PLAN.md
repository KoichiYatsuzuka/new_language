...
**⇒ #3 に着手できる状態**。**#29 でゲートの穴も塞いだ**（**128 例題すべて完走**・サンプル判定 0）。

直近の要点だけ先に:
- **#29 完了（2026-08-17）: 未判定 5 例題 → 0・128 例題すべて完走**。実際の穴は
  **タイムアウト時に stderr を読まずに捨てていた**こと（force エラーが出ていても取り逃す）。
  直した 3 手: `-Timeout` 20→45 秒（`flat_bench` が完走）／**kill の前に窓を閉じる**
  （DxLib・pygame は `ProcessMessage()` 非 0／QUIT で正常終了するので**終了処理まで走る**）／
  **閉じる操作を繰り返す**（`cs_form_app` はダイアログを順に出すので 1 回では終わらない）。
  ⚠ `MainWindowHandle` はキャッシュされるので毎回 `Refresh()` してから送ること。
  ⚠ **判定の健全性の前提**: `VmForceError` は `make_internal_raised_error` の許可リストに無く
  **`try/except` で捕まらない**＝必ずプロセスを終わらせる（＝生きている間は force エラー無し）。
- **#27-d 段階 2b 完了（2026-08-16）: 可変キャプチャを VM 化 → `force_gate` 2→0**。
  ...
  自分のローカル ③`static mut`（`static_cells` 直読みでセル表は使わない）。`MakeFn` が
  `cell_captures`/`static_captures` から `CapturedVar::Mutable(cell)` を組む
  ...
  ずれ**、`LoadLocal` が範囲外を読んで落ちた。セル化した名前も同じ理由で**穴を残す**
  （`cell_by_slot` で `Resolution::Local` を振り替える）。
...
- **#32 完了（2026-08-16）: async 本体を VM 化・3.77x**。`mng <- async->T:` の本体は worker で
  **100% ツリーウォーク**だった。`compile_async_body`（`compile_block_expr` ＋ `Return`）で Chunk 化し、
  ...
  ⚠ **worker は型注釈を持てない**（`Rc` で `Send` でない）ので空注釈＝型特化は乗らない。それでも
  3.77x 出るのは**ディスパッチと名前引きが消える**から。⚠ **捕捉名と本体の宣言が衝突したら bail**。
  回帰検知は [async_vm_body.ar](examples/async/async_vm_body.ar)。
...
- **#27-c 完了（2026-08-16）: `vm_bail_toplevel` 0**。flat リスト組み込み（1 実装化）・
  ...
  ⚠⚠ **`let x = <識別子>` は `Resolution` を見てはいけない**（`Resolution::Global` を非識別子式の
  枝へ落とすと**可変グローバルのコピー＆フリーズが漏れる** — 潜在バグ 1 件・#15e の原則）。
  ...
  文種別を足したら必ず採番側も見る**。
  ⚠ **組み込みのキーワードは名前ごとに扱いが違う**（`enumerate` は `start` だけ／`zip` はエラー／
  `len` は無視）。確認済みの名前だけ `VM_BUILTIN_KW_NAMES` へ。メソッドは
...
- **#27-c 前半**（`toplevel_FAILED` 175→5）。スライス式・`freeze`・`on`/`once`/`off`・`let a,b = t`・
  テンプレート実体化・**最上位の未解決識別子**・`d[k] += v`・`obj.attr = v` 等。
...
- **#15b / #15-3 は消費者 0 件**。⚠ `Value::Str` の複製は **`deep_clone` だけ独立バッファ必須**（async の share-nothing）。
...
### 次にやる候補
1. **#3（強制バイトコード）** — フォールバック撤去＋TLS 4 本／センチネル 2 種の実削除。
   **前提は満たした**（`force_gate` 0 件・128 例題すべて完走・制御フローを持つツリーウォーク 0 文）。
...
| [force_gate.ps1](force_gate.ps1) | **#3 へ進めるかのゲート**（#25）。全例題を `--vm=force` で実行し `VmForceError` を列挙。⚠ **止めて判定する**用途で件数は `tw_stats.ps1` で見る。GUI 例題は**タイムアウト後に窓を閉じて**完走させる（#29） |
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **前提は全て満たした**（`force_gate` 0 件・**128 例題すべて完走**・制御フローを持つツリーウォーク 0 文） | **着手可**（本系列の最終段） |
...
```
3（強制バイトコード）← 前提は全て満たした（27/27-c/27-d/32/29 完了・`force_gate` 0・128 例題完走）
