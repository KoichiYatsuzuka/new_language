...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a・#22-a・#22-b・#22-c・#22-d・#21-b まで完了（**#22 系列は完了**）。
git の HEAD は `eac47e5 "replan"`（#15b〜#15e・#20・#21-a までコミット済）。
**#22-a〜#22-d / #21-b と文書更新は未コミット**（ユーザー許可待ち）。
...
1. **#12 R0-A 明示フレームスタック** — 残る最大の速度余地（~630ns/call）。統一目的には寄与しない。
2. **#2a / #2b V-F 最適化**（peephole・型注釈からの単型算術命令）。
3. **#23 評価済み引数の struct 化** — `NativeFunction` を C 軸へ寄せる前提（27 箇所・速度効果なし）。
...
| 12 | R0-A 明示フレームスタック | `Rc<Frame>` のスタック化 | — | 保留（**残る最大の速度余地** ~630ns/call） |
...
| 2c | V-F R0-A エスケープ解析 | 非エスケープフレームのフラット確保 | **← 12** | ブロック中 |
