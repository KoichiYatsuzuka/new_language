...
## 🚩 次スレッドへの引き継ぎ（2026-08-12・**ここだけ読めば再開できる**）

### 現在地
**#16（AST 型解決層）＝本系列の主目的は完了**。三経路（ツリーウォーク／VM／ネイティブ）が同一の型解決注釈を消費する。
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a・#22-a・#22-b・#22-c・#22-d・#21-b・#12・**#2b** まで完了（**#22 系列は完了**）。
git の HEAD は `052e6d3 "#12 (partially)"`（#12 までコミット済）。
**#2b は未コミット**（ユーザー許可待ち）。
...
1. **#2a V-F peephole** — 中規模。ジャンプ先再マップが要る。
   **#2b が具体的な着手先を 1 つ実測済み**: 属性複合代入 `obj.x += e` の命令列
   （`LoadLocal; e; LoadLocal; GetAttr; Swap; Bin; SetAttr` の 7 命令）。
   `GetAttrLocal` 化と `Swap` 除去で、変数版との残差 ~22ns/assign を狙える（根拠は実装ログ）。
...
- **検証は 4 点セット**: `cargo build`（**警告 0**）・`cargo test`（**699 緑**）・
...
| 2a | V-F peephole | Jump 除去＋ジャンプ先再マップ。**着手先の実測あり**: `obj.x += e` の 7 命令（`GetAttrLocal` 化＋`Swap` 除去で残差 ~22ns/assign） | — | 保留（中規模） |
...
| 1 | V-E 本体（汎用行テーブル） | op→span 行テーブル＋停止判定 | — | 保留（暫定対応で実用上不要） |
...
（12b → 2c は循環依存のため両方保留）
10, 1 → 3
（#22 系列は完了）　（21-b は独立: #15e で 22-d への依存が解消）
```
