...
⚠⚠ **#58〜#74 は全件完了**（**保守性レーン第 2 弾** #58〜#68 を 2026-08-21 の全 src 機械診断で
一括起票・#72〜#74 は派生／**速度レーン** #69 `interp_init`・#70 最上位ループ）。
...
- **同じ木を歩く walker が 2 つあるとずれる**。`compile_stmt` に文種別を足したら
  **必ず採番側（`collect_nested_decls`）も見る**（#27-c で 2 回踏んだ）。
  ⇒ **強制化は 3 点セットで完成した**（exhaustive 2 段で **variant を足すと止まる**）: 束縛する名前＝
  [decl_names.rs](src/decl_names.rs)(#59)／式の直下＝[expr_walk.rs](src/expr_walk.rs)(#81)／
  文の下の本体・部分式＝[stmt_walk.rs](src/stmt_walk.rs)(#84)。⚠ **`_ => {}` で黙らせない**
  （降りないなら**理由を書いて列挙**）。⚠⚠ **順序に意味がある束縛は種類を分ける**（`for` ターゲット・
  `except as` 別名 — 畳むと採番がずれる）。⚠⚠ **止まるのは 2 段目だけ**で「列挙し忘れ」は人間が読むしかない
  ⇒ **畳む前に「元の N 実装が何を見ていたか」の表を機械生成して突き合わせる**（#84 は実バグ 3 件が出た）。
...
- **検証は 5 点セット**: `cargo build`（**警告 0**）・`cargo test`（**750 緑**）・
  ...
  **使う前に同一 exe 同士で負の対照**を取る。#68 は 108/108 を確認してから A/B を読んだ）。
  ...
  clippy は **`cargo clippy` で 50 件**・**`cargo clippy --all-targets` で 65 件**（差は `benches/`）。
  ⚠⚠ **この数字を信用せず、同じセッションで HEAD を stash して測り直す** — #84 の時点で
  プラン「50/63」・#75 の実装ログ「51/66」・実測「50/65」と **3 通りあった**。
...
🎉 VM 一本化レーン（33〜45）／保守性レーン第 1 弾（51〜57）・第 2 弾（58〜74）は**全完了**
   （1 行ずつは冒頭の到達点表。⚠ **完了レーンの内訳は区切り線の下へ移した**・規約 3）
残り: **#76〜#82**（第 3 弾・**#75 は完了**）＋ 別レーン 3 件（#19 / #17-a / #17-b・外部接続系・優先度低）
