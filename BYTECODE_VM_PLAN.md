...
### 次にやる候補
1. **#39** — `--vm=off` でしか走らない**言語機能**の残り（関数からのグローバル代入）。着手可。
...
> **#34 / #35 / #37 / #40 は完了**（2026-08-17）。⚠ どれも「VM に載せる」だけでは終わらず、
...
> ⚠ **`--vm=on` で動かない正しいプログラムはまだ残っている**（#39）。
...
--vm=off 廃止レーン（34/35/37/40 は完了。残り 2 本 → 最後に 33）
    34（break/continue 貫通）／35（block_return 検査）／37（finally 跨ぎ）／40（finally 本体）……完了
