...
⇒ **#84・#85・#86・#88 は完了**（下の到達点表）。**残りは #87 と別レーン 3 件**。
...
- **⚠⚠ 採番はリゾルバと同順・同数**（`static` で飛ばして `LoadLocal` が範囲外を読んだ）。
  ⇒ **#86 でデバッグビルドが止める**（`local_slot` が突き合わせ・`into_chunk` が範囲検査）。
...
- **⚠ VM は「解決情報が揃っている」前提**（#3/#36）。供給しない入口では正しいコードでも
  `VmForceError` になる。⇒ **#88 で配線を 1 箇所へ**（`resolve_and_annotate` ＋
  `wire_resolution`。setter は private ＝ **対でしか渡せない**）。
...
1. 🎉 **第 3 弾（#75〜#83）＋ #84〜#86・#88 完了**。着手できるのは **#87** と**別レーン 3 件**
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 87 | `peephole::code_target_mut` の `_ => None` | 全 89 variant を列挙し**op を足すと止まる**形へ（⚠ #86 の負の対照で**ここだけ止まらない**と実証済み。⚠ 手本は #86 の `storage_operands`＝機械生成＋件数 assert） | — | 未着手 |
