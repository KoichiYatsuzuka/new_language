...
⇒ **着手可能なのは #76〜#82 ＋ 別レーン 3 件**（全件が前提なし）。
...
> **✅ 全ゲート緑（2026-08-24・#75 完了時に自分で走らせて確認）**
> `cargo test` **750** ／ `cargo build` **警告 0**（`--features prof` / `--features tw_stats` も 0）／
> `cargo clippy` **51**・`--all-targets` **66**（増分 0。⚠ 数え方は下記）／
...
> ⚠ **`cargo clippy` と `cargo clippy --all-targets` は別の数**（51 ↔ 66。差は `benches/`）。
...
1. ⚠⚠ **#76〜#82（第 3 弾）が起票済み・全件が前提なし**（**#75 は完了**）。実バグの温床は **#76・#77**。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 76 | typed ABI 引数マーシャリングの **4 コピー** | 書き戻し先だけを引数化して 1 本へ（#54 が呼び出しだけ畳んだ続き） | — | 未着手 |
...
> （第 3 弾。A群＝実バグの温床 75〜77／B群＝逐語重複 78〜80／C群＝ドリフト強制の穴 81・82。
