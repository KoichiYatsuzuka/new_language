...
**残るのは #24（低優先・効果は要実測）と外部接続系の別レーンだけ。**
...
1. **#24** — peephole パターンの追加（低優先・効果は要実測）。**速度目的の残タスクはこれだけ**。
2. 別レーン（#19 / #17-a / #17-b）— 外部接続系。
3. 新しく速度課題を立てるなら**まず計測して支配項を出す**（#45 は自分が書いた前提が外れた）。
...
- **⚠⚠ `Vec<T>` を `Rc<[T]>` に変えると `.clone()` の意味が黙って変わる**（#45）。
  「中身の複製」から「**参照カウント加算**」になり、**コンパイルは通る**。
  スレッド送出経路（`deep_clone`）でこれをやると **#15 の穴（非アトミック参照カウント）が復活**する。
  ⇒ 該当したのは 5 箇所中 3 箇所。**型では守れないのでポインタ同一性を見るテストで固定**する。
  ⚠ **async の実地ストレスでは検出できなかった**（clone されるのは外側 `Rc` で、内側に触るのは
  タスク終了時の drop 1 回だけ＝競合窓が狭い）。**決定的な検査を別に置くこと**。
