...
1. **保守性レーン（#53 ／ #55 → #54）** — 2026-08-19 の全 src 診断で起票。
   **#51 / #52 は完了**。速度目的ではないので A/B は**退行が無いことの確認**に使う。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 53 | `vm/compiler.rs` のサブ分割 | §5.1 の実装差分（stmt/expr/block_expr/decls/emit）を実施 | —（52 完了で解除） | 未着手 |
| 54 | native typed ABI 呼び出しの 1 本化 | 3 経路に手書きされた `transmute`＋呼び出し＋cleanup を 1 関数へ（#23 を「速度」から「2 実装の畳み込み」へ再定義した縮小版） | **← 55**（式経路が死んでいれば「畳む」でなく「消す」になる） | 未着手 |
| 55 | ツリーウォークの**式**評価経路の生死判定 | `tw_stats` に `record_expr` を feature 付きで追加し全例題で計測 | — | 未着手 |
...
> **53〜55 は保守性レーン**（2026-08-19 の全 src 診断で起票。**51 / 52 は完了**）。速度目的ではないので
> A/B は**退行が無いことの確認**に使う。⚠ 55 を 54 より先に走らせること（規模が変わる）。
...
残り: 速度目的は無し。**保守性レーン 53〜55**（診断で起票・51/52 は完了）／別レーン（外部接続系）／
      ブロック中の 14 / 11 R2-c
