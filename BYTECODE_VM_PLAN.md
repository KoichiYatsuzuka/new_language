...
1. **#27-b レシーバ dispatcher の畳み込み** — #3 の前提で、**最大の梃子になった**。
   `eval_method_call_evaled` を CallArg 版と同じ 16 レシーバへ（残 14 アーム・644 行）。
   これで最上位の bail 163 件のうち **98 件**と、#27 の残り 12 件が同時に片付く。
2. **#10-c2 最上位の式文・`if`/`try`** — 残ツリーウォーク 2,071 件のうち `Expr` 909 が最大で
   **まだ試行すらしていない**。受理対象を広げるだけで測れる。
3. **#27 の残り** — 入れ子 `fn`（クロージャ）8 が最大。可変長 2・`static` 2 は小粒。

> ⚠ **`--vm=force` は現状 no-op**（`main.rs` でパースされるだけで `Auto` と区別されていない）。
> §5.4/§10 の「`--vm=force` で穴を可視化」は**実装されていない**。#3 へ進む前に、
> これを実際に失敗させて例題＋テスト全体で bail 0 を要求するゲートにするのが前提。
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← 10-d ＋ 27**（#1・#10-b・#10-c は完了） | ブロック中（本系列の最終段） |
