...
## 🚩 次スレッドへの引き継ぎ（2026-08-18・**ここだけ読めば再開できる**）
...
**定義文だけ**（設計どおり・#10-d）。`force_gate` **0 件・139 例題完走**。

**#30 も完了した（クロージャは定義サイトごとに 1 回だけコンパイル・`bench_closure` 1.456x）。
残るのは #44（デバッガ golden の録り直し・**ゲートが赤い**）／#45（`FnValue.body` の共有）／
#24（低優先）／外部接続系の別レーン。**
...
1. **#44** — [debug_session.ps1](debug_session.ps1) の golden 録り直し（**最優先**）。
   **#33 partial からずっと赤い**＝ステッピングを検査しているゲートが 1 つも無い状態。
2. **#45** — `FnValue.body` の共有（`Rc<[Stmt]>` 化）。#30 の実測で残った **1.05µs/実体**。
3. **#24** — peephole パターンの追加（低優先・効果は要実測）。
4. 別レーン（#19 / #17-a / #17-b）— 外部接続系。
...
- **検証は 5 点セット**: `cargo build`（**警告 0**）・`cargo test`（**734 緑**）・
  [compare_python_impl.ps1](compare_python_impl.ps1)（参照実装との stdout 差分・**46/46**）・
  [scan_examples.ps1](scan_examples.ps1)（例題 **FAIL 0**）・[force_gate.ps1](force_gate.ps1)（`VmForceError`
  **0 件**）。⚠ **release バイナリを見る**。デバッガに触るなら [debug_session.ps1](debug_session.ps1)、REPL なら [repl_session.ps1](repl_session.ps1)、
...
| [debug_session.ps1](debug_session.ps1) | **対話デバッガのステッピングの回帰検知**（#1 で新設・#33 で golden 化）。`examples/debugger/<name>.{ar,in,out}` の期待値比較。⚠ 他のどのゲートも stdin を与えないので、**ステッピングはこれだけが検査している**。⚠⚠ **`6bf039c` から 5 件とも FAILED**（golden が BOM 修正前のまま＝#44）。**直すまでステッピングは無検査**なので、当面は**同じシナリオを 2 バイナリで走らせて transcript を突き合わせる**（#30 で実施） |
...
- **⚠ `ReadToEnd()` を stdout→stderr の順に逐次呼ぶと子とデッドロックする**（#34 で 1 時間停止・#38 で修正）。
  ...
  **必ず `ReadToEndAsync()` で同時に読む**（[scan_examples.ps1](scan_examples.ps1) が手本）。
  ...
  ⚠ **普段の実行では踏まない**（子の stderr が数 KB を超えて初めて発火する）ので、
  新しく子プロセスを起こすスクリプトを書いたら **`AR_VM_DUMP=1` を付けて回して確かめる**
...
- **⚠⚠ ゲートは「緑だと報告された」ではなく「自分で走らせて緑」を確かめる**（#30）。
  [debug_session.ps1](debug_session.ps1) は **#33 partial から赤いまま**で、しかも #33 の完了報告には
  「5 identical」と書かれていた。**修正とその golden を同じコミットで入れると、録り直し漏れが
  レビューでも気づかれない**（`6bf039c` は「stdin の BOM 修正」と「修正前の golden」を同時に入れた）。
...
- **退行を疑ったら「そもそも触っているか」を先に見る**。⚠ `--vm=off` での切り分けは #33 で
  失われたので、**同じ 2 バイナリで測り直して再現するか**と「変更と同規模のプローブ」で判断する
  （#43 では 2 回再現したので実費用と判断し、原因を `AR_VM_DUMP` で特定できた）。
...
- **⚠ worker スレッドは `Interpreter::new()` を作る**（#32）。**`--vm` も型注釈も引き継がない**ので、
  親の設定（型注釈等）を渡し忘れると**ゲートに穴が開く**（実際に開いていた）。
- **⚠ `force_gate` 0 件は「その時の例題で 0」であって「言語全体で 0」ではない**（#34/#35 で判明）。
...
- **⚠ `--vm=on` が bail する形は「エラー報告」も食い違う**（#34）。実行時エラーになると分かっている
  文（囲むループの無い `break`）を bail すると `VmForceError` になり、**本来のエラー文言が消える**。
  **必ず失敗する文は bail せず `Op::Fail` で同じ文言を出す**。
- **⚠⚠ 最上位 Chunk キャッシュは `Stmt` の**アドレス**をキーにする**（#36）。載せた AST を解放すると
  アロケータが同じアドレスを再利用し、**別の文が前の文の Chunk を実行する**（REPL がブロックごとに
  AST を捨てて `let xs = …` が `let total = …` の Chunk を走らせた）。新しい入口は **AST を保持し続ける**。
- **⚠ ゲートが「例題の書き方」に依存していないか疑う**（#36）。`toplevel_vm_candidate` が
  `!toplevel_globals.is_empty()` を条件にしていたため、**最上位に宣言の無いプログラム**は
  最上位が丸ごとツリーウォークだったのに `force_gate` 0 件・`tw_control_flow` 0 のままだった
  （例題が必ず何かを宣言していたため）。**負の対照が発火しないときは、まず配線を疑う**。
...
| **44** | **デバッガ golden の録り直し** | [debug_session.ps1](debug_session.ps1) が **#33 partial（`6bf039c`）からずっと FAILED**。同じコミットが「stdin を BOM 無しで書く」修正と「修正前の出力の golden」を**同時に**入れたため、golden に BOM 由来の**二重プロンプトと偽エラー**が焼き付いている。⚠ **`-Update` で黙って上書きしない**（差分の中身＝偽の 1 行が消えることを確認してから）。⚠ ステッピングを見ているゲートはこれだけなので**現状は無検査** | — | **未着手（最優先・ゲートが赤い）** |
| **45** | **`FnValue.body` の共有**（`Rc<[Stmt]>` 化） | #30 の実測で、クロージャ生成そのものに **1.05µs/実体**残っている。大半は `FnValue` が `body: Vec<Stmt>` を**実体ごとにディープクローン**する費用。`Rc` 共有にすれば消えるが `body` の消費者が多い。⚠ **#30 とは別軸**（#30 はコンパイル、これは AST 複製） | — | 未着手（効果は実測済み・影響範囲は要調査） |
...
残り: 44（デバッガ golden・**ゲートが赤い**）／45（FnValue.body 共有）／24（peephole・低優先）
...
```
cargo test                          # 734 passed を維持（各ステップ/各段ごと）
cargo build                         # 警告0 を維持
cargo clippy --all-targets          # exit 0
./scan_examples.ps1                 # 例題スイートの回帰確認（FAIL 0）
./compare_python_impl.ps1           # 参照実装との stdout 差分（46/46）— off/on 比較の代替
