...
16. **AST 型解決層 — コンパイル/ツリーウォーク/バイトコードの挙動統一**（#13 を包含・plan A の土台）。
    ...
    **注釈を信頼する FFI 境界検査では誤検知**になる。実際に再現した:
    ...
    - **モジュール横断（段階 F）**: node-id は**プログラム全体で一意**になった（C1 の実装漏れを修正済み）が、
      import 先モジュールの**関数本体は型検査の対象外**なので注釈が付かない
      （`collect_module_types` は署名しか読まない）。そのため import 先では
      注釈駆動の最適化・FFI 境界検査がスキップされる（誤動作はしない）。
      なお段階 E の「型注釈からの導出」は import 先でも効くため、二項演算の特化は一部すでに届いている。
