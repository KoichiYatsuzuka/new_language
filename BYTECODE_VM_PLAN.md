...
16. **AST 型解決層 — コンパイル/ツリーウォーク/バイトコードの挙動統一**（#13 を包含・plan A の土台）。**【新タスク・本命】**
    ...
    **「型検査の走査中に node-id テーブルへ型＋検査指示を書き込む」**（narrowing はスコープ再宣言で `infer` に既に反映済み＝
    ...
    - **【推奨・次の本命】`infer_attr` の戻り値をフィールド実型にする**（型検査の解像度・第2弾）。
      ...
      （[infer.rs:333-354](src/type_check/infer.rs#L333)）。このため `p.x * p.x` のような式が `Unresolved` 同士の
      `BinOp` になり **`binop_kind` が付かず VM の型特化 op が出ない**。戻り値を `annot_ty` に変えると
      **VM（plan A の 1.30x/1.36x 路線）とネイティブ codegen の双方に同時に効く**。for 推論と同じ構図で、
      「消費側ではなく型検査の解像度が律速」という c-2 結論の直接の続き。
      リスク: `Unresolved` で素通りしていた式に型検査が入るため新規エラーが出うる（for 推論と同じ手順で実測して判断）。
    - **段階(c-3)**: 自前再導出の撤去＋`CallInfo` の引数検査指示を境界インライン検査へ。
      **注意**: for 推論後の実測でも注釈が codegen を上回るのは `annot_only=7`（flat_bench_module）＋`call=2` のみ。
      ...
      「両経路が同一解決を共有する」という #16 の目的の達成としては意味があるが、速度・適用範囲の拡大は上記の
      型検査解像度の向上が先。
