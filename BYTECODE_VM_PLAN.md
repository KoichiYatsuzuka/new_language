...
- **#27-c 進行中**（`toplevel_FAILED` 175→**126**・**#3 に効く制御フロー文の bail は 18→10**）。
  リゾルバ 4 件＋`block:` 文＋`for k, v in ...`（`Op::UnpackTuple` 1 個）＋`collect_nested_decls` の
  `Stmt::Block` 漏れ。⚠ **VM コンパイラにシャドウ減算は不要**（文を 1 つずつ見るので、その文の束縛は
  全部 `slots` に入る。**`slots` を先に引く順序が健全性そのもの**）。
  ⚠ **同じ木を歩く walker が 2 つあると必ずずれる**（`collect_nested_decls` は `Stmt::Block` を
  降りていないのに `block_body_bails` は降りていた）。
...
- **#27-a/#26 完了。レシーバ判定を「形＋出自」の 2 段に**（`NamedInstance` は **Arrow クラスと外部言語
  スタブを区別しない**ので `arrow_class_names` で出自を見る）。⚠ **健全化で 2 件減った**＝正しい変化。
