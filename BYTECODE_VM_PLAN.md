...
1. **#23 評価済み引数の struct 化** — `NativeFunction` を C 軸へ寄せる前提（27 箇所・速度効果なし）。
2. **#10 import モジュール Chunk** — 高コスト・低効果と実測済だが **#3（強制バイトコード）の前提 2 件のうち 1 件**。
   本系列を D2 まで進めるなら避けて通れない。

> **⚠ 速度目的の残タスクはほぼ枯れた**（2026-08-13）。#12（2.61x）・#2b（1.6x）・#2a（+4%）で
> VM 経路の分かりやすい無駄は取り切った。残りは **#3 に向けた地固め（#10・#1）**か、
