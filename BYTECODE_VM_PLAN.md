...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20 まで完了（下表）。
git の HEAD は `5e13e9c "#15-b (resolver統合)"`（#15b/#15c はコミット済）。
**#15 / #15d / #20 と文書再編は未コミット**（ユーザー許可待ち）。
...
1. **py 組み込みスタブ整備**（`time`/`math`）— `Unresolved` の実測分布から効く先はここ（根拠は実装ログ #15b）。
2. **#17-b JS スタブの型付け** — FFI 境界検査は完成済みで、型が入れば即座に効く。
3. **#17-a C/C++ `void*` の専用型**。
4. **#12 R0-A 明示フレームスタック** — 残る最大の速度余地。統一目的には寄与しない。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| — | py 組み込みスタブ整備（`time`/`math`） | 同梱 `.pyi` ＋ `python_search_dirs()` に置き場を追加 | — | 未着手・**推奨** |
| 17-b | JS スタブの型付け | 既定 `Any`・`.d.ts` から `.ars` 生成 | — | 未着手 |
| 17-a | C/C++ `void*` の専用型 | 不透明ハンドル型を導入し int との相互代入を静的禁止 | — | 未着手 |
