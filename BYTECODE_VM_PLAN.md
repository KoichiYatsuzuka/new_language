...
最上位のツリーウォークは **1123 万 → 563 件**、関数本体内は **111 件**まで進行。
...
- **⚠ op 追加の代償が実測で見え始めた**: 今回 2 個で `bench_control_flow` **0.944x**・`flat_bench_interp`
  **0.936x**（`--vm=off` は 1.027x ＝ **ディスパッチループの肥大**が原因で当該 op は未実行）。→ **#28**。
- **#27-c 進行中**（`toplevel_FAILED` 175→**126**・**#3 に効く制御フロー文の bail は 18→10**）。
  ⚠ **VM コンパイラにシャドウ減算は不要**（文を 1 つずつ見るので束縛は全部 `slots` に入る。
  **`slots` を先に引く順序が健全性そのもの**）。⚠ **同じ木を歩く walker が 2 つあると必ずずれる**
  （`collect_nested_decls` が `Stmt::Block` を降りていなかった）。
...
1. **#27-c の残り**（126 件・**#3 に効くのは 10 件**: `For` 4・`Try` 2・`Match` 2・`While` 1・`Block` 1）。
   ⚠ 件数を優先度にしない。bail のキーは `<文種別>/<理由>` なので**制御フロー文の行だけ見ればよい**
   （1 つ潰すと次の層が露出するので件数の増減では判断しない）。
2. **#25 `--vm=force` のゲート化** — 小規模。**#3 に進めるかを判定できる唯一の手段**。
   ⚠ 定義文を「設計上インタプリタが実行する」と明示除外する設計判断が要る。
3. **#28 `Op::Rare` への畳み込み** — op 追加の累積コストが実測で見え始めた（今回 2 個で最大 6%）。
   #3 に向けて op はまだ増えるので、先に土台を作る価値がある。
4. **#27-d クロージャ本体の VM 化**（20 件）／**#27 の残り 11 件** — いずれも architectural。

> ⚠ **`--vm=force` は現状 no-op**（パースされるだけで `Auto` と区別されていない）。文書の各所にある
> 「`--vm=force` で穴を可視化」は**未実装**＝ **#25**。現状の可視化は `AR_TW_STATS` が担う。
...
| 28 | `Op::Rare` への畳み込み | 稀な 7 op（`MakeFn`/`UnpackTuple`/`BreakPoint`/`DeclareGlobal`/`GetTraitAttr`/`SetTraitAttr`/`LoadSelfClass`）を 1 アームに集約し `#[inline(never)]` の二段目 match で捌く。**op 1 個あたり ~1〜1.5%** の実測に基づく | — | 未着手（効果は要実測・#3 に向けて op はまだ増える） |
| 27-c | 最上位 Chunk 化の残り bail（**126**・うち #3 に効くのは **10**） | **#3 に効く 10 件**: `For` 4・`Try` 2・`Match` 2・`While` 1・`Block` 1。⚠ 残る `ident-unresolved:outer` は**クロージャのキャプチャ**とみられ #27 の入れ子 `fn` と同根。件数上位（一度きりの式文で #3 には効かない）: テンプレート実体化 31・`Slice` 21・`let x = <識別子>` 13 | — | 進行中（175→126 まで完了） |
...
| 25 | `--vm=force` を実際のゲートにする | 現状 `Auto` と同一の no-op。`VmMode::Force` のとき**コンパイル失敗を握り潰さずエラー**にし、例題＋テスト全体で bail 0 を要求する検証スクリプトを足す。**#3 の可否を判定できる唯一の手段**（`AR_TW_STATS` は件数を数えるだけで失敗させない） | — | 未着手（**#3 のゲート**・小規模） |
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← 27-c ＋ 27 ＋ 25**（#10 系列は完了・#10-d は保留） | ブロック中（本系列の最終段） |
...
> ⚠ 当初は各段の穴を `--vm=force` で可視化する想定だったが、**`Force` は実装されず `Auto` と同一**。
> 実際に使ったのは `AR_TW_STATS`。ゲート化は **#25**。
...
cargo run -- --vm=force <file.ar>   # ⚠ 現状 --vm=auto と同一の no-op（ゲート化は #25）
AR_TW_STATS=1 ./tw_stats.ps1        # 未対応構文の可視化は現状こちら（要 --features tw_stats）
...
- **移行中（Phase V 実装期間）はデュアルモード**が生命線: 未対応構文はツリーウォークにフォールバックし、`cargo test` を常時緑に保つ。
  ...
  ```
  `--vm=off` / `--vm=auto`（既定）/ `--vm=force`（**未実装。現状 auto と同一** — ゲート化は #25）の CLI フラグ。
