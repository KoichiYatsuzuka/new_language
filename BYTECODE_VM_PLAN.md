...
### 次にやる候補
1. **#35** — `--vm=off` でしか走らない**言語機能**の残り（`block_return`/`loop_yield` の実行時検査）。着手可。
2. **#36** — `--vm=off` でしか走らない**入口**（REPL・単体テスト）の VM 移行。着手可（#35 と並行可）。
3. **#33** — 上 2 つが終わってはじめて成立する（`--vm=off` の削除とツリーウォーク制御フローの実削除）。
4. **#30 / #31** — 独立レーン。いつでも着手できる。

> **#34 は完了**（2026-08-17）。制御フロー式を貫通する `break`/`continue` が VM に載った。
> ⚠ 途中で**バグを 3 種**踏んだ（詳細は実装ログ）: ツリーウォークの `continue` 2 件
> （SyntaxError 化・**黙って握り潰し**）＝*bail する形はツリーウォークが正しいとは限らない*の再演／
> **`try` のハンドラが残る**（`has_escape` は**文しか歩かない**のでブロック式内の `break` を見落とす）。
...
| **35** | **`block_return`/`loop_yield` の実行時検査を VM へ** | `BLOCK_RETURN_EXPECTED_TYPE` 由来の型検査（`block ->int: block_return "hello"`）と `loop_yield` の位置検査が VM に無く、**off は `TypeError`・on は素通り**＝現存する off/on 意味論差。単体テスト 6 件が押さえている。例題＋`_error` 例題の新設が必須 | — | **未着手（off/on 不一致）** |
...
--vm=off 廃止レーン（34 は完了。残り 3 本 → 最後に 33）
    34（break/continue 貫通の VM 化）……完了
    35（block_return 実行時検査の VM 化）┐
    36（REPL・単体テストの VM 移行）     ├→ 33（--vm=off とツリーウォーク制御フローの削除）
    37（try/finally を跨ぐ脱出の VM 化） ┘   ↑ 31 を先に済ませて代替網を作るのが望ましい
