...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e まで完了（下表）。
git の HEAD は `af72298 "#20"`（#15b/#15c/#15/#15d/#20 と文書再編はコミット済）。
**#15e のみ未コミット**（ユーザー許可待ち）。
...
1. **#21 最上位が `Unresolved` の件の結論** — #22-d の設計前提。
2. **#22-a 呼び出しディスパッチの分類**（コード変更なし）→ 22-b/c/d。
...
| 22-b〜d | 同 統合 | C を 4 分岐へ集約 → B を分離 → A に `res` を閉じ込め | **← 22-a, 21** | ブロック中 |
...
| 21 | 最上位コードが丸ごと `Unresolved` な件の調査 | リゾルバ対象をモジュール最上位へ広げるか判断 | — | 未着手（#15 の実測で判明。#10 の前提「最上位は初期化のみ」と関係） |
...
12 → 2c
10, 1 → 3
21 → 22-d,  22-a → 22-b → 22-c → 22-d（15e を吸収）
```
