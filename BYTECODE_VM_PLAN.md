...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a まで完了（下表）。
git の HEAD は `af72298 "#20"`（#15b/#15c/#15/#15d/#20 と文書再編はコミット済）。
**#15e / #21-a と文書更新は未コミット**（ユーザー許可待ち）。
...
1. **#22-a 呼び出しディスパッチの分類**（コード変更なし）→ 22-b/c/d。
2. **#21-b リゾルバを最上位へ広げる** — 上限 1.18x・機構は既存（21-a で判断済み）。
3. **#12 R0-A 明示フレームスタック** — 残る最大の速度余地。統一目的には寄与しない。
...
- **検証は 4 点セット**: `cargo build`（**警告 0**）・`cargo test`（**697 緑**）・
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 22-a | 呼び出しディスパッチの分類 | 全アームを A(同定)/B(正規化)/C(実行方式) に仕分け・コード変更なし | — | 未着手 |
| 22-b〜d | 同 統合 | C を 4 分岐へ集約 → B を分離 → A に `res` を閉じ込め | **← 22-a** | ブロック中 |
...
| 21-b | リゾルバを最上位文列へ広げる | `resolve_program` が最上位も rewrite（機構は既存: `collect_program_globals`＋`Resolution::Global`） | — | 未着手（**21-a で「やる価値あり」と判断**・上限 1.18x） |
...
12 → 2c
10, 1 → 3
22-a → 22-b → 22-c → 22-d　（21-b は独立: #15e で 22-d への依存が解消）
```
