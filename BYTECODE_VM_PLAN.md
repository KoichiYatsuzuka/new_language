...
## 🚩 次スレッドへの引き継ぎ（2026-08-16・**ここだけ読めば再開できる**）

### 現在地
**#16（AST 型解決層）＝本系列の主目的は完了**。三経路（ツリーウォーク／VM／ネイティブ）が同一の型解決注釈を消費する。
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15 系列・#20・#21・#22 系列・#12・#2a/#2b・#1・
**#10-a/-b/-c/-c2・#26・#27-a/#27-b・#25・#27-c・#27-d 段階 1/2a** まで完了。
git の HEAD は `80551e6 "#27-d"`（#27-c/#27-d の実装はコミット済み）。

**→ #3 に残る実装は #27-d 段階 2b（可変キャプチャ）ただ 1 つ**（#10-d は保留・#29 は判定の穴）。
最上位のツリーウォークは **1123 万 → 347 件＝全て定義文**（#32 で async 本体が VM へ移り、
定義文以外は消えた）。`module_body` の 20 件も全て定義文。関数本体内は **50 件**。
**`force_gate` は 2/123**（`make_counter` / `make_id_generator`）で、`vm_bail_toplevel` は **0**、
`vm_bail_fn` 4 件と `vm_ineligible` 13 件は**すべて「可変キャプチャ」1 種**に収束した。
⇒ **段階 2b が終われば制御フローを持つツリーウォークがゼロになり、#3 の TLS 実削除が可能になる。**

直近の要点だけ先に:
- **#32 完了（2026-08-16）: async 本体を VM 化・3.77x**。`mng <- async->T:` の本体は
  worker で `eval_block_expr` により **100% ツリーウォーク**だった（本体に直接書いたループは
  全反復がツリーウォーク）。`compile_async_body`（`compile_block_expr` ＋ `Return`）で Chunk 化。
  捕捉環境は #27-d 段階 1 と**同じ `captured_slots`** の仕組みで末尾 slot へ。
  ⚠⚠ **`run_task` が `Interpreter::new()` を作るので `--vm` が worker へ届いていなかった**
  （既定の `Auto` に戻る）。**`--vm=force` が async 本体を一切検査していなかった**＝ゲートの穴。
  `vm_mode` を `AsyncTask` 経由で運び、`Force` でコンパイル失敗なら `VmForceError` にした。
  ⚠ **worker は型注釈を持てない**（`AstAnnotations` は `Rc` で `Send` でない）ので空注釈で
  コンパイルする＝型特化は乗らない。それでも 3.77x 出るのは**ディスパッチと名前引きが消える**から。
  ⚠ **捕捉名と本体の宣言が衝突したら bail**（宣言より前は捕捉値が見えるので、slot を宣言側に
  取られると黙って値が変わる）。回帰検知は [async_vm_body.ar](examples/async/async_vm_body.ar)。
- **#27-d 段階 1・2a 完了（2026-08-16）: `force_gate` 4→2・`in_fn` 70→50**。
  ⚠⚠ **「クロージャ＝フレーム表現の変更が必須」は言い過ぎだった**。着手前に内訳を測ったら
  **4 例題中 2 つは可変キャプチャを持っていなかった**。architectural なのは可変キャプチャだけ。
  - **段階 1（不変キャプチャ）**: `compile_fn` に `captures` を渡し、**全宣言の採番後・末尾に** slot を
    採る（`Resolution::Local` の並びを壊さないため）。`Chunk.captured_slots` を呼び出し側が
    **名前で引いて**書き込む（`captured_env` は HashMap なので反復順に依存させない。#32 も同じ表を使う）。
    ⚠ 値は `clone` だけでよい（`Immutable` は生成時に deep_copy 済み）。
    ⚠ **キャプチャ名が既存 slot とぶつかったら bail**（黙って上書きすると閉包変数が消える）。
  - **段階 2a（`static`）**: 記憶域は元から**フレームではなく `Interpreter::static_cells`**（宣言位置が
    キー）なので、**セル表を作らずに** `LoadStatic`/`StoreStatic` で載る。初期化子は
    `StaticInit(span, after)` で「セルがあれば飛び越す」形（`exec_static_var` と同じ）。
    ⚠ `static` 名は **slot を持たない**。`slots` を引く経路すべてで**先に `statics` を見る**こと。
- **#27-c 完了（2026-08-16）: `vm_bail_toplevel` 0・`force_gate` 8→4**。載せたもの: flat リスト組み込み
  （1 実装化）・`let x = <グローバル識別子>`（`StoreLocalFromIdent`）・**`try/except/finally`**・
  ブロック式内の入れ子 `fn`・一般の呼び先式・**組み込み/メソッドのキーワード引数**
  （`CallBuiltinKw`/`CallMethodKw`）。
  ⚠ **`force_gate` は例題ごとに最初の 1 件で止まる**（5 例題の裏に 7 原因）。潰すたび測り直す。
  ⚠⚠ **`let x = <識別子>` は `Resolution` を見てはいけない**。`exec_let` は「`Expr::Ident` で
  ありさえすれば」ソース変数の可変性で分岐するので、`Resolution::Global` を非識別子式の枝へ
  落とすと**可変グローバルのコピー＆フリーズが漏れる**（潜在バグ 1 件・#15e の原則）。
  ⚠ **「相互作用が複雑」と書かれた bail は既存の組み合わせで足りることがある**:
  `try/except/finally` は **`try/except` を `try/finally` で包む**だけで全経路が揃った（実質 +10 行。
  根拠は `run` が例外時に `handlers.pop()` してから landing pad へ跳ぶこと）。
  ⚠ **採番 walker の漏れが再発**（`collect_nested_decls` の `Stmt::FnDef`）。
  **`compile_stmt` に文種別を足したら必ず採番側も見る**。
  ⚠ **組み込みのキーワードは名前ごとに扱いが違う**（`enumerate` は `start` だけ許容／`zip` はエラー／
  `len` は名前を無視）。一致を確認した名前だけ `VM_BUILTIN_KW_NAMES` に挙げ、メソッドは
  **`CallMethodLocal` 融合の可否を引数コンパイル前に決める**。
- **#27 続き完了（2026-08-15）: `fn_FAILED` 11→4・`force_gate` 12→8**。載せたもの: **関数本体の
  未解決 Ident**（`Op::LoadGlobal`）・**属性複合代入のレシーバ無制限**・**for ターゲットのシャドウ**・
  **可変長引数**（`local::args`）。
  ⚠ **`force_gate` の文言だけで bail と `vm_ineligible` を区別するな**（どちらも
  "cannot compile function 'X'"）。帰属は `AR_TW_STATS` の両表を突き合わせる。
  ⚠ **bail を潰しても例題が落ち続ける組み合わせがある**。優先度は「その例題が落ちる理由が
  **全部**消えるか」で決めること。
  ⚠ **`LoadName` は関数本体で使えないが `LoadGlobal` なら使える**（`scopes[0]` だけを見るため）。
  根拠は「**`slots` に無い＝この関数のローカルではない**」がコンパイル時に確定していること。
  ⚠ **レシーバ制限の理由は 2 通りある**: `attr_assign` は「2 実装の差」、属性複合代入は
  **`GetAttrLocal` 融合（レシーバ 1 回評価）の前提**。後者は**融合を使わない一般経路**を足して外す。
  ⚠ 副次効果で **#26/#27-a の「形＋出自」判定が dead code になり削除**（約 90 行）。
  **`Value::Instance` 前提の op を新設するなら復活させること**。
- **⚠⚠ `for-target-shadow` は「VM の表現力不足」ではなく「リゾルバの実バグ」だった**。外側に同名の
  base 宣言があると**ループ本体の読みが外側の値を指していた**（Python 実装 **6100** に対し Rust
  **400100**）。`resolve_function` が `base` から入れ子束縛を差し引いていなかった（グローバル側は
  既に差し引いていたので**同じ関数を共有**）。⚠ **`compare_vm_modes.ps1` はこの種のバグを検知
  できない**（両モードともツリーウォーク）。**bail する形はツリーウォークが正しいとは限らない**。
- **#27 前半**: `pass`/`break_point`/`undefined`/`obj::Trait.attr`/`Self` で 20 件解消
  （⚠ `break_point` は **`vm_debug_pause` 経由**。直呼びだと REPL からローカルが見えない）＋
  **クロージャの外側関数を VM 化**（`Op::MakeFn`＋`Chunk.fn_defs`）。
  ⚠ **`capture_env` と自由変数の定義がずれると閉包変数が黙って消える**。
- **#25 完了。`--vm=force` が実際のゲート**（[force_gate.ps1](force_gate.ps1)・36→**2**/123）。
  ⚠ **`vm_eligible` が偽（クロージャ等）も失敗として数える**こと。⚠ **定義文は対象外**（#10-d）。
  ⚠ 役割分担: **force_gate = 止めて判定／AR_TW_STATS = 数えるだけ**。件数は後者で見ること。
- **#27-c 前半**（`toplevel_FAILED` 175→5）。スライス式・`freeze`・`on`/`once`/`off`・`let a,b = t`・
  テンプレート実体化・**最上位の未解決識別子**（`Op::LoadName`）・`d[k] += v`・`obj.attr = v` 等。
  ⚠ **コンパイラの「この形のときだけ載せる」条件は必ず理由を確かめてから外す**（2 実装の差／
  最適化の前提／本当の非対応の 3 通りがあり、外し方が違う。詳細は実装ログ）。
  ⚠ **テンプレート実体化は引数を未評価のまま持ち回る**（先に評価すると制約検証より前に副作用が出る）。
  ⚠ **VM コンパイラにシャドウ減算は不要**（**`slots` を先に引く順序が健全性そのもの**）。
  ⚠ **「対象外」と「失敗」を同じ `None` で表すな**（意図的スキップ 336 件が失敗に化けていた）。
- **#10-d は保留**（計測で **#3 に寄与しない**と判明＝設計上インタプリタが実行するものとして除外）。
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（受理判定を**許可リスト→定義文の除外リスト**
  へ反転。⚠ **許可リストは新しい文種別を黙って取りこぼす**／宣言文を `collect_nested_decls` に
  渡すと**宣言名に slot を振り値がフレームへ消える**）。
- **#27-b 完了。メソッド dispatcher を 1 本化**（実質 **-230 行**。見積もり 644 行は過大だった）。
  ⚠ **レシーバ制限を外すなら `node_id`＋表示名/位置（`ffi_call_info`）を運ぶこと**（でないと FFI
  戻り値検査が VM だけ素通り）。⚠ **最頻路に判定を 1 つ足すだけで 3% 動く**。
- **⚠⚠ この規模の変更は VM 支配ベンチを ±5% 揺らす**（**#28 は却下**。稀な op 7 個を 1 アームへ
  畳んでも**何も回復せず** 1 件は悪化 ⇒ 効いているのは**アーム数ではなくコード配置**で、
  「op 1 個あたり ~1〜1.5%」というモデル自体が誤りだった）。→ **数 % の差で良し悪しを判断しない**。
  判断材料は「`--vm=off` でも同じ差が出るか」と「**変更と同規模**のプローブとの比較」の 2 つだけ。
- ⚠ **保留理由は経年で腐る**（#10 の保留判断は 2 点が実測で覆った）。
- **#10-b で最上位ループを Chunk 化**（E2E 1.11〜1.28x。欠けていたのは **`StoreGlobal` op ただ 1 つ**）。
  ⚠ **性能の罠を 4 件踏んだ**（索引キャッシュ／診断フック 11%／`exec_op` の展開／関数内ループの
  巻き添え）。この周辺を触るときは必ず A/B すること。
- **#1 完了。デバッグ中も VM を使う**（`run_stepping`）。⚠ **`exec_op` は `#[inline(always)]` が必須**
  （1.06〜1.32x 損していた）。⚠ デバッガ／`run.rs` を触ったら
  [compare_debug_modes.ps1](compare_debug_modes.ps1) を必ず回す。
- **#2a/#2b**: peephole は総命令の 0.31%。型特化の穴は op ではなく**文の側**にあった
  （`x += e` に `binop_kind` 未付与・1.9x）。
  ⚠ **コード索引を持つ op は飛び先だけではない**（`ForIter` の exit・`SetupTry` の handler・
  `StaticInit` の after）。追加時は `code_target_mut` へ（唯一の窓口）。
  ⚠ **`Expr` に注釈を付けたら同じ演算の `Stmt` 側も見ること**（複合代入・属性複合代入）。
- **#15b / #15-3 は消費者 0 件**（再訪価値なし）。⚠ `Value::Str` を複製する新コードは **`deep_clone` だけ独立バッファ必須**（async の share-nothing）。
- **#22 系列**: 同じ判断をする 2 実装は**片方を他方への委譲にして畳む**。畳めないものは
  **不変条件をテストで固定する**（22-d）。⚠ **`*_evaled` 版とずれた実装を作らない** — 実バグ **4 回**。
- **#15e/#15d の原則**: **注釈（`res`・型）は最適化ヒントであって意味論の根拠ではない**。破ると
  「同じコードが書かれた場所で挙動を変える」（**実バグ 5 件** — `mut→let` のコピー漏れ／FFI OutPtr の
  書き戻し漏れ／組み込み振り分け／#26 の `NamedInstance`≠Arrow クラス／#27-c の
  `let x = <Resolution::Global の ident>`）。⚠ `Resolution::Unresolved` は**「シャドウが無い」を
  意味しない**。判定は**実際の束縛／出自**を見る。

### 統一の到達度
**型の解決**（#16 c-3）・**ローカルの解決**（#11 R2-a/R2-a′）は三経路とも到達済み。
**グローバルの解決**（#11 R2-b）はツリーウォーク／VM が共有（ネイティブは参照が無く保留）。
### 次にやる候補
1. **#27-d 段階 2b: 可変キャプチャのセル共有**（`force_gate` の **残り 2 件**）— **#3 への最後の実装**。
   (1) **クロージャ側**（両方に必要）: `captured_env` の `Mutable(cell)` を呼び出しごとに
   フレームのセル表へ束縛し、本体の読み書きを `LoadCell`/`StoreCell` にする
   （`Chunk.n_cells` ＋ `captured_cells`、`run`/`exec_op` に `cells` を通す）。
   (2) **外側フレーム側**（`make_counter` だけ）: 「入れ子 `fn` に可変キャプチャされるローカル」を
   コンパイル前に洗い出し、slot ではなくセルに置く。`MakeFn` がそのセルを `Mutable` として渡す。
   ※ `make_id_generator` は外側のセルが `static_cells` にあるので **`MakeFn` が static セルを
   `Mutable` キャプチャとして渡すだけ**で済む。
   ⚠ **`freeze` との相互作用**: 「`Cell` 変数は freeze できない」規則があるので、
   セル化すると決めた名前に `freeze` があれば bail すること。
2. **#29（ゲートの穴）** — タイムアウト 5 件が判定されていないので、`force_gate` 0 件の意味が弱い。
...
| [force_gate.ps1](force_gate.ps1) | **#3 へ進めるかのゲート**（#25）。全例題を `--vm=force` で実行し `VmForceError` を列挙。⚠ **止めて判定する**用途で件数は `tw_stats.ps1` で見る。⚠ **タイムアウトした例題は判定されない**（#29） |
...
| 27 | fn の VM コンパイル失敗の解消（残 **4**） | 残り 4 件はすべて `nested-fn-mutable-capture`＝**#27-d 段階 2b と同一原因**。⚠ 解消済みの手: 未解決 Ident=`LoadGlobal`／属性複合代入=融合を使わない 2 回評価経路／for シャドウ=本体の間だけ slot 差し替え／可変長=`local::args` を末尾 slot へ／`static`=`static_cells` 直読み | **← 27-d 段階 2b と同時** | 進行中（49→**4**・原因は 1 種のみ） |
| 27-d | クロージャ本体の VM 化（残 `vm_ineligible` **13**・`force_gate` **2**） | **段階 1（不変キャプチャ→末尾 slot・`Chunk.captured_slots`）と段階 2a（`static`→`static_cells` 直読み）は完了**。残るのは**段階 2b＝可変キャプチャのセル共有**（クロージャ側のセル表＋外側ローカルのセル化）。⚠ クロージャ実体ごとに `FnValue` が別物で **Chunk を使い回せない**（ループ内生成で毎回コンパイル・要 A/B） | — | 進行中（**#3 の唯一の前提**・20→**13**） |
...
| **29** | **`force_gate` のタイムアウト 5 件を判定可能にする** | GUI/対話/長時間ベンチ（`spider_solitaire`・`flat_bench`・`cs_form_app`・`langtons_ant`(×2)）は 20s で kill され**判定されていない**＝「0 件」が「全例題が載る」を意味しない穴。入力を与える／反復数を絞る／`--vm=force` 専用の短縮モードのいずれかで到達範囲を広げる | — | **未着手（#3 の判定の健全性に直結）** |
| **30** | **クロージャ Chunk の実体跨ぎ再利用＋計測** | `get_or_compile_chunk` は `FnValue` ごとにキャッシュするので、**クロージャは実体ごとに再コンパイル**する。#27-d 段階 1 で初めて実際に走るようになった（それまでクロージャは VM 非対象）。⚠ **クロージャのベンチが 1 本も無い**ので、まず計測手段を作ってから判断する | **← 27-d 段階 2b**（対象が増えてから測る） | 未着手（**新たに生じた性能リスク**） |
| **31** | **`--vm=off` × `impl_python` の差分検査** | `compare_vm_modes` は**両モードともツリーウォークに落ちる形**を構造的に検知できない（#27 の `for-target-shadow` を取り逃していた）。参照実装との突き合わせスクリプトを作る。⚠ `impl_python` の対応範囲が狭ければ対象例題を絞る（実現可能性の確認込み） | — | 未着手（**実バグ 1 件を取り逃した穴**） |
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← `force_gate.ps1` が 0 件**（現在 **2/123**）。#32 完了で**制御フローを持つツリーウォークは `in_fn` の 50 件だけ**＝段階 2b で消える | ブロック中（本系列の最終段） |
...
```
27（残り 4 bail）＋ 27-d 段階 2b ─┬→ 3   ← 実装として残るのは **可変キャプチャ 1 種だけ**
29（ゲートの穴を塞ぐ）           ─┘        （27-c・27-d 段階 1/2a・32 は完了）
                                          判定は `force_gate.ps1` が 0 件（現在 2/123）
27-d 段階 2b → 30（クロージャ Chunk 再利用の計測）／ 31 は独立していつでも可
