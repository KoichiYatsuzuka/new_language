...
**→ 残っているのは #33（TLS/センチネルの削除・要判断）と、独立レーンの #30/#31 だけ。**
...
### 次にやる候補
1. **#33（TLS/センチネルの削除）— まず「`--vm=off` を捨てるか」を決める**。捨てないなら
   **このタスクは成立しない**（4 つとも `--vm=off` のためだけに生きている）。実装より判断が先。
2. **#30 / #31** — 独立レーン。いつでも着手できる。
...
| **33** | **ツリーウォーク制御フローと TLS/センチネルの削除** | ⚠ **まず「`--vm=off` を捨てるか」を決める**。`BLOCK_YIELDS`/`LOOP_DEPTH`/`BLOCK_RETURN_EXPECTED_TYPE`/`BREAK_SENTINEL` は通常実行で 0 回（実測）だが **`--vm=off` のために生きている**。`GENERATOR_YIELDS`（#8）と `RAISE_SENTINEL`（V-C）は **VM が使うので削除対象外**。捨てると [compare_vm_modes.ps1](compare_vm_modes.ps1)（唯一の差分検出網）と「`--vm=off` で切り分ける」判定基準を失う | **← `--vm=off` 廃止の判断** | **未着手（要判断）** |
...
「--vm=off を捨てるか」の判断 → 33（TLS/センチネル削除）  ← 捨てないなら 33 は成立しない
