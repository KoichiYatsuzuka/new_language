...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a・#22 系列・#21-b・#12・#2b・#2a・#1・**#10-a/#10-b/#27/#27-a/#26/#10-c** まで完了。
git の HEAD は `27ed68e "replan"`。**#27-c は未コミット**。

**→ #3（強制バイトコード）の前提は #10 だけではないと実測で判明**（下記 #10-a）。
前提は **bail の解消**（最上位 511 = #27-c ／ 関数 17 = #27）と**判定ゲート**（#25）。
最上位のツリーウォークは **1123 万 → 642 件**まで進行。**#10-d は計測で価値なしと判明し保留**。

直近の要点だけ先に:
- **#27 で 20 件解消**（`pass`／`break_point`／`undefined`／`obj::Trait.attr`／`Self`）。⚠ **`break_point` は
  `vm_debug_pause` を通すこと**（直呼びだと REPL からフレームのローカルが見えない）。
- **#27-c 進行中**（`toplevel_FAILED` 175→**134**・**新 op ゼロ**）。修正 3 件はいずれも
  **保守的すぎる方向のバグ**（動作は正しいまま最適化だけ効かない）: `enum`/`new_type` 名が
  `collect_program_globals` に無い／`AsyncAssign` の `target` を束縛扱い（実際は `get_var` だけ）／虚数リテラル。
  ⚠ **計測の穴を 2 つ塞いだ**: `toplevel_FAILED` 511 のうち **336 件は意図的スキップ**で失敗ではなかった
  （**「対象外」と「失敗」を同じ `None` で表すな**）。未帰属 46→3 は **bail に文種別と識別子名を添えて**特定。
  ⚠ リゾルバを触ったので **IR byte-identical を確認済み**（[dump_native_ir.ps1](dump_native_ir.ps1)）。
...
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（`Op::DeclareGlobal`・受理判定を
  **許可リスト→定義文の除外リスト**へ反転）。⚠ **許可リストは新しい文種別を黙って取りこぼす**。
  ⚠ **想定と実体が違った**: 93% は **`mut xs = for ...: loop_yield ...` の初期化子が N 回まわる**形。
  ⚠ 宣言文を `collect_nested_decls` に渡すと**宣言名に slot を振り値がフレームへ消える**。
  ⚠ **計測フックのバグを修正**: `record_stmt` が VM 試行より前にあり **VM へ渡した文まで数えていた**。
- **#27-b 完了。メソッド dispatcher を 1 本化**（`fn_FAILED` 29→**17**・実質 **-230 行**）。
  ⚠ **見積もり 644 行は過大**で、実際に要ったのは引数の扱い 4 パターンだけ。
  ⚠ **レシーバ制限を外すなら `node_id`＋表示名/位置（`ffi_call_info`）を運ぶこと**（でないと FFI 戻り値検査が
  VM 経路だけ素通り）。⚠ **最頻路に判定を 1 つ足すだけで 3% 動く**（ラッパより arm で直接分岐）。
- **#27-a/#26 完了。レシーバ判定を「形＋出自」の 2 段に**（`NamedInstance` は **Arrow クラスと
  外部言語スタブを区別しない**ので `arrow_class_names` で出自を見る）。⚠ **健全化でカバレッジが
  2 件減った**＝正しい変化。
...
- **#10-a で「#3 の前提は #10 のみ」が誤りと判明**（負荷は実質すべて最上位だが fn の失敗も 49 件あった）。
...
- **#1 完了。デバッグ中も VM を使う**（`run_stepping`）。⚠ **`exec_op` は `#[inline(always)]` が必須**
  （元から展開されておらず、付けたら 1.06〜1.32x 速くなった）。
  ⚠ デバッガ／`run.rs` に触ったら [compare_debug_modes.ps1](compare_debug_modes.ps1) を必ず回すこと。
...
- **#15b / #15-3 は消費者 0 件**（再訪価値なし）。⚠ `Value::Str` を複製する新コードでは
  **`deep_clone` だけは独立バッファ必須**（async の share-nothing）。
...
1. **#27-c の残り 134 件** — ⚠ **件数を優先度にしないこと**（#10-d と同じ罠）。#3 は TLS の削除なので
   効くのは**制御フローを含む文**（最上位の非定義文 ~239 件中 `If` 19・`For` 9・`Block` 5・`Match` 3・
   `Try` 2・`While` 1 の **計 39 件**）。**この 39 件を落としている bail から潰す**のが最短路。
2. **#25 `--vm=force` のゲート化** — 小規模。**#3 に進めるかを判定できる唯一の手段**。
   ⚠ 定義文を「設計上インタプリタが実行する」と明示除外する設計判断が要る。
3. **#27 の残り 17 件** — 入れ子 `fn`（クロージャ）8 が最大で architectural。他は小粒。
...
| 27-c の一部 | 最上位 bail **175 → 134**（新 op ゼロ） | `enum`/`new_type` 名を `collect_program_globals` へ（10）・`AsyncAssign` の `target` を束縛扱いしない（18）・虚数リテラル（13）。あわせて**計測の穴 2 件**を修正（意図的スキップ 336 件が失敗に化けていた／未帰属 46→3） |
...
| 27 | fn の VM コンパイル失敗の解消（残 **17**） | 内訳: **入れ子 `fn`（クロージャ）8**・`static` 2・可変長 2・未帰属 2・属性代入 2・for ターゲットのシャドウ 1 | — | 進行中（**#3 の前提**・49→17 まで完了） |
| 27-c | 最上位 Chunk 化の残り bail（**134**） | テンプレート実体化 31・`Slice` 21・`let x = <識別子>` 13・キーワード/可変長引数 10・`EventSubscribe`/`freeze`/`for` タプル分解 各 7・`LetTuple` 6・組み込みグローバル名 ~10 ほか。⚠ **#3 に効くのは制御フローを含む文だけ**（下記） | — | 進行中（175→134 まで完了） |
