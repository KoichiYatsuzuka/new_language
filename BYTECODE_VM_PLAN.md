...
⇒ **着手可能なのは #81・#82 ＋ 別レーン 3 件**（全件が前提なし。**75〜80 と 83 は完了**）。
...
> **✅ 全ゲート緑（2026-08-25・#80 完了時に自分で走らせて確認）**
...
1. ⚠⚠ **残りは C 群の #81・#82 だけ**（前提なし）。**A 群 75〜77・B 群 78〜80・派生 83 は完了**。**#81 は #75 と同じ穴（`Expr` の `_ => {}`）の恒久対策**なので再発防止の価値が最大。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 81 | `Expr` 再帰骨格の重複 3 本（`_ => {}` 終端） | 部分式を exhaustive に列挙する visitor を 1 本（**降り方の方針は walker 側に残す**） | — （75 完了で解除。1 本は 75 が exhaustive 化済み） | 未着手 |
| 82 | `generate_llvm_module` の適格関数収集 4 ブロック | 収集を 1 本化（**5 本目の `Stmt` walker**） | — | 未着手 |
...
> （第 3 弾。A群＝実バグの温床 75〜77／B群＝逐語重複 78〜80／C群＝ドリフト強制の穴 81・82／**83 は #76 からの派生**。**75〜80 と 83 は完了**（残りは C 群 81・82）。
