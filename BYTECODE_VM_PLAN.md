...
保守性は**第 2 弾（#58〜#68）を 2026-08-21 に起票**し、**#68（実バグ）・#66・#58・#59・#63・#62・#61・#69 が完了**。
直近の到達点は 1 行ずつ（詳細は
...
2. ⚠ **#71 は実バグ**（`import[py]` の関数が呼べない）なので保守性より先。
   保守性は**残り 4 件（#60・#64・#65・#67）＋#72**、全件が前提なし。次点は **64**（⚠ **まず理由を確かめる**）。
...
| **71** | **`import[py]` の関数呼び出しが `VmForceError`**（実バグ・#69 で発見） | `vm_eligible = !fn_val.is_python` が #33 でフォールバック消失により致命化。Python 関数を呼び出し前に振り分ける | — | **未着手**（原因確定・`exec_fn_evaled`。**同型の 5 度目**） |
...
> **58・59・61・62・63・66・68・69 は完了**。残りは **71（実バグ）→ 60・64・72 → 67 → 70**（65 は最後）。
...
🎉 VM 一本化レーンは全完了（2026-08-18）
    ...
    47（master との端点 A/B）……完了 ＝ 解釈 3.97x / native 境界 1.60x / 純ネイティブ 1.00x
    48（native `mut` ポインタ書き戻し）……**完了** ＝ #47 で検出した実バグを修正（cdll が 1.13〜1.16x）
    ...
    58〜67（保守性レーン第 2 弾）……**58・59・61・62・63・66 完了・残り 4 件は起票のみ**（2026-08-21 の全 src 機械診断）
...
残り: **第 2 弾は 58・59・61・62・63・66・68 完了**／速度は **69 完了**・70／**71（実バグ）・72 を新規起票**／別レーン／ブロック中 14・11 R2-c
      ⇒ **今すぐ着手できるのは 71（実バグ）・60・64・65・67・72 の 6 件・70 と別レーン 3 件**
