...
#44 でデバッガ golden を録り直し、**全ゲートが緑**に戻った。
残るのは #45（`FnValue.body` の共有）／#24（低優先）／外部接続系の別レーン。**
...
1. **#45** — `FnValue.body` の共有（`Rc<[Stmt]>` 化）。#30 の実測で残った **1.05µs/実体**
   （クロージャ生成ごとの本体 AST ディープクローン）。⚠ `body` の消費者が多いので影響範囲を先に調べる。
2. **#24** — peephole パターンの追加（低優先・効果は要実測）。
3. 別レーン（#19 / #17-a / #17-b）— 外部接続系。
...
- **⚠⚠ ゲートは「緑だと報告された」ではなく「自分で走らせて緑」を確かめる**（#30 で発見・#44 で修正）。
  [debug_session.ps1](debug_session.ps1) は **`6bf039c`（#33 partial）から 5/5 赤いまま**で、
  しかも #33 の完了報告には「5 identical」と書かれていた。**修正とその golden を同じコミットで
  入れると、録り直し漏れがレビューでも気づかれない**（`6bf039c` は「stdin の BOM 修正」と
  「修正前の golden」を同時に入れた）。
  ⚠⚠ **赤いゲートは「検知力が落ちる」のではなく「別物を検査する」**。`step_over_heavy` は
...
残り: 45（FnValue.body 共有）／24（peephole・低優先）
