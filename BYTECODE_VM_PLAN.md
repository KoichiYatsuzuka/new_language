...
### 次にやる候補
1. **#33 の残り** — ツリーウォーク制御フロー本体の削除（≈700 行）。#41/#42 で消費者を
   両方潰した。⚠ **着手前に必ず `tw_stats.ps1` を全例題で取り直す**こと
   （「例題が踏まない形が残っている」を **5 回**踏んでいる）。
...
| **43** | **`loop_yield` の実行時型検査の静的省略** | #35 が足した `Op::CheckLoopYield` は**反復ごとに走る**。`bench_block_expr` が **0.936x**（測定 2 回で再現・`CHECK_LOOP_YIELD` は 1 個だが `for i in range(n) ->list[int]: loop_yield i*i` の内側）。⚠ **`--vm=off` も同じ検査をしていた**ので退行ではなく「正しさの費用」。⇒ 注釈が `list[int]` で yield 式の型も `int` と確定しているなら**コンパイル時に省ける** | — | 未着手（**唯一の実測された速度課題**） |
...
| **33** | **ツリーウォーク制御フローの実削除** | ⚠ **部分完了**。到達不能だった分（関数本体 125 行・ジェネレータ 84 行・async 経路・`VmMode`/`--vm`・`FnBodyGuard`）は削除済み（**src 実質 -284 行**）。**制御フロー本体（`eval/control_expr.rs`・`exec/control_flow.rs`・TLS 5 種・`ExecResult` の 4 バリアント）は #42 が終わるまで削除できない**（#41 で定義文脈は潰したが、**import モジュール本体**が残っている） | **← 42** | **部分完了**（残りは #42 待ち） |
...
--vm=off 廃止レーン（34/35/36/37/39/40 すべて完了 → 33 が着手可能になった）
    ...
    33（--vm とツリーウォークの一部を削除）……**部分完了**（-284 行）
        41（定義文脈の式の VM 化）……完了 ／ 42（モジュール本体の VM 化）……完了
        → 33 の残り（制御フロー本体の削除・≈700 行）が着手可能
