...
git の HEAD は `a1d9c7e "#10-c"`。**#27-b・#10-c2 は未コミット**。

**→ #3（強制バイトコード）の前提は #10 だけではないと実測で判明**（下記 #10-a）。
最上位（#10-d）と**関数の VM コンパイル失敗**（#27）の両方が要る。#27 は **49 → 17**、
最上位のツリーウォークは **1123 万 → 663 件**まで進行し、**その 56% は定義文**（＝ #10-d 待ち）。

直近の要点だけ先に:
- **#27 で 20 件解消**（`pass`／`break_point`／`undefined`／`obj::Trait.attr`／`Self`）。
  ⚠ **`break_point` は `vm_debug_pause` を通すこと**（直呼びだと REPL からフレームのローカルが見えない）。
- **#10-c2 完了。最上位の残り文を Chunk 化**（受理判定を**許可リスト→定義文の除外リスト**へ反転・
  最上位 Chunk 499→**1,368**）。⚠ **許可リストは新しい文種別を黙って取りこぼす**（着手時、式文 909 件が
  「試行すらしていない」状態だった）。⚠ **残り 663 件の 56% は定義文**＝次に効くのは #10-d。
  ⚠ **計測フックのバグを修正**: `record_stmt` が VM 試行より前にあり **VM へ渡した文まで数えていた**
  （これまでの最上位の数字は過大。正: 663 件）。
- **#27-b 完了。メソッド dispatcher を 1 本化**（`fn_FAILED` 29→**17**・`method-receiver` bail 110→**0**・
  実質 **-230 行**）。⚠ **見積もり 644 行は過大**で、実際に要ったのは引数の扱い 4 パターンだけ。
  ⚠ **レシーバ制限を外すなら `node_id`＋表示名/位置（`ffi_call_info`）を op/Chunk で運ぶこと**
  （でないと FFI 戻り値検査が VM 経路だけ素通り＝#22-a と同型の穴）。
  ⚠ **最頻路（Instance）に判定を 1 つ足すだけで 3% 動く**（ラッパ 1 枚より arm で直接分岐が速い）。
- **#10-c 完了。最上位の宣言文を Chunk 化**（`Op::DeclareGlobal`）。⚠ **想定と実体が違った**:
  93% は **`mut xs = for ...: loop_yield ...` の初期化子が N 回まわる**形で、
  「1 回しか実行されない文は損」で切ると取り逃す。
  ⚠ 宣言文を `collect_nested_decls` に渡すと**宣言名に slot を振り値がフレームへ消える**
  （初期化子に `collect_expr_decls` を呼ぶ）。
- **#27-a/#26 完了。レシーバ判定を「形＋出自」の 2 段に**。`NamedInstance` は **Arrow クラスと
  外部言語スタブを区別しない**ので、型検査の `arrow_class_names`（外部 import 本体の `ClassDef` を
  除外）で出自を見る。⚠ **健全化でカバレッジが 2 件減った**＝正しい変化。
...
- **#15b / #15-3 は消費者 0 件**（実測・再訪価値なし）。⚠ `Value::Str` を複製する新コードでは
...
1. **#10-d 定義文と import モジュール本体** — 最上位に残るツリーウォーク 663 件の **56%（369 件）**
   がここ。`exec_module` の本体 Chunk 化と、定義文を既存 exec への委譲 op で足りるかの見極め。
2. **#25 `--vm=force` のゲート化** — 小規模。**#3 に進めるかを判定できる唯一の手段**で、
   これが無い限り「残り何件か」は例題コーパスの性質でしかない。
3. **#27-c 最上位の残り bail 511 件** — 小粒の集合。**まず未帰属 46 件の計装を潰す**こと
   （#10-a と同じ手順。潰さないと何が効くか決められない）。
4. **#27 の残り 17 件** — 入れ子 `fn`（クロージャ）8 が最大で architectural。他は小粒。

> ⚠ **`--vm=force` は現状 no-op**（`main.rs` でパースされるだけで `Auto` と区別されていない）。
> 文書の各所にある「`--vm=force` で穴を可視化」は**未実装**。**#25 として昇格**した（#3 のゲート）。
> 現状の穴の可視化は `AR_TW_STATS` が担っている。
...
| 10-d | 定義文のオペコード化・import モジュール本体 | `exec_module` の本体も Chunk 化。定義文は既存 exec への委譲 op で足りるか要検討。**最上位に残るツリーウォーク 663 件の 56%（369 件）がここ** | — | 未着手（**#3 の前提**・最上位で最大） |
| 27 | fn の VM コンパイル失敗の解消（残 **17**） | 内訳: **入れ子 `fn`（クロージャ）8**・`static` 2・可変長 2・未帰属 2・属性代入 2・for ターゲットのシャドウ 1 | — | 進行中（**#3 の前提**・49→17 まで完了） |
| 27-c | 最上位 Chunk 化の残り bail（**511**） | **未帰属 46（まず計装を潰す）**・テンプレート実体化 31・`Slice` 21・虚数リテラル 13・`let x = <識別子>` 13・キーワード/可変長引数 10・`for` タプル分解 7・`freeze` 7・`LetTuple` 6 ほか | — | 未着手（#3 の前提・小粒の集合） |
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← 10-d ＋ 27 ＋ 25**（#1・#10-b・#10-c は完了） | ブロック中（本系列の最終段） |
...
（12b → 2c は循環依存のため両方保留）
10-d ────────┐
27 ─────────┼→ 3    ← 前提は「最上位の全カバー」「関数の失敗 0」（#10-a の実測）と
25 ─────────┘         「それを判定できるゲート（#25）」の 3 つ
