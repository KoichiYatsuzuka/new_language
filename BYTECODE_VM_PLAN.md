...
16. **AST 型解決層 — コンパイル/ツリーウォーク/バイトコードの挙動統一**（#13 を包含・plan A の土台）。**【新タスク・本命】**
    ...
    **AST に焼き込み**、可能な限り低レベルまで解決する: **具象型**・**「実行時型検査の要否」マーカー**・
    **メソッド/フィールドのバイトオフセット**・**呼び出しシグネチャ**まで。解決できた点は決め打ち（直接オフセット／
    直接ディスパッチ・検査なし）、できない点だけ「動的検査ノード」を AST に挿入する。
    - **消費者（三経路が同一の解決済み AST を消費）**: ネイティブ codegen（＝#13 の統一。現状 `context.rs` で独自再導出
      している解決を共有解決へ置換）／ ツリーウォーク（`eval`）／ バイトコード（plan A: 検査省略）。
    - **目的（速度ではない・承知の上）**: (1) **コンパイル時とツリーウォーク時の挙動統一**。(2) **内部処理の条件分岐削減**
      ── バイトコード化・各最適化で発生しがちな「型が確定しているか？／どの経路か？」等の例外的分岐を、AST 段階で
      解決済みにして**極力なくす**（各経路の実装が単純化し、最適化時の場合分け＝例外的処理が減る）。**速度貢献は現段階で
      限定的**（VM の動的経路は R3/メソッド IC が緩衝するため。#3 実測: overloaded 演算は「探索」~2-3% のみ解決可能で、
      残りは呼び出し機構＋オブジェクト確保で不可避。ただし**ネイティブ側は IC が無いぶん共有解決の効果が桁違いに大きい**）。
    - **内省保持（plan A）**: VM の `Value` は boxed のまま（型タグ保持）＝プリミティブもデバッグ等で型読み取り可能。
      unbox（解釈B・~2-3x）は健全性/境界変換の負担が大きいため採らず、検査省略に留める。
    - **動的検査を挿入する箇所**: `mustbe`/ダウンキャスト・他言語ライブラリ FFI 境界・**非コンパイルの Arrow ライブラリ
      境界**（コンパイル済みライブラリは typed ABI 保証のため無検査）。型が `protocol`/`Any`/`Union`/`Unresolved` の点は要検査。
    - **入口コスト（調査済み・#1）**: 型は既に計算済みだが `infer` は `&Expr` を取り結果を**破棄**、`check` はエラーのみ返す
      （書き戻し grep 0 件）。よって第一歩は**「`infer` が出した型を AST 注釈へ永続化する層」の新設**（型推論の再実装は不要＝
      中規模）。注釈形態は埋め込み（`Cell`・R1/R3 流儀）か node-id 別テーブル（§9-1）。
    - **関係**: R1(`LocalRef`)/R3(`AttrCache`)/R4(呼び先) の解決注釈を**型情報まで拡張・一本化**したもの。**#13 を包含**し、
      plan A（VM 検査省略）と #11 resolve-time R2 はこの層の**消費側**に位置づく。**「AST 解決を可能な限り低レベルへ
      押し込む」統一目的の中核タスク**。
    - **段階案**: (a) 型注釈永続化層（型検査器→AST）→ (b) ツリーウォーク/VM が消費して決め打ち（plan A・byte-identical 維持）
      → (c) ネイティブ codegen が消費（#13）。
