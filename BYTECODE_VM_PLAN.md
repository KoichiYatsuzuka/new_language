...
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15b・#15c・#15・#15d・#20・#15e・#21-a・#22-a・#22-b・#22-c・#22-d・#21-b・#12 まで完了（**#22 系列は完了**）。
git の HEAD は `9123595 "#21, 22"`（#15b〜#15e・#20・#21・#22 系列までコミット済）。
**#12（呼び出し高速化）と #12b/#2c 保留の文書更新は未コミット**（ユーザー許可待ち）。
...
1. **#2b V-F 単型算術命令** — #16 完了で解禁済み。算術支配ループ向け。
2. **#23 評価済み引数の struct 化** — `NativeFunction` を C 軸へ寄せる前提（27 箇所・速度効果なし）。
3. **#2a V-F peephole** — 中規模。ジャンプ先再マップが要る。
...
- **検証は 4 点セット**: `cargo build`（**警告 0**）・`cargo test`（**698 緑**）・
...
| 2a | V-F peephole | Jump 除去＋ジャンプ先再マップ | — | 保留（中規模） |
| 2b | V-F 単型算術命令 | 型注釈から専用 op を emit | — | 保留（#16 完了済で解禁） |
