...
git の HEAD は `56b8e66 "#10 (partially)"`（#10-a/#10-b までコミット済）。**#27 は未コミット**。

**→ #3（強制バイトコード）の前提は #10 だけではないと実測で判明**（下記 #10-a）。
最上位（#10-c/#10-d）と**関数の VM コンパイル失敗**（#27）の両方が要る。#27 は **49 → 31** まで進行。

直近の要点だけ先に:
- **#27 で 18 件解消**（`pass`／`break_point`／`undefined`／`obj::Trait.attr`／メソッド本体の `Self`）。
  ⚠ **`break_point` は `vm_debug_pause` を通すこと**。`exec_breakpoint` を直接呼ぶと
  REPL からそのフレームのローカルが見えない（`dbg_vars.ar` が off/auto 不一致で検出）。
  ⚠ **残り 31 のうち 16 件が「レシーバを Instance と証明できない」1 原因**（#26）。ここが最大の梃子。
- **⚠⚠ opcode を足すと、その op を実行しなくても E2E ベンチが 5〜12% 動く**（#27 で確定）。
  **ノイズ床のプローブは変更と同規模でなければ無意味**。#10-b は小関数 1 本で測って
  「このベンチは配置に鈍感」と誤結論した（同規模で測ると同じベンチが 0.884x 動く）。
  → **opcode 追加の可否を E2E ベンチの数 % で判断しないこと**。判断材料は
  「`--vm=off` でも同じ差が出るか」と「同規模プローブとの比較」。
...
- **#2a/#2b**: peephole は JUMP の 14.3% 除去だが総命令の 0.31%（E2E は分岐支配で +4.4%）。
  型特化の穴は op ではなく**文の側**にあった（`x += e` に `binop_kind` が焼かれていなかった・1.9x）。
...
1. **#27-a → #26** — `eval_method_call_evaled` の FFI レシーバ対応を先に済ませれば、
   #26 で **#27 の残り 31 のうち 16 件**が一度に片付く（最大の梃子）。効果も実測済み
   （最上位ツリーウォーク **373x 減**・method_call 1.38x）。順序を逆にすると off/auto 不一致になる。
...
| 27 の一部 | fn の VM コンパイル失敗 **49 → 31** | `pass`(3)／`break_point`(5, `Op::BreakPoint`＋`vm_debug_pause` 委譲)／`undefined`(1)／`obj::Trait.attr`(5, `Get/SetTraitAttr`＋`trait_*_evaled` へ委譲)／メソッド本体の `Self`(4, `Op::LoadSelfClass`)。bail 計上を fn/最上位に分離し未帰属を 6→2 に |
...
| 26 | レシーバを Instance と証明できない件（**#27 の残り 31 のうち 16 件**） | `object_is_instance` を注釈（`NamedInstance`）へ拡張。**先に `eval_method_call_evaled` が FFI レシーバを CallArg 版と同等に扱えるようにすること**（順序を逆にすると off/auto 不一致になる。実装して巻き戻した記録あり） | **← 27-a** | 保留（効果は実測済: 最上位ツリーウォーク **373x 減**・method_call 1.38x） |
...
| 27 | fn の VM コンパイル失敗の解消（残 **31**） | 内訳: **入れ子 `fn`（クロージャ）8**・可変長 2・`static` 2・for ターゲットのシャドウ 1・未帰属 2。残り 16 件は #26 の 1 原因 | — | 進行中（**#3 の前提**・49→31 まで完了） |
