...
git の HEAD は `a1d9c7e "#10-c"`。**#27-b は未コミット**。

**→ #3（強制バイトコード）の前提は #10 だけではないと実測で判明**（下記 #10-a）。
最上位（#10-c2/#10-d）と**関数の VM コンパイル失敗**（#27）の両方が要る。#27 は **49 → 17**、
最上位のツリーウォークは **1123 万 → 2,071 件（5,425x 減）**まで進行。

直近の要点だけ先に:
- **#27 で 20 件解消**（`pass`／`break_point`／`undefined`／`obj::Trait.attr`／`Self`）。
  ⚠ **`break_point` は `vm_debug_pause` を通すこと**（直呼びだと REPL からフレームのローカルが見えない）。
- **#27-b 完了。メソッド dispatcher を 1 本化**（`fn_FAILED` 29→**17**・`toplevel_FAILED` 163→**65**・
  `method-receiver` の bail 110→**0**・実質 **-230 行**）。⚠ **見積もり 644 行は過大**で、
  実際に要ったのは引数の扱い 4 パターンだけだった（着手前に `args` の使われ方を分類したのが効いた）。
  ⚠ **レシーバ制限を外すなら `node_id` を op で運ぶこと**。でないと外部言語メソッドの
  FFI 戻り値検査が VM 経路だけ素通りする（#22-a と同型の穴）。
  ⚠ **最頻路（Instance）に判定を 1 つ足すだけで 3% 動く**。きれいな 1 枚のラッパより、
  `exec_op` の arm で直接分岐する方が実測で速かった。
- **#10-c 完了。最上位の宣言文を Chunk 化**（`Op::DeclareGlobal`・**30,123 → 2,071**）。
  ⚠ **想定と実体が違った**: 93% は **`mut xs = for ...: loop_yield ...` の初期化子が N 回まわる**形で、
  「1 回しか実行されない文は損」で切ると取り逃す。
  ⚠ 宣言文を `collect_nested_decls` に渡すと**宣言名に slot を振り値がフレームへ消える**
  （初期化子に `collect_expr_decls` を呼ぶ）。
- **#27-a/#26 完了。レシーバ判定を「形＋出自」の 2 段に**（最上位ツリーウォーク **120x 減**）。
  `NamedInstance` は **Arrow クラスと外部言語スタブを区別しない**ので、型検査の
  `arrow_class_names`（外部 import 本体の `ClassDef` を除外）で出自を見る。
  ⚠ **健全化でカバレッジが 2 件減った**（従来 unsound にコンパイルしていた分＝正しい変化）。
...
1. **#10-c2 最上位の式文・`if`/`try`** — 残ツリーウォーク 2,071 件のうち `Expr`（式文）909 が最大で
   **まだ試行すらしていない**。`compile_toplevel_stmt` の受理対象を広げるだけで測れる。
2. **#25 `--vm=force` のゲート化** — 小規模。**#3 に進めるかを判定できる唯一の手段**で、
   これが無い限り「残り何件か」は例題コーパスの性質でしかない。
3. **#27-c 最上位の残り bail 65 件** — 小粒の集合（虚数リテラル・テンプレート実体化・
   キーワード引数・`for` タプル分解…）。1 つずつ潰せて `AR_TW_STATS` で進捗が数えられる。
...
| 10-c2 | 最上位の**式文・`if`/`try`** を Chunk 化 | `compile_toplevel_stmt` の受理対象を広げる。残ツリーウォーク 2,071 件のうち `Expr`（式文）909 が最大で**まだ試行すらしていない** | — | 未着手（#3 の前提） |
| 10-d | 定義文のオペコード化・import モジュール本体 | `exec_module` の本体も Chunk 化。定義文は既存 exec への委譲 op で足りるか要検討 | **← 10-c2** | 未着手（#3 の前提） |
| 27 | fn の VM コンパイル失敗の解消（残 **17**） | 内訳: **入れ子 `fn`（クロージャ）8**・`static` 2・可変長 2・未帰属 2・属性代入 2・for ターゲットのシャドウ 1 | — | 進行中（**#3 の前提**・49→17 まで完了） |
| 27-c | 最上位 Chunk 化の残り bail（**65**） | 虚数リテラル 13・`let x = <識別子>` 13（最上位グローバルの可変性が要る）・テンプレート実体化呼び出し 13・キーワード/可変長引数 8・`for` タプル分解 7・未帰属 7 | — | 未着手（#3 の前提・小粒の集合） |
