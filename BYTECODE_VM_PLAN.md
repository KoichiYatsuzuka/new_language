...
**#10-a/-b/-c/-c2・#26・#27-a/#27-b・#25・#27-c** まで完了。git の HEAD は `4660420 "#27-c"`。
**2026-08-15〜16 の #27 続き・#27-c 完了分は未コミット**（`git status` で確認できる）。

**→ #3 に残っているのは #27-d ただ 1 つ**（#10-d は保留）。
最上位のツリーウォークは **1123 万 → 365 件（全て定義文＝設計上の対象外）**、関数本体内は **70 件**。

**`force_gate` は 4/124**（`spider_solitaire`/`functions`/`variable`/`langtons_ant_profile`）で、
**4 件とも `vm_ineligible: closure-capture`＝#27-d**。`vm_bail_toplevel` は **0**。
`vm_bail_fn` の残り 4（`static` 2・可変キャプチャ 2）も #27-d と同じセル表を要求する。
...
- **#27-c 完了（2026-08-16）: `vm_bail_toplevel` 0・`force_gate` 8→4**。載せたもの:
  ...
  **組み込み/メソッドのキーワード引数**（`CallBuiltinKw`/`CallMethodKw`）。
  ⚠ **`force_gate` は例題ごとに最初の 1 件で止まる**。1 例題 = 1 原因ではない
  （5 例題の裏に 7 原因あった）。潰すたびに測り直すこと。
  ⚠⚠ **`let x = <識別子>` は `Resolution` を見てはいけない**。`exec_let` は
  「`Expr::Ident` でありさえすれば」ソース変数の可変性で分岐するので、`Resolution::Global` を
  非識別子式の枝へ落とすと**可変グローバルのコピー＆フリーズが漏れる**（潜在バグ 1 件・#15e の原則）。
  ...
  ⚠ **採番 walker の漏れが再発**（`collect_nested_decls` に `Stmt::FnDef` が無くブロック式内の
  `fn` に slot が振られていなかった）。**`compile_stmt` に文種別を足したら必ず採番側も見る**。
  ⚠ **組み込みのキーワードは名前ごとに扱いが違う**（`enumerate` は `start` だけ許容／`zip` はエラー／
  `len` は名前を無視）。一致を確認した名前だけ `VM_BUILTIN_KW_NAMES` に挙げること。
  ⚠ メソッドは **`CallMethodLocal` 融合の可否を引数コンパイル前に決める**（融合はレシーバを
  push しないので後から一般形へ切り替えられない）。
- **#27 続き完了（2026-08-15）: `fn_FAILED` 11→4・`force_gate` 12→8**。載せたもの:
  ...
  **for ターゲットのシャドウ**・**可変長引数**（`local::args`）。
  ⚠ **`force_gate` の文言だけで #27 と #27-d を区別するな**（どちらも "cannot compile function 'X'"）。
  帰属は `AR_TW_STATS` の `vm_bail_fn` と `vm_ineligible` を突き合わせる（計画書の見積もりが実測で覆った）。
  ⚠ **bail を潰しても例題が落ち続ける組み合わせがある**（`functions.ar`/`variable.ar` は bail と
  クロージャ本体の**両方**を持つ）。優先度は「その例題が落ちる理由が全部消えるか」で決めること。
...
- **⚠ #27 の残り 4 bail は #27-d と同じ土台を要求する**（`static` 2・可変キャプチャ 2）。
  どちらも `Rc<RefCell<Value>>` の共有セルを束縛するので、**slot と並行するセル表＋
  `LoadCell`/`StoreCell`** が要る。⇒ **分けて着手する意味がない。#27 は #27-d に合流させる。**
- **#27 前半**: `pass`/`break_point`/`undefined`/`obj::Trait.attr`/`Self` で 20 件解消
  （⚠ `break_point` は **`vm_debug_pause` 経由**。直呼びだと REPL からローカルが見えない）＋
  **クロージャの外側関数を VM 化**（`Op::MakeFn`＋`Chunk.fn_defs`。**自由変数 ∩ 外側 slot が
  全て不変**のときだけ載せ値を複製する）。⚠ **`capture_env` と自由変数の定義がずれると閉包変数が黙って消える**。
- **#25 完了。`--vm=force` が実際のゲート**（[force_gate.ps1](force_gate.ps1)・36→**4**/124）。
...
- **#27-c 前半**（`toplevel_FAILED` 175→5）。載せたもの: スライス式・`freeze`・`on`/`once`/`off`・
  `let a,b = t`・テンプレート実体化・**最上位の未解決識別子**（`Op::LoadName`）・
  `d[k] += v`・`obj.attr = v`（レシーバ無制限）・`for _ in ...`・`open`/`close`。
  ⚠ **最優先で見るのは `force_gate` であって bail の件数ではない**
  （**1 文でも載らなければその例題は落ちる**）。
  ⚠ **コンパイラの「この形のときだけ載せる」条件は必ず理由を確かめてから外す**（2 実装の差／
  最適化の前提／本当の非対応の 3 通りがあり、外し方が違う。詳細は実装ログ）。
  ⚠ **テンプレート実体化は引数を未評価のまま持ち回る**（`TemplateArgs`）。先に評価すると
  制約検証より前に副作用が出る。
  ⚠ **VM コンパイラにシャドウ減算は不要**（文を 1 つずつ見るので束縛は全部 `slots` に入る。
  **`slots` を先に引く順序が健全性そのもの**）。
  ⚠ **「対象外」と「失敗」を同じ `None` で表すな**（意図的スキップ 336 件が失敗に化けていた）。
  ⚠ **内訳の文種別を「最上位文の種別」と読み違えない**（`If` 19 は bail した親文に入れ子の `If`）。
- **#10-d は保留**（**計測で両半分とも #3 に寄与しないと判明**: モジュール本体は 20 文・定義文 350 件は
  **制御フローも TLS も持たない**＝設計上インタプリタが実行するものとして `force_gate` からも除外）。
...
- **#27-a/#26 完了 → その後 #27 で撤去**（レシーバの「形＋出自」判定は消費者を失った。上記参照）。
...
- ⚠ **保留理由は経年で腐る**。#10 の保留判断（2026-07-27）は 2 点が実測で覆った（#10-a）。
...
- **#22 系列**: 同じ判断をする 2 実装は**片方を他方への委譲にして畳む**（22-b/22-c）。畳めないもの
...
1. **#27-d クロージャ本体の VM 化**（`vm_ineligible` **20**・`force_gate` の **4 件全部**）— architectural。
   **これが #3 への最後の一歩**。**#27 の残り 4 bail（`static` 2・可変キャプチャ 2）もここに合流する**
   （同じセル表が要る）。⚠ 件数ではなく [force_gate.ps1](force_gate.ps1)（現在 **4/124**）で測る。

> **⚠ 速度目的の残タスクは枯れた**（2026-08-14）。#12（2.61x）・#2b（1.6x）・#2a（+4%）・#1-x（1.06〜1.32x）・
> #10-b（1.11〜1.28x）・#26（method_call 1.45x）で取り切った。以降は #3 に向けたカバレッジの話。
> 新しく速度課題を立てるなら**まず計測して支配項を出すこと**。
> ⚠ #1-x の教訓: **`#[inline]` は効いているとは限らない**（巨大関数は LLVM が却下する）。
>
> **⚠ #12b / #2c は保留**（循環依存。2c の目的は VM 経路が既にフラットなので達成済み・速度理由も #12 で消えた）。
> **⚠ #19 / #17-a / #17-b（外部接続系）は優先度低**（本系列と独立。実行系が一段落してから別レーンで扱う）。
...
- **高リスク低リターンと判断したらスキップして保留にし、理由を記録する**（勝手に大改造しない）。
  ...
  リターンには実行速度だけでなくコード・ロジックの簡潔さも含む。
  ⚠ **タスクの定義が目的に対して過剰なら定義を見直す**（#27-a は 644 行の畳み込みと定義されていたが、
  目的は 50 行で足りた。#27-c の `try/except/finally` も新実装ではなく入れ子で足りた）。
...
| 27 | fn の VM コンパイル失敗の解消（残 **4**） | 残り: `static` 2・**可変キャプチャの入れ子 `fn` 2**。⚠ **どちらも共有セル束縛なので #27-d と同じセル表を要求する**（分けて着手する意味がない）。⚠ 解消済みの手: 未解決 Ident=`LoadGlobal`（`LoadName` は `frame_floor` の関係で関数本体では使えない）／属性複合代入=融合を使わない 2 回評価経路／for シャドウ=本体の間だけ slot 差し替え／可変長=`local::args` を末尾 slot へ | **← 27-d と同時**（合流） | 進行中（**#3 の前提**・49→**4**。⚠ **残り 4 は `force_gate` を減らさない**＝該当例題はクロージャ本体も抱えている） |
| 27-d | **クロージャ本体**の VM 化（`vm_ineligible` **20**・`force_gate` の **4 件全部**） | キャプチャ変数を slot へ束縛して実行。⚠ **可変キャプチャはフレーム表現の変更が必須**（slot と並行するセル表＋`LoadCell`/`StoreCell`）＝**#27 の残り 4 bail と同じ土台**。⚠ クロージャ実体ごとに `FnValue` が別物で **Chunk を使い回せない**（ループ内生成で毎回コンパイル） | — | 未着手（**#3 の前提**・architectural・**#27 の残りを含む**） |
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← `force_gate.ps1` が 0 件**（現在 **4/124**・**全て #27-d**） | ブロック中（本系列の最終段・**残るブロッカーは #27-d だけ**） |
...
```
27（残り 4 bail・同じセル表を要求）→ 27-d → 3   ← #3 をブロックするのは **#27-d だけ**（27-c 完了）
モジュール間ネイティブ直リンク（未計画）→ 14 → 11 R2-c ／ 12b → 2c は循環依存で両方保留
判定は **`force_gate.ps1` が 0 件**（現在 4/124・全て #27-d）。定義文は対象外（#10-d 保留）
```
