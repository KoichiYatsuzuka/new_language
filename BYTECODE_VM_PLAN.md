...
2. **#27-d クロージャ本体の VM 化**（20 件）／**#27 の残り 11 件** — いずれも architectural。
   ⚠ 進捗は [force_gate.ps1](force_gate.ps1)（現在 **36/125**）で測る。
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← `force_gate.ps1` が 0 件**（現在 **36/125**。内訳は #27-c／#27／#27-d） | ブロック中（本系列の最終段・**進捗が測れるようになった**） |
...
27-d ────────┘      判定は **#25 の `force_gate.ps1` が 0 件になること**（完了・現在 36/125）
