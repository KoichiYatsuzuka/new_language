...
## 🚩 次スレッドへの引き継ぎ（2026-08-14・**ここだけ読めば再開できる**）

### 現在地
**#16（AST 型解決層）＝本系列の主目的は完了**。三経路（ツリーウォーク／VM／ネイティブ）が同一の型解決注釈を消費する。
続けて #18・#11 R2-a/R2-a′/R2-b・#14 の一部・#15 系列・#20・#21・#22 系列・#12・#2a/#2b・#1・
**#10-a/-b/-c/-c2・#26・#27/#27-a/#27-b** まで完了。git の HEAD は `27ed68e "replan"`。**#27-c 以降は未コミット**。

**→ #3 の前提は「bail の解消（最上位 #27-c ／ 関数 #27・#27-d）」＋「判定ゲート（#25）」**（#10-d は保留）。
最上位のツリーウォークは **1123 万 → 382 件**（うち定義文 347＝設計上の対象外）、関数本体内は **111 件**。

直近の要点だけ先に:
- **#27 で 20 件解消**（`pass`/`break_point`/`undefined`/`obj::Trait.attr`/`Self`）。⚠ `break_point` は **`vm_debug_pause` 経由**（直呼びだと REPL からローカルが見えない）。
- **#27 クロージャ: 外側関数を VM 化**（`decl-prepass:FnDef` 8→**0**・`fn_FAILED` 17→**11**）。
  `Op::MakeFn`＋`Chunk.fn_defs`。**自由変数 ∩ 外側 slot が全て不変**のときだけ載せ値を複製する。
  ⚠ **可変キャプチャは表現の変更が必須**（`capture_env` は `Var::Cell` へ昇格して外側と**セルを共有**するが、
  VM のフラット slot は `Value` 直値）。⚠ **`capture_env` と自由変数の定義がずれると閉包変数が黙って消える**。
  ⚠ **クロージャ本体（`vm_ineligible` 20）は未着手＝#27-d**。実体ごとに `FnValue` が別物で
  **Chunk を使い回せない**（ループ内生成で毎回コンパイル）。
- **#25 完了。`--vm=force` が実際のゲートになった**（[force_gate.ps1](force_gate.ps1)）。
  初回計測は **125 例題中 36 件がまだフォールバック**。**これが #3 へ進めるかの唯一の判定基準**。
...
- **⚠ #28（`Op::Rare` 畳み込み）は却下**。「op を足すと ~1〜1.5% 落ちる」を前提に 7 op を 1 アームへ
  畳んだが**何も回復せず** 1 件は悪化した。⇒ **効いているのはアーム数ではなくコード配置**で、
  この規模の変更は**どちら向きにも ±5% 揺れる**。数 % を根拠に設計を決めないこと。
- **#27-c 進行中**（`toplevel_FAILED` 175→**5**・`force_gate` **36→12/125**）。
  載せたもの: スライス式・`freeze`・`on`/`once`/`off`・`let a,b = t`・テンプレート実体化 `Tmpl[T](...)`・
  **最上位の未解決識別子**（組み込み型名 `Signal`/`dict` 等）・キーワード/可変長引数・
  添字への複合代入 `d[k] += v`・`obj.attr = v`（レシーバ無制限）・`for _ in ...`・`open`/`close`。
  ⚠ **最優先で見るのは `force_gate` であって bail の件数ではない**（#25 以降、制御フロー文かどうかで
  絞る意味は無い。**1 文でも載らなければその例題は落ちる**）。
  ⚠ **コンパイラのレシーバ制限は「2 実装の差」を隠していただけだった**: `attr_assign`（ツリーウォーク）と
  `attr_assign_evaled`（VM）で `Value::Class` の扱いが違い、`object_is_instance` でそこを避けていた。
  **1 実装に畳めば制限ごと外せる**（#22 の原則）。他の制限も同じ疑いで見ること。
  ⚠ **未解決識別子は `Op::LoadName` でそのまま載る**（ツリーウォークの `Unresolved` は `get_val` と
  **エラー文言まで同一**）。ただし**最上位限定**: スコープ隔離は `frame_floor` が担うが VM フレームは
  それを進めないので、関数本体で使うと**呼び出し元のローカルが見えてしまう**。
  ⚠ **テンプレート実体化は引数を未評価のまま持ち回る**（`TemplateArgs`）。先に評価すると
  制約検証より前に副作用が出る。
  ⚠ **VM コンパイラにシャドウ減算は不要**（文を 1 つずつ見るので束縛は全部 `slots` に入る。
  **`slots` を先に引く順序が健全性そのもの**）。⚠ **同じ木を歩く walker が 2 つあると必ずずれる**
  （`collect_nested_decls` が `Stmt::Block` を降りていなかった／`Stmt::LetTuple` を見ていなかった。
  後者は**宣言文の除外リストと対で直す**必要があり、片方だけだとループ 2 周目で「already declared」、
  もう片方だけだと最上位の値がフレームへ消える — 実測で両方踏んだ）。
  ⚠ **計測の穴を 3 つ塞いだ**: 意図的スキップ 336 件が失敗に化けていた（**「対象外」と「失敗」を同じ
  `None` で表すな**）／未帰属 46→3／bail を `<文種別>/<理由>` で切れるように。
  ⚠ **内訳の文種別を「最上位文の種別」と読み違えない**（`If` 19 は bail した親文に入れ子の `If`）。
- **#10-d は保留**（**計測で両半分とも #3 に寄与しないと判明**: モジュール本体は 20 文・定義文 350 件は
  **制御フローも TLS も持たない**）。⚠ **#25 を作るときは定義文の扱いを決めること**
  （「bail 0」だと永久に引っかかる。**設計上インタプリタが実行する**と明示除外するのが素直）。
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（受理判定を**許可リスト→定義文の除外リスト**
  へ反転。⚠ **許可リストは新しい文種別を黙って取りこぼす**）。⚠ **想定と実体が違った**: 93% は
  **`mut xs = for ...: loop_yield ...` の初期化子が N 回まわる**形。
  ⚠ 宣言文を `collect_nested_decls` に渡すと**宣言名に slot を振り値がフレームへ消える**。
- **#27-b 完了。メソッド dispatcher を 1 本化**（実質 **-230 行**）。⚠ **見積もり 644 行は過大**で、
  実際に要ったのは引数の扱い 4 パターンだけ。⚠ **レシーバ制限を外すなら `node_id`＋表示名/位置
  （`ffi_call_info`）を運ぶこと**（でないと FFI 戻り値検査が VM だけ素通り）。
  ⚠ **最頻路に判定を 1 つ足すだけで 3% 動く**（ラッパより arm で直接分岐）。
- **#27-a/#26 完了。レシーバ判定を「形＋出自」の 2 段に**（`NamedInstance` は Arrow クラスと外部言語スタブを区別しないので `arrow_class_names` で出自を見る）。⚠ **健全化で 2 件減った**＝正しい変化。
- **⚠⚠ この規模の変更は VM 支配ベンチを ±5% 揺らす**（#28 で確定・**以前の「op 1 個あたり
  ~1〜1.5%」というモデルは誤り**）。稀な op 7 個を 1 アームに畳んでも**何も回復しなかった**ので、
  効いているのは**アーム数ではなくコード配置**。実行されないダミーでも同じ差が出る。
...
- **#10-b で最上位ループを Chunk 化**（E2E 1.11〜1.28x）。欠けていた基本要素は記録どおり
  **`StoreGlobal` op ただ 1 つ**だった（読み側は #21-b で既に揃っていた）。
...
1. **#27 の残り 11 件**（`force_gate` 12 件中 **7 件がここ**）— `static` 2・可変長引数 2・
   可変キャプチャの入れ子 `fn` 2・識別子 2・属性複合代入 1・for ターゲットのシャドウ 1。
2. **#27-c の残り 5 件**: `try/except/finally` 1・`Try` 内の `let v = <未定義識別子>` 1・
   メソッドのキーワード引数 1・`block:` 式の中の入れ子 `fn` 1・`callee-expr:Block` 1。
   ⚠ 件数ではなく [force_gate.ps1](force_gate.ps1)（現在 **12/125**）で測る。
3. **#27-d クロージャ本体の VM 化**（20 件）— architectural。
...
> **⚠ #19 / #17-a / #17-b（外部接続系）は優先度を下げる**（本系列とは独立した外部言語境界の話題。
> 実行系が一段落してから別レーンとしてまとめて扱う）。

### 作業の進め方（この系列で有効だった型）
- **推測せず先に計測する**。見積もりは本系列で何度も外れた: #12（支配項は「フレーム構築」ではなく
  **per-call のヒープ確保 2 件**・設計変更ゼロで 2.61x）／c-2「効果あり」→実質デッドコード／
  c-3「効果小」→ 1.73x／(b)(i)「IC 削減」→真因は `Value` clone／
  **#10-c「宣言文は 1 回しか実行されないから損」→実体は初期化子のループ式で 93%**。
...
- **高リスク低リターンと判断したらスキップして保留にし、理由を記録する**（勝手に大改造しない）。
  ...
  目的（#26 を安全にする）は 50 行の「出自の証明」で足りた）。
...
| 27 | fn の VM コンパイル失敗の解消（残 **11**） | `static` 2・可変長 2・**可変キャプチャの入れ子 `fn` 2**・属性代入 2・識別子 2・for ターゲットのシャドウ 1 | — | 進行中（**#3 の前提**・49→11 まで完了） |
| 27-d | **クロージャ本体**の VM 化（`vm_ineligible` **20**） | キャプチャ変数を slot へ束縛して実行。⚠ **可変キャプチャはフレーム表現の変更が必須**（slot と並行するセル表＋`LoadCell`/`StoreCell`）。⚠ クロージャ実体ごとに `FnValue` が別物で **Chunk を使い回せない**（ループ内生成で毎回コンパイル） | — | 未着手（**#3 の前提**・architectural） |
| 28 | `Op::Rare` への畳み込み | 稀な 7 op を 1 アームに集約 | — | **却下**（実装して A/B した結果**何も回復せず** 1 件は悪化。**前提だった「op 1 個あたり ~1〜1.5%」というモデル自体が誤り**だった。詳細は実装ログ） |
| 27-c | 最上位 Chunk 化の残り bail（**5**） | 残り: `try/except/finally` 1・`Try` 内の `let v = <未定義識別子>` 1・メソッドのキーワード引数 1・`block:` 式の中の入れ子 `fn` 1・`callee-expr:Block` 1。⚠ **`force_gate` で測る**（bail 件数ではない） | — | 進行中（175→**5**・`force_gate` 36→**12**） |
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← `force_gate.ps1` が 0 件**（現在 **12/125**。内訳は #27 が 7・#27-c が 5） | ブロック中（本系列の最終段・**進捗が測れるようになった**） |
...
（12b → 2c は循環依存のため両方保留）
27-c ────────┐      ← #3 をブロックするのは **bail**（TLS を使う制御フローを含む）。
27 ─────────┼→ 3      定義文は制御フローも TLS も持たないので #10-d は保留。
27-d ────────┘      判定は **#25 の `force_gate.ps1` が 0 件になること**（完了・現在 12/125）
（#22 系列・#2 系列・#1・#10-a/#10-b/#10-c・#27-a/#26 は完了）
```
