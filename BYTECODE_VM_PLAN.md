...
git の HEAD は `b1d75aa "#1"`。**#10-a/#10-b は未コミット**（ユーザー許可待ち）。

**→ #3（強制バイトコード）の前提は #10 だけではないと実測で判明**（下記 #10-a）。
最上位（#10-c）と**関数の VM コンパイル失敗 49 件**（#27）の両方が要る。

直近の要点だけ先に:
- **#10-a で「#3 の前提は #10 のみ」が誤りと判明**。診断フック `AR_TW_STATS` で実測すると、
  ツリーウォークの負荷は**実質すべて最上位ループ本体**（1123 万回中 99.8%）で関数側は 204 回まで枯れている。
  一方で **fn の VM コンパイル失敗が 49 件**残っており、フォールバックは撤去できない。
...
- **#10-b で最上位ループを Chunk 化**（最上位ツリーウォーク **3.09x 減**・E2E 1.11〜1.28x）。
  ...
  詳細は実装ログ。**いずれも実測でしか見つからない種類**なので、この周辺を触るときは必ず A/B すること。
  ⚠ **ノイズと退行の切り分けは `AR_TW_STATS` で「触っているか」を先に見る**。
  3 件の「退行」が top-level chunk 0 件＝無関係だった。
- **#1 完了。デバッグ中も VM を使う**（`vm/run.rs` の `run_stepping` が文境界で停止判定）。
  ⚠ 着手前に**既存バグを発見**した: VM フレームへ step-out すると**停止せず走り抜けていた**
  （`--vm=off` とは不一致）。「暫定対応が正しく見えていた」のは**一度も検証されていなかった**から。
  ⚠ **`exec_op` に `#[inline(always)]` が要る**。呼び出し元が 2 つになると `#[inline]` では
  展開されず 3〜5% 退行する。付けたら**元より 1.06〜1.32x 速くなった**（元から展開されていなかった）。
...
- **#2a で「V-F は速度より基盤」と確定**。peephole は静的には **JUMP の 14.3% を除去**するが
  **総命令数では 0.31%** で、E2E は**分岐支配コードで +4.4%・それ以外はほぼ 0**。
  ...
  追加時は `code_target_mut` に足すこと（唯一の窓口・テストで固定済み）。
- **#2b で「特化の穴は op ではなく文の側にあった」**。型特化 op は #16 で出揃っていたのに
  **`x += e` だけが型検査で `binop_kind` を焼かれておらず**汎用 `Bin` に落ちていた（`x = x + e` の 1.9x 遅い）。
  ⚠ **`Expr` 側に注釈を付けたら、同じ演算を表す `Stmt` 側も見ること**（複合代入・属性複合代入）。
- **#15b / #15-3 は消費者 0 件**（実測）。速度目的で再訪する価値は無い。
  ⚠ `Value::Str` を複製する新コードでは **`deep_clone` だけは独立バッファ必須**（async の share-nothing）。
- **#22 系列で「重複への対処は 2 通りしかない」と確定**: 同じ判断をする 2 実装は
  **片方を他方への委譲にして畳む**（22-b/22-c）。畳めないもの（コンパイル時 vs 実行時）は
  **不変条件をテストで固定する**（22-d の `VM_BUILTIN_NAMES`）。
  ⚠ **`*_evaled` 版とずれた実装を作らないこと** — 実バグを **4 回**出した（#22-a/-b/-c・#10-b′ の `CsObject`）。
- **#15e で原則が確定**: **`res` は「どの記憶域から読むか」の最適化ヒントであって、
  「それが何を意味するか」の根拠にしてはいけない**。破ると「同じコードが書かれた場所で挙動を変える」
  （実バグ 2 件を検出・修正: `mut→let` のコピー漏れ・FFI OutPtr の書き戻し漏れ）。
- **#15d で「`res` を見れば済む」という当初想定が否定された**。`Resolution::Unresolved` は
  **「シャドウが無い」を意味しない**（リゾルバはトップレベル関数の本体しか処理せず、
  **モジュール最上位・テンプレート本体・合成 AST は常に `Unresolved`**）。
  呼び先や組み込みの健全性を判定するときは `res` ではなく**実際の束縛**を見ること。
...
1. **#27 fn の VM コンパイル失敗 49 件の解消** — **#3 の前提**で、内訳は #10-a で実測済み
   （入れ子 `fn` 8・`BreakPoint` 5・`Pass` 3・`TraitAccess` 3・可変長 2・`Static` 2・未帰属 6 ほか）。
   `Pass` のように 1 行で済むものから片付き、`AR_TW_STATS` で進捗を数えられる。
2. **#10-c 最上位の残りを Chunk 化** — もう片方の #3 前提。`DeclareGlobal` op を足して
   `let`/`Assign`/`if`/`try`/式文を覆う。#10-b の `StoreGlobal` と同じ形で足せる。
3. **#27-a → #26** — `eval_method_call_evaled` の FFI レシーバ対応を先にやれば、
   #26（グローバル受信者）の効果は実測済み（最上位ツリーウォーク **373x 減**・method_call 1.38x）。

> **⚠ 速度目的の残タスクは「関数内」では枯れたが「最上位」には残っていた**（2026-08-13 更新）。
> #12（2.61x）・#2b（1.6x）・#2a（+4%）・#1-x（1.06〜1.32x）に加え、**#10-b で最上位ループが 1.11〜1.28x**。
> まだ #26（実測済み）と #10-c が残る。新しく速度課題を立てるなら**まず計測して支配項を出すこと**。
> ⚠ #1-x の教訓: **`#[inline]` は効いているとは限らない**。巨大関数は LLVM が却下する。
> ホットループの関数はインライン展開されているかを疑うこと（実測 1.3x が眠っていた）。

> **⚠ #12b / #2c は保留にした**（2026-08-12）。`12b → 2c` は**循環依存**で、
> 2c は 12b が作るヒープ確保コストを取り消すためだけのタスク。VM 経路は既に `scopes` を使わず
> フラットなので 2c の目的は達成済み。速度理由も #12（2.61x）で消えた。
> 再訪の条件は「残 0.138 µs/call を分解してフレーム構築が支配的と出たとき」か
> 「`Var::Cell` 昇格起因のクロージャバグが実際に出たとき」。根拠は実装ログ。
...
- **退行を疑ったら「そもそも触っているか」を先に見る**（#10-b）。`AR_TW_STATS` で
  top-level chunk 0 件と分かれば無関係＝ノイズ。これを飛ばして 3 件を誤帰属した。
  レイアウトを疑うときは **HEAD 自体に無意味な関数を 1 つ足してノイズ床を測る**（実測 約 1%）。
...
| 26 | グローバル受信者のメソッド呼び出し | `object_is_instance` を注釈（`NamedInstance`）へ拡張。**先に `eval_method_call_evaled` が FFI レシーバを CallArg 版と同等に扱えるようにすること**（順序を逆にすると off/auto 不一致になる。実装して巻き戻した記録あり） | **← 27-a** | 保留（効果は実測済: 最上位ツリーウォーク **373x 減**・method_call 1.38x） |
| 27-a | `eval_method_call_evaled` の FFI レシーバ対応 | `CsObject`/`PyObject`/`JsProcFn` 等を CallArg 版と 1 実装に畳む（#22 系列の型）。**畳むと広範な退行が出たので性能設計込み** | — | 未着手（#26 の前提） |
| 27 | fn の VM コンパイル失敗 49 件の解消 | 実測内訳: 入れ子 `fn` 8・`BreakPoint` 5・`Pass` 3・`TraitAccess` 3・可変長 2・`Static` 2・未帰属 6 ほか。`Pass` は 1 行で済む | — | 未着手（**#3 の前提**・#10 と独立） |
...
27-a → 26           ← 26 単独で入れると off/auto 不一致になる（実装して巻き戻し済）
