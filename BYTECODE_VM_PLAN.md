...
git の HEAD は `4660420 "#27-c"` で、**2026-08-15〜16 の分は未コミット**（`git status` で確認）。

**→ #3 に残っているのは #27-d 段階 2b（可変キャプチャ）ただ 1 つ**（#10-d は保留）。
最上位のツリーウォークは **1123 万 → 365 件（全て定義文＝設計上の対象外）**、関数本体内は **50 件**。
**`force_gate` は 2/122**（`functions.ar` の `make_counter` / `variable.ar` の `make_id_generator`）で、
`vm_bail_toplevel` は **0**、`vm_bail_fn` 4 件と `vm_ineligible` 13 件は
**すべて「可変キャプチャ」1 種**（`Rc<RefCell<Value>>` の共有）に収束した。
...
- **#27-d 段階 1・2a 完了（2026-08-16）: `force_gate` 4→2・`in_fn` 70→50**。
  ...
  **4 例題中 2 つは可変キャプチャを持っていなかった**。architectural なのは可変キャプチャだけ。
  - **段階 1（不変キャプチャ）**: `compile_fn` に `captures` を渡し、**全宣言の採番後・末尾に**
    slot を採る（`Resolution::Local` の並びを壊さないため）。`Chunk.captured_slots` を
    呼び出し側が**名前で引いて**書き込む（`captured_env` は HashMap なので反復順に依存させない）。
    ⚠ 値は `clone` だけでよい（`Immutable` は生成時に deep_copy 済みのスナップショット）。
    ⚠ **キャプチャ名が既存 slot とぶつかったら bail**（黙って上書きすると閉包変数が消える）。
  - **段階 2a（`static`）**: 記憶域は元から**フレームではなく `Interpreter::static_cells`**
    （宣言位置がキー）なので、**セル表を作らずに** `LoadStatic`/`StoreStatic` で載る。
    初期化子は `StaticInit(span, after)` で「セルがあれば飛び越す」形にする（`exec_static_var` と同じ）。
    ⚠ `static` 名は **slot を持たない**。`slots` を引く経路すべてで**先に `statics` を見る**こと。
- **#27-c 完了（2026-08-16）: `vm_bail_toplevel` 0・`force_gate` 8→4**。載せたもの:
  flat リスト組み込み（1 実装化）・`let x = <グローバル識別子>`（`StoreLocalFromIdent`）・
  **`try/except/finally`**・ブロック式内の入れ子 `fn`・一般の呼び先式・
  **組み込み/メソッドのキーワード引数**（`CallBuiltinKw`/`CallMethodKw`）。
  ...
  成立の根拠は `run` が例外時に `handlers.pop()` してから landing pad へ跳ぶこと。
...
- **#27 続き完了（2026-08-15）: `fn_FAILED` 11→4・`force_gate` 12→8**。載せたもの:
  **関数本体の未解決 Ident**（`Op::LoadGlobal`）・**属性複合代入のレシーバ無制限**・
  **for ターゲットのシャドウ**・**可変長引数**（`local::args`）。
  ⚠ **`force_gate` の文言だけで bail と `vm_ineligible` を区別するな**（どちらも
  "cannot compile function 'X'"）。帰属は `AR_TW_STATS` の両表を突き合わせる。
  ⚠ **bail を潰しても例題が落ち続ける組み合わせがある**。優先度は
  「その例題が落ちる理由が**全部**消えるか」で決めること。
  ⚠ **`LoadName` は関数本体で使えないが `LoadGlobal` なら使える**（`scopes[0]` だけを見るため）。
  健全性の根拠は「**`slots` に無い＝この関数のローカルではない**」がコンパイル時に確定していること。
  ⚠ **レシーバ制限の理由は 2 通りある**: `attr_assign` は「2 実装の差」、属性複合代入は
  **`GetAttrLocal` 融合（レシーバ 1 回評価）の前提**。後者を外す手は**融合を使わない
  一般経路（object を 2 回評価）を足す**こと。
  ⚠ 副次効果で **#26/#27-a の「形＋出自」判定が dead code になり削除**（約 90 行）。
  **`Value::Instance` 前提の op を新設するなら復活させること**。
- **⚠⚠ `for-target-shadow` は「VM の表現力不足」ではなく「リゾルバの実バグ」だった**。
  外側に同名の base 宣言があると**ループ本体の読みが外側の値を指していた**
  （Python 実装 **6100** に対し Rust **400100**）。`resolve_function` が `base` から
  入れ子束縛を差し引いていなかった（グローバル側は既に差し引いていたので**同じ関数を共有**）。
  ⚠ **`compare_vm_modes.ps1` はこの種のバグを検知できない**（両モードともツリーウォークに
  落ちるため一致してしまう）。**bail する形はツリーウォークが正しいとは限らない**。
...
- **#25 完了。`--vm=force` が実際のゲート**（[force_gate.ps1](force_gate.ps1)・36→**2**/122）。
  ⚠ **`vm_eligible` が偽（クロージャ等）も失敗として数える**こと。見逃すと「bail 0 なのに
  ツリーウォークが残る」穴になる。⚠ **定義文は対象外**（#10-d の判断。含めると永久に 0 にならない）。
  ⚠ 役割分担: **force_gate = 止めて判定／AR_TW_STATS = 数えるだけ**。件数は後者で見ること。
- **#27-c 前半**（`toplevel_FAILED` 175→5）。スライス式・`freeze`・`on`/`once`/`off`・`let a,b = t`・
  テンプレート実体化・**最上位の未解決識別子**（`Op::LoadName`）・`d[k] += v`・`obj.attr = v`・
  `for _ in ...`・`open`/`close`。
  ⚠ **最優先で見るのは `force_gate`**（1 文でも載らなければその例題は落ちる）。
  ...
  ⚠ **VM コンパイラにシャドウ減算は不要**（束縛は全部 `slots` に入る。
  **`slots` を先に引く順序が健全性そのもの**）。
  ⚠ **「対象外」と「失敗」を同じ `None` で表すな**（意図的スキップ 336 件が失敗に化けていた）。
- **#10-d は保留**（**計測で両半分とも #3 に寄与しないと判明**＝設計上インタプリタが実行するものとして
  `force_gate` からも除外）。
- **#10-c/#10-c2 完了。最上位の宣言文・残り文を Chunk 化**（受理判定を**許可リスト→定義文の除外リスト**
  へ反転。⚠ **許可リストは新しい文種別を黙って取りこぼす**）。
  ⚠ 宣言文を `collect_nested_decls` に渡すと**宣言名に slot を振り値がフレームへ消える**。
- **#27-b 完了。メソッド dispatcher を 1 本化**（実質 **-230 行**。見積もり 644 行は過大だった）。
  ⚠ **レシーバ制限を外すなら `node_id`＋表示名/位置（`ffi_call_info`）を運ぶこと**（でないと
  FFI 戻り値検査が VM だけ素通り）。⚠ **最頻路に判定を 1 つ足すだけで 3% 動く**。
- **⚠⚠ この規模の変更は VM 支配ベンチを ±5% 揺らす**（**#28 は却下**。稀な op 7 個を 1 アームへ
  畳んでも**何も回復せず** 1 件は悪化した ⇒ 効いているのは**アーム数ではなくコード配置**で、
  「op 1 個あたり ~1〜1.5%」というモデル自体が誤りだった。実行されないダミーでも同じ差が出る）。
  → **数 % の差で良し悪しを判断しない**。判断材料は「`--vm=off` でも同じ差が出るか」と
  「**変更と同規模**のプローブとの比較」の 2 つだけ（#10-b の小関数 1 本プローブは規模不足で誤結論した）。
- ⚠ **保留理由は経年で腐る**（#10 の保留判断は 2 点が実測で覆った）。
- **#10-b で最上位ループを Chunk 化**（E2E 1.11〜1.28x）。欠けていた基本要素は
  **`StoreGlobal` op ただ 1 つ**だった。⚠ **性能の罠を 4 件踏んだ**（索引キャッシュ／診断フック 11%／
  `exec_op` の展開／関数内ループの巻き添え）。この周辺を触るときは必ず A/B すること。
- **#1 完了。デバッグ中も VM を使う**（`run_stepping`）。⚠ **`exec_op` は `#[inline(always)]` が必須**
  （元から展開されておらず 1.06〜1.32x 損していた）。⚠ デバッガ／`run.rs` を触ったら
...
- **#2a/#2b**: peephole は総命令の 0.31%。型特化の穴は op ではなく**文の側**にあった
  ...
  `StaticInit` の after）。追加時は `code_target_mut` へ（唯一の窓口・テストで固定済み）。
...
- **#22 系列**: 同じ判断をする 2 実装は**片方を他方への委譲にして畳む**。畳めないもの
  （コンパイル時 vs 実行時）は**不変条件をテストで固定する**（22-d）。
...
- **#15e/#15d の原則**: **注釈（`res`・型）は最適化ヒントであって意味論の根拠ではない**。破ると
  ...
  意味しない**（最上位・テンプレート本体・合成 AST は常に `Unresolved`）。判定は**実際の束縛／出自**を見る。

### 統一の到達度（この系列の本来の目的）
**型の解決**（#16 c-3）・**ローカルの解決**（#11 R2-a/R2-a′）は三経路とも到達済み。
**グローバルの解決**（#11 R2-b）はツリーウォーク／VM が共有（ネイティブは参照自体がほぼ無く保留）。

### 次にやる候補
1. **#27-d 段階 2b: 可変キャプチャのセル共有**（`force_gate` の **残り 2 件**）— **#3 への最後の一歩**。
   必要なのは 2 つ:
   ...
   `Mutable` キャプチャとして渡すだけ**で済む（外側フレームの変更は不要）。
   ⚠ **`freeze` との相互作用**: 「`Cell` 変数は freeze できない」規則があるので、
   セル化すると決めた名前に `freeze` があれば bail すること。
   ⚠ 件数ではなく [force_gate.ps1](force_gate.ps1)（現在 **2/122**）で測る。

> **⚠ 速度目的の残タスクは枯れた**（2026-08-14）。#12（2.61x）・#2b（1.6x）・#2a（+4%）・#1-x・
> #10-b・#26（method_call 1.45x）で取り切った。以降は #3 に向けたカバレッジの話。
...
### 作業の進め方（この系列で有効だった型）
- **推測せず先に計測する**。見積もりは本系列で何度も外れた（#12・c-2・c-3・(b)(i)・#10-c・
  #27 の帰属・#27-c の「複雑さ」…一覧は実装ログ末尾の表）。診断フックを足して数字を見てから設計を決めること。
- **検証は 4 点セット**: `cargo build`（**警告 0**）・`cargo test`（**706 緑**）・
  [compare_vm_modes.ps1](compare_vm_modes.ps1)（off/auto byte-identical）・
  [scan_examples.ps1](scan_examples.ps1)（例題 **FAIL 0**）。⚠ **前 2 つは release バイナリを見る**。
...
- **高リスク低リターンと判断したらスキップして保留にし、理由を記録する**（勝手に大改造しない）。
  ...
  ⚠ **タスクの定義が目的に対して過剰なら定義を見直す**（#27-a は 644 行の畳み込みと定義されていたが
  目的は 50 行で足りた／#27-c の `try/except/finally` は入れ子で足りた／
  **#27-d は「フレーム表現の変更が必須」と書かれていたが、必須なのは可変キャプチャだけだった**）。
...
| [force_gate.ps1](force_gate.ps1) | **#3 へ進めるかのゲート**（#25）。全例題を `--vm=force` で実行し `VmForceError` を列挙。⚠ **止めて判定する**用途で、件数は `tw_stats.ps1` で見ること |
...
- **`compare_vm_modes.ps1` は「両モードともツリーウォークに落ちる形」を検知できない**（#27）。
  VM が bail する構文は off/auto が同じ経路になるので必ず一致する。**bail する形はツリーウォークが
  正しいとは限らない**（`for-target-shadow` で実バグ。基準は `python -m impl_python` の出力）。
...
- **`compare_vm_modes.ps1` / `scan_examples.ps1` は `target/release` を見る**。`cargo build`（debug）だけして「直った」と判断しないこと（#10-b で 1 度踏んだ）。
- **`$ErrorActionPreference='Stop'` のスクリプトから `cargo` を呼ぶと進捗の stderr で終了エラーになる**。その呼び出しの間だけ `Continue` に落とす（`tw_stats.ps1` 参照）。
...
**完了項目の一覧は区切り線の下（「参照・根拠」冒頭）へ移した**（規約 3: 確定済み・履歴は下へ）。
実測値・判断の経緯は [IMPLEMENTATION_LOG.md](IMPLEMENTATION_LOG.md)。
...
| 3 | 強制バイトコード（D2） | フォールバック撤去＋TLS4本/センチネル2種の実削除 | **← `force_gate.ps1` が 0 件**（現在 **2/122**・**全て可変キャプチャ**） | ブロック中（本系列の最終段・**残るブロッカーは #27-d 段階 2b だけ**） |
...
```
27（残り 4 bail）＋ 27-d 段階 2b → 3   ← 残る原因は **可変キャプチャ 1 種だけ**（27-c と 27-d 段階 1/2a は完了）
モジュール間ネイティブ直リンク（未計画）→ 14 → 11 R2-c ／ 12b → 2c は循環依存で両方保留
判定は **`force_gate.ps1` が 0 件**（現在 2/122）。定義文は対象外（#10-d 保留）
```
