...
1. **保守性レーン（#55 → #54）** — 2026-08-19 の全 src 診断で起票。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| **56** | **`is_builtin_callee` の bail が #33 で実バグ化** | 「bail＝ツリーウォークへ落とす」前提が消えたのに bail し続けている。`parse_ar` が **`VmForceError` で完全に死んでいる**／`tuple`/`list`/`type`/`byte` は `NameError` が `VmForceError` に化ける。VM に載せるか `Op::Fail` で正しい文言を出す（#34 の手法） | — | **未着手・実バグ** |
| 54 | native typed ABI 呼び出しの 1 本化 | 3 経路に手書きされた `transmute`＋呼び出し＋cleanup を 1 関数へ（#23 を「速度」から「2 実装の畳み込み」へ再定義した縮小版） | —（55 完了で解除。⇒ **「消す」でなく「畳む」**） | 未着手 |
...
> **54/55 は保守性レーン**（2026-08-19 の全 src 診断で起票。**51 / 52 は完了**）。速度目的ではないので
...
残り: 速度目的は無し。**保守性レーン 54/55**（診断で起票・51/52/53 は完了）／別レーン（外部接続系）／
      ブロック中の 14 / 11 R2-c
