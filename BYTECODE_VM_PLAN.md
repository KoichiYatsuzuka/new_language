...
1. **保守性レーン（#52 → #53 ／ #55 → #54）** — 2026-08-19 の全 src 診断で起票。
   **#51 は完了**（実バグ 1 件を回収）。速度目的ではないので A/B は**退行が無いことの確認**に使う。
...
| # | タスク | 手法 | 前提（依存） | 状態 |
|---|---|---|---|---|
| 52 | `CompileMode` 導入 ＋ `Compiler::base`/`finish` | 4 モードフラグを enum 1 本に畳み、38 フィールド×5／24 フィールド×5 の構造体リテラルを既定値＋差分へ | — | 未着手 |
| 53 | `vm/compiler.rs` のサブ分割 | §5.1 の実装差分（stmt/expr/block_expr/decls/emit）を実施 | **← 52**（モード整理後の方が差分が小さい） | 未着手 |
...
> **52〜55 は保守性レーン**（2026-08-19 の全 src 診断で起票。**51 は完了**）。速度目的ではないので
...
残り: 速度目的は無し。**保守性レーン 52〜55**（診断で起票・51 は完了）／別レーン（外部接続系）／
      ブロック中の 14 / 11 R2-c
