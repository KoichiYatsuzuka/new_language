...
5つのレバーを実装した（すべて解釈経路の per-access / per-call コスト削減）:
...
## 検証（安定性）
- `cargo test` → **672 passed / 0 failed**（デバッグビルドで **3種の一致 assert**（LocalRef slot / AttrCache slot / R4 呼び先）が全テストで発火せず＝キャッシュ結果が実行時解決と完全一致。`frame_floor` 隔離もクロージャ・再帰・async・メソッド・import 全テストで正しく動作。引数束縛の高速経路・空 defaults 許容も arity/kwargs/デフォルト/可変長を含め全テスト緑）。
...
- **解釈経路が全面的に 1.27〜1.72x 高速化**（E2E フィールドアクセス 1.62〜1.69x、微ベンチ最大 1.72x、全テスト緑、回帰0）。
- 段階ごとの主因:
...
- BYTECODE_VM_PLAN 投影の **Phase R 1.3〜2x を達成**（フィールド/呼び出し支配で ~1.6-1.7x）。
- **まだツリーウォークのまま**の支配項: `Value` clone・命令ディスパッチ・算術・メソッド呼び出し。

## 次に効くレバー（このスライスが土台）
1. **`Value` clone 削減（§7.4）** — `Value::Str(Rc<str>)` 等。文字列ワークロードに効く。
2. **Phase V バイトコード VM** — 命令ディスパッチ・制御フロー・算術・メソッド呼び出しをまとめて潰す本命。
3. **メソッド呼び出しの IC** — `eval_method_call` の `lookup_method_in_class` 辞書引きを class_id IC へ（R3 の属性 IC を CALL_METHOD 相当へ拡張）。

`LocalRef` / slot 化 `Scope` / `frame_floor` フレームモデル / bind_args 高速経路 / `AttrCache` / R4 呼び先キャッシュ / リゾルバは上記すべての土台として再利用される。
