...
### #1 本体（VM 内の文単位ブレーク）は**未着手のまま**

残る代償は **StepInto のときだけ**（次に入る 1 フレームがツリーウォーク）。本体に必要なのは:

1. op→span の行テーブル（⚠ `peephole.rs` が命令を削るので**同時に直す必要がある**）
2. **ディスパッチループへの停止判定** — ⚠ ここが問題。文境界ごとの検査はホットループに入り、
   #12/#2b/#2a で削ってきたコストを戻す。`run()` 入口で 1 回だけ分岐して
   専用ループへ入れば通常経路は無傷にできるが、**セッション開始時に既に走っている VM フレーム**が
   停止判定を持たないという穴が残る（この穴は現行実装にも既にある）。
3. 停止 VM フレームのローカルを REPL から参照する経路（`chunk.local_names` ＋ flat buffer。
   `local_names` は V-E で用意済みだが**まだ消費者ゼロ**）。

→ **#1-b で実利の大半（step-over の 1.97x）が取れた**ので、本体は
「StepInto の 1 フレームだけのために、ホットループへ停止判定を入れる価値があるか」という判断になる。
着手するなら **#1-a の安全網が緑であることを条件**にすること。
