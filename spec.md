...
- C++, C#, Rustのdll読み込み: .hファイルとかを本言語の字句へと割り当てる必要あり。
...
- 非可変属性と定数属性への再代入は静的なエラーとなる。
- 非可変同士の値代入は参照のコピー、それ以外は値のコピーとなる。値コピーを行うため、Pythonのようにコピー先を編集することでコピー元の値が書き変わることはない。
- 関数の引数にも可変属性と非可変属性がある。可変属性引数に非可変属性変数を使うと静的なエラーになる（非可変が保証されないため）。非可変属性引数の可変属性引数を使うと値のコピーを行う（関数実行中に値が変わらないことを保証）。可変属性引数に可変属性引数を与えると参照を渡す（値の書き換えの許可）。
- 可変属性にfreezeを使うと非可変属性になる。
- 適切に扱うことで、借用の概念を使うことなく、値コピーによるオーバーヘッドを防ぎつつ、書き変わりうる値の追跡が容易になる。
