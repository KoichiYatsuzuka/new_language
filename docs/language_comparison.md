...
| Force source | — | — | `import[tl] module` |
| Force compiled | — | — | `import[tlc] module` |
| Python interop | — | native | `import[py] module` / `import[py-int]` |
| Auto (prefer compiled) | — | — | `import module` (prefers `.tlc` if present) |
...
| Partial compile to native | `cargo run -- --compile mod.tl` | Emits `.tlc` (compiled binary) + `.tls` (type stub for IDE) |
| Native dispatch | auto on import | `import` prefers `.tlc`; eligible fns run as native code (~100–200× faster for typed int/float) |
