...
⚠ **バグ修正 B1〜B13**（Python→Arrow 変換器の作業中に見つかった 13 件）のまとめは
[BUGFIX_B1_B13.md](BUGFIX_B1_B13.md) へ分けてある。
辞書のキー・等値・`let` の不変性・`for` のスコープ・ジェネレータのコルーチン化など、
**言語仕様の決定になったもの**が含まれる。
...
## bug_fix.md B6 — モジュール本体からの呼び出しと `LoadName` の単価（2026-09-03・完了）

### 直したこと

被呼び出し側の名前ロードが**読み側と違う判定**をしていた。読みは `reads_by_name()` で
`LoadName` / `LoadGlobal` を切っていたのに、呼び出しは `is_debug_repl()` だけを特別扱いし、
残りを `LoadGlobal`（`scopes[0]` 限定）に落としていた。

`exec_module` は本体の前に `push_scope()` するので、**モジュール本体の宣言は `scopes[0]` に
入らない**。⇒ 参照は通るのに呼び出しだけが `NameError` になった。最上位が動いていたのは
宣言が `scopes[0]` そのものだからという偶然。逆アセンブルで確定:
...
⇒ 判定を `emit_callee_load` 1 箇所に閉じ、読み側と同じ `reads_by_name()` にした。
同型が `mng <- async` のマネージャ読み（`calls.rs`）にもあり、こちらも実測で再現・修正した。

### 実測（`ab_bench` min・**負の対照を同セッションで取得**）

| 対象 | 負の対照 | A/B（旧→新） | 意味 |
|---|---|---|---|
| `bench_name_vs_global.ar`（最上位から 400 万回呼ぶだけ） | 0.997x | **0.970x** | 3.0% 遅い |
| `bottleneck_bench.ar`（全体が最上位ホットループ） | 1.005x | **0.943x** | **5.7% 遅い**（実質の上限） |
| `bench_toplevel_loop.ar` | — | 0.987x | |
| `bench_ab_interp.ar` | — | 0.986x | |

壁時計差 0.043s ÷ 400 万呼び出し = **約 11 ns/call**（`LoadName` − `LoadGlobal`）。

⚠ **区間別の printed metric は使い物にならない**（同一バイナリ内で 35% 揺れる区間がある）。
壁時計 min ＋ 負の対照でしか判断しないこと。
⚠ 最初に「3 つのループを並べて比べる」形で測って **−10ns という符号の誤り**を出した。
最上位ループは制御変数まで自由名になるので、ロード命令以外が大量に混ざる。
⇒ **被演算子 1 つだけが違う形**に揃えること。

### 変数アクセスの単価ラダー

| 経路 | 追加コスト | 中身 |
|---|---|---|
| `LoadLocal` | 基準 | フレーム配列の index 読み |
| `LoadGlobal` | **+13〜27 ns**（min 13.3・中央 ~21） | 索引キャッシュ命中でも epoch 検査＋値 clone |
| `LoadName` | さらに **+約 11 ns** | `get_val` のスコープ鎖＋名前ハッシュ（**キャッシュなし**） |

別プローブ（`bench_toplevel_loop.ar`）: 最上位ループは fn 内の **1.29〜1.51 倍**。
⚠ ただしこれは融合命令（`IntBinLL`）の有無も含むので、ロード単体の比ではない。

### #11 R2-c で回収できる見込み（概算）

**(a) B6 で払った分**: 約 11 ns/call ⇒ 最上位呼び出しが支配的なコードで **3.0〜5.7%**。

**(b) それ以上** ← ⚠ **こちらが本命**:
**最上位の変数「読み」は B6 の前から `LoadName`** だった（`reads_by_name()`）。つまり
#11 R2-c は B6 の回収だけでなく、**最上位・モジュール本体の全ての自由名アクセス**を
索引化できる。上限の目安は上の「1.29〜1.51 倍」＝ **最上位コードで 22〜34% の短縮**。

⚠⚠ **(b) は上限であって見込みではない。** 1.29〜1.51 倍の内訳には
「名前引き」と「slot が無いので融合命令が効かない」の**2 つ**が入っており、
#11 R2-c が後者まで解くかは設計次第。⇒ 着手時は**まず内訳を分離して測る**こと。
⚠ モジュール本体は実行時に宣言が増えるので、配列の採番と無効化の設計が要る。

---

## R3 属性 IC の再測と「多相 IC」の語の確定（2026-09-08）

型束縛検査の作業（[type_binding_enforcement_plan.md](../implementation_plans/type_binding_enforcement_plan.md)）の
最終フェーズとして、[BYTECODE_VM_PLAN.md](BYTECODE_VM_PLAN.md) §4.3 の **R3** に
未実装分が残っていないかを確認した。**結論: 残っていない。**

### ⚠ 「多相 IC」は N-way の意味ではない（読み違えの記録）
...
この「前回と同じなら再利用・違えば引き直して上書き」は**エントリ 1 個の inline cache**
そのもので、[ast.rs](../src/ast.rs) の `AttrCache`（`Cell<u64>` に
`class_id | access | slot` をパック）と一致する。⇒ **計画の「多相 IC」は
「多相な呼び出し点でも（引き直して）動く IC」の意**であり、複数エントリを指していない。
`ast.rs` が同じ機構を「単相 IC」と呼んでいるのは inline cache の標準的な用語
（monomorphic = エントリ 1 個）に従ったもので、**両者は同じコードを別の言葉で説明している**。

⚠⚠ 作業開始時、この語を N-way の意味に読んで「計画と実装に差がある」と報告してしまった。
**同じ機構に 2 つの呼び名が付いている**のが原因なので、R3 行の側に注記を足した。

### 計測基盤（新規）

| 対象 | 内容 |
|---|---|
| `prof::IC_HIT` / `IC_COLD` / `IC_POLY` | ミスを **cold**（キャッシュが空＝初回）と **poly**（class_id 違い＝多相）に分ける |
...
⚠ **分けずに「ミス率」だけ見ても判断できない。** N-way 化で救えるのは poly だけ。
⚠ **テンプレート実体化クラスのメモ化（Phase T）より後でないと測れない。** メモ化前は
実体化のたびに新しい `class_id` を発行していたので、多相性ではなく**キャッシュ欠落**を測る。
...
例題が多相ワークロードを含まないだけの可能性があるので、意図的に多相な地点を作って測った
（4 クラスを交替させて属性を読む／同じ probe 数の単相版と比較）:

| | probe | hit | poly miss | 実行時間 |
|---|---|---|---|---|
| 単相（1 クラス） | 3,200,000 | 100% | 0 | 383 / 393 ms |
| 多相（4 クラス交替） | 3,200,000 | **0%** | **100%** | 709 / 707 ms |

⇒ 多相地点は**約 1.8x 遅い**。病理そのものは実在するが、R3 行が
「できねば辞書アクセス」と明示的に許容していた範囲の挙動である。

### 判断

**R3 は完了として閉じる。** N-way 化は原計画に無く、起票するなら新規タスクになるが、
判定基準（「**消費者が居るか**」— #11 R2-c / #14 / #15b を保留にしたのと同じ基準）に照らして
**現時点で消費者が居ない**（9 / 7,740 万）。

⇒ 再開の条件は「多相な属性アクセスがホットなワークロードが現れること」。
そのときは `prof_attr_ic.ps1` で**先に測ってから**規模を決める。
設計案（記録のみ・未着手）: エントリ 0 を先に見る **2-way** ならヒット経路のコストは不変で、
`AttrCache` は 8 → 16 バイト。N-way にするなら mono → poly(N) → megamorphic の状態遷移が要る
（埋め直しの空回りを止めるため）。⚠ どちらも `AttrCache::Clone` が空を返す不変条件
（AST コピーごとの再解決）を壊さないこと。

### ⚠ 副作用: op を足すときの注意を 1 つ増やした

`Op::CoerceFloat`（案 B・型束縛検査で追加）を **`enum Op` の末尾へ移した**。
[vm/op_prof.rs](../src/vm/op_prof.rs) が**宣言順のインデックス表**（`--features prof` 専用・
自動生成）を持っており、途中へ挿すと以降の採番が全部ずれる。`op.rs` の末尾に
「新しい op は必ずここへ足す」と明記した。
⚠ 併せて `--features prof` / `--features tw_stats` のビルドも通した
（`op_prof.rs` 自身が「既定ビルドでは気づけない」と警告している箇所）。
