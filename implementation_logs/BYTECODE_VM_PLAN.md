...
⇒ 実行系は一段落した（2026-08-19）。**今すぐ着手できるのはこの 3 件だけ**（2026-08-24 時点。うち #19 は 2026-09-19 に完了）。
⚠ #19 は完了（2026-09-19）。前提だった型検査の食い違い（INF-D）を先に直してから入れた —— 逆順だと
`time.sleep(0.01)` が静的エラーになる純粋な退行だった。残りは #17-a / #17-b の 2 件。
